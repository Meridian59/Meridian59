				 ...
				 int num_name_parms,parm_node name_parm_array[])
{
	...
	{
		bprintf("C_SendPacket object %i can't send to non-session %i,%li\n",
		...
		case TAG_STRING :
			{
				...
				if (snod == NULL)
				{
					bprintf("C_Debug can't find string %li\n",each_val.v.data);
					return NIL;
				}
					  ...
					  int num_name_parms,parm_node name_parm_array[])
{
	...
	{
		bprintf("C_GetInactiveTime can't use non-session %i,%li\n",
	...
	if (s == NULL)
	{
		bprintf("C_GetInactiveTime can't find session %li\n",session_val.v.data);
		return NIL;
	}
	if (s->state != STATE_GAME)
	{
		bprintf("C_GetInactiveTime can't use session %li in state %i\n",
				  ...
				  int num_name_parms,parm_node name_parm_array[])
{
	...
	{
		bprintf("C_SendMessage OBJECT %i can't send non-message %i,%li\n",
	...
	else if (object_val.v.tag != TAG_OBJECT)
	{
		...
		bprintf("C_SendMessage OBJECT %i CLASS %s can't send MESSAGE %s (%li) to non-object %i,%li\n",
			object_id,
				  ...
				  int num_name_parms,parm_node name_parm_array[])
{
	...
	if (message_val.v.tag != TAG_MESSAGE)
	{
		bprintf("C_PostMessage OBJECT %i can't send non-messsage %i,%li\n",
			object_id,message_val.v.tag,message_val.v.data);
		return NIL;
	}
	...
	{
		/* assumes object_id (the current 'self') is a valid object */
		bprintf("C_PostMessage OBJECT %i CLASS %s can't send MESSAGE %s (%li) to non-object %i,%li\n",
			object_id,
			GetClassByID(GetObjectByID(object_id)->class_id)->class_name,
			GetNameByID(message_val.v.data), message_val.v.data,
			object_val.v.tag,object_val.v.data);
				   ...
				   int num_name_parms,parm_node name_parm_array[])
{
	...
	{
		bprintf("C_CreateObject can't create non-class %i,%li\n",
			  ...
			  int num_name_parms,parm_node name_parm_array[])
{
	...
	if (object_val.v.tag != TAG_OBJECT)
	{
		bprintf("C_IsClass can't deal with non-object %i,%li\n",
			object_val.v.tag,object_val.v.data);
		return NIL;
	}
	...
	if (class_val.v.tag != TAG_CLASS)
	{
		bprintf("C_IsClass can't look for non-class %i,%li\n",
			class_val.v.tag,class_val.v.data);
		return NIL;
	}
	...
	if (o == NULL)
	{
		bprintf("C_IsClass can't find object %li\n",object_val.v.data);
		return NIL;
	}
			   ...
			   int num_name_parms,parm_node name_parm_array[])
{
	...
	if (object_val.v.tag != TAG_OBJECT)
	{
		bprintf("C_GetClass can't deal with non-object %i,%li\n",
			object_val.v.tag,object_val.v.data);
		return NIL;
	}
	...
	if (o == NULL)
	{
		bprintf("C_GetClass can't find object %li\n",object_val.v.data);
		return NIL;
	}
...
bool LookupString(val_type val, const char *function_name, const char **str, int *len)
{
	...
	case TAG_STRING :
		...
		if (snod == NULL)
		{
			bprintf( "%s can't use invalid string %i,%li\n",
	...
	case TAG_RESOURCE :
		...
		if( r == NULL )
		{
			bprintf( "%s can't use invalid resource %li as string\n",
   ...
   case TAG_NIL:
	...
	default :
		bprintf( "%s can't use with non-string thing %i,%li\n",
               function_name, val.v.tag, val.v.data );
		return false;
	}
					   ...
					   int num_name_parms,parm_node name_parm_array[])
{
   ...
   const char *s1, *s2, *subspot;
	...
	if (snod0 == NULL)
	{
		bprintf( "C_StringSub can't modify first argument non-string %i,%li\n",
	...
	case TAG_STRING :
		...
		if( snod1 == NULL )
		{
			bprintf( "C_StringSub can't sub for invalid string %i,%li\n",
	...
	case TAG_RESOURCE :
		...
		if( r == NULL )
		{
			bprintf( "C_StringSub can't sub for invalid resource %li\n", s1_val.v.data);
			return NIL;
		}
		s1 = r->resource_val;
	...
	default :
		bprintf( "C_StringSub can't sub for non-string thing %i,%li\n",
			s1_val.v.tag, s1_val.v.data );
		return NIL;
	}
	...
	{
		bprintf( "C_StringSub can't sub for null string %i,%li\n",
			s1_val.v.tag, s1_val.v.data );
		return NIL;
	}
				  ...
				  int num_name_parms,parm_node name_parm_array[])
{
	...
	{
		bprintf("C_SetResource can't set non-resource %i,%li\n",
	...
	{
		bprintf("C_SetResource can't set non-dynamic resource %i,%li\n",
			drsc_val.v.tag,drsc_val.v.data);
		return NIL;
	}
	...
	case TAG_RESOURCE :
		{
			...
			if (r == NULL)
			{
				bprintf("C_SetResource can't set from bad resource %li\n",
					str_val.v.data);
				return NIL;
			}
	...
	default :
		bprintf("C_SetResource can't set from non temp string %i,%li\n",
				  ...
				  int num_name_parms,parm_node name_parm_array[])
{
	...
	{
		bprintf("C_ParseString can't parse non-temp string %i,%li\n",
	...
	{
		bprintf("C_ParseString can't use separator non-debugstr %i,%li\n",
	...
	{
		bprintf("C_ParseString can't callback non-message %i,%li\n",
				...
				int num_name_parms,parm_node name_parm_array[])
{
	...
	{
		bprintf("C_SetString can't set non-string %i,%li\n",
			s1_val.v.tag,s1_val.v.data);
		return NIL;
	}
	...
	if (snod == NULL)
	{
		bprintf("C_SetString can't set invalid string %i,%li\n",
			s1_val.v.tag,s1_val.v.data);
		return NIL;
	}
	...
	case TAG_STRING :
		...
		if( snod2 == NULL )
		{
			bprintf( "C_SetString can't find string %i,%li\n",
	...
	case TAG_RESOURCE :
		...
		if (r == NULL)
		{
			bprintf("C_SetString can't set from invalid resource %li\n",s2_val.v.data);
			return NIL;
		}
	...
	default :
		bprintf("C_SetString can't set from non-string thing %i,%li\n",
			s2_val.v.tag,s2_val.v.data);
		return NIL;
	}
					   ...
					   int num_name_parms,parm_node name_parm_array[])
{
	...
	case TAG_STRING :
		...
		if(snod == NULL )
		{
			bprintf( "C_AppendTempString can't find string %i,%li\n", s_val.v.tag, s_val.v.data );
			return NIL;
		}
	...
	case TAG_RESOURCE :
		...
		if (r == NULL)
		{
			bprintf("C_AppendTempString can't set from invalid resource %li\n",s_val.v.data);
			return NIL;
		}
	...
	default :
		bprintf("C_AppendTempString can't set from non-string thing %i,%li\n",s_val.v.tag,s_val.v.data);
		return NIL;
	}
	return NIL;
}
				  ...
				  int num_name_parms,parm_node name_parm_array[])
{
	...
	if (object_val.v.tag != TAG_OBJECT)
	{
		bprintf("C_CreateTimer can't create a timer for non-object %i,%li\n",
			object_val.v.tag,object_val.v.data);
		return NIL;
	}
	...
	if (message_val.v.tag != TAG_MESSAGE)
	{
		bprintf("C_CreateTimer can't create timer w/ non-message id %i,%li\n",
	...
	{
		bprintf("C_CreateTimer can't create timer in negative int %i,%li milliseconds\n",
	...
	{
		bprintf("C_CreateTimer can't create timer w/ message %li not for class %i\n",
				  ...
				  int num_name_parms,parm_node name_parm_array[])
{
	...
	{
		bprintf("C_DeleteTimer can't delete non-timer %i,%li\n",
					   ...
					   int num_name_parms,parm_node name_parm_array[])
{
	...
	if (timer_val.v.tag != TAG_TIMER)
	{
		bprintf("C_GetTimeRemaining can't use non-timer %i,%li\n",
			timer_val.v.tag,timer_val.v.data);
		return NIL;
	}
	...
	if (t == NULL)
	{
		bprintf("C_GetTimeRemaining can't find timer %i,%li\n",
			timer_val.v.tag,timer_val.v.data);
		return NIL;
	}
			   ...
			   int num_name_parms,parm_node name_parm_array[])
{
	...
	{
		bprintf("C_CreateRoomData can't use non-resource %i,%li\n",
			   ...
			   int num_name_parms,parm_node name_parm_array[])
{
	...
	{
		bprintf("C_RoomSize can't operate on non-room %i,%li\n",
			room_val.v.tag,room_val.v.data);
		return NIL;
	}
	...
	if (room == NULL)
	{
		bprintf("C_RoomSize can't find room id %li\n",room_val.v.data);
		return NIL;
	}
					...
					int num_name_parms,parm_node name_parm_array[])
{
    ...
    */
	...
	if (room_val.v.tag != TAG_ROOM_DATA)
	{
		bprintf("C_CanMoveInRoom can't use non room %i,%li\n",
			room_val.v.tag,room_val.v.data);
		return ret_val.int_val;
	}
	...
	{
		bprintf("C_CanMoveInRoom row source can't use non int %i,%li\n",
	...
	{
		bprintf("C_CanMoveInRoom col source can't use non int %i,%li\n",
	...
	{
		bprintf("C_CanMoveInRoom col dest can't use non int %i,%li\n",
	...
	if (r == NULL)
	{
		bprintf("C_CanMoveInRoom can't find room %li\n",room_val.v.data);
		return ret_val.int_val;
	}
						   ...
						   int num_name_parms,parm_node name_parm_array[])
{
    ...
    */
	...
	if (room_val.v.tag != TAG_ROOM_DATA)
	{
		bprintf("C_CanMoveInRoomFine can't use non room %i,%li\n",
			room_val.v.tag,room_val.v.data);
		return ret_val.int_val;
	}
	...
	if (row_source.v.tag != TAG_INT)
	{
		bprintf("C_CanMoveInRoomFine row source can't use non int %i,%li\n",
			row_source.v.tag,row_source.v.data);
		return ret_val.int_val;
	}
	...
	if (col_source.v.tag != TAG_INT)
	{
		bprintf("C_CanMoveInRoomFine col source can't use non int %i,%li\n",
			col_source.v.tag,col_source.v.data);
		return ret_val.int_val;
	}
	...
	if (row_dest.v.tag != TAG_INT)
	{
		bprintf("C_CanMoveInRoomFine col dest can't use non int %i,%li\n",
			row_dest.v.tag,row_dest.v.data);
		return ret_val.int_val;
	}
	...
	if (r == NULL)
	{
		bprintf("C_CanMoveInRoomFine can't find room %li\n",room_val.v.data);
		return ret_val.int_val;
	}
			...
			int num_name_parms,parm_node name_parm_array[])
{
	...
	{
		bprintf("C_First object %i can't take First of a non-list %i,%li\n",
			object_id,list_val.v.tag,list_val.v.data);
		return NIL;
	}
	if (!IsListNodeByID(list_val.v.data))
	{
		bprintf("C_First object %i can't take First of an invalid list %i,%li\n",
			object_id,list_val.v.tag,list_val.v.data);
		return NIL;
	}
		   ...
		   int num_name_parms,parm_node name_parm_array[])
{
	...
	if (list_val.v.tag != TAG_LIST)
	{
		bprintf("C_Rest object %i can't take Rest of a non-list %i,%li\n",
			object_id,list_val.v.tag,list_val.v.data);
		return NIL;
	}
	if (!IsListNodeByID(list_val.v.data))
	{
		bprintf("C_Rest object %i can't take Rest of an invalid list %i,%li\n",
			object_id,list_val.v.tag,list_val.v.data);
		return NIL;
	}
			 ...
			 int num_name_parms,parm_node name_parm_array[])
{
	...
	if (list_val.v.tag != TAG_LIST)
	{
		bprintf("C_Length object %i can't take Length of a non-list %i,%li\n",
			object_id,list_val.v.tag,list_val.v.data);
		return NIL;
	}
	ret_val.v.tag = TAG_INT;
		  ...
		  int num_name_parms,parm_node name_parm_array[])
{
	...
	if (list_val.v.tag != TAG_LIST)
	{
		bprintf("C_Nth object %i can't take Nth of a non-list %i,%li\n",
			object_id,list_val.v.tag,list_val.v.data);
		return NIL;
	}
	if (!IsListNodeByID(list_val.v.data))
	{
		bprintf("C_Nth object %i can't take Nth of an invalid list %i,%li\n",
			object_id,list_val.v.tag,list_val.v.data);
		return NIL;
	}
	...
	{
		bprintf("C_Nth can't take Nth with n = non-int %i,%li\n",
			   ...
			   int num_name_parms,parm_node name_parm_array[])
{
	...
	if (list_val.v.tag != TAG_LIST)
	{
		bprintf("C_SetFirst object %i can't set elem of non-list %i,%li\n",
			object_id,list_val.v.tag,list_val.v.data);
		return NIL;
	}
			 ...
			 int num_name_parms,parm_node name_parm_array[])
{
	...
	if (n_val.v.tag != TAG_INT)
	{
		bprintf("C_SetNth object %i can't take Nth with n = non-int %i,%li\n",
				  ...
				  int num_name_parms,parm_node name_parm_array[])
{
	...
	if (list_val.v.tag != TAG_LIST)
	{
		bprintf("C_DelListElem object %i can't delete elem from non-list %i,%li\n",
			object_id,list_val.v.tag,list_val.v.data);
		return NIL;
	}
				   ...
				   int num_name_parms,parm_node name_parm_array[])
{
	...
	if (list_val.v.tag != TAG_LIST)
	{
		bprintf("C_FindListElem object %i can't find elem in non-list %i,%li\n",
			object_id,list_val.v.tag,list_val.v.data);
		return NIL;
	}
                        ...
                        int num_name_parms,parm_node name_parm_array[])
{
	...
	if (list_val.v.tag != TAG_LIST)
	{
		bprintf("C_MoveListElem object %i can't move elem in non-list %i,%li\n",
            object_id,list_val.v.tag,list_val.v.data);
		return NIL;
	}
	...
	if (n_val.v.tag != TAG_INT)
	{
		bprintf("C_MoveListElem object %i can't lookup non-int index %i,%li\n",
            object_id, n_val.v.tag, n_val.v.data);
    return NIL;
  }
			 ...
			 int num_name_parms,parm_node name_parm_array[])
{
	...
	{
		bprintf("C_Random got an invalid boundary %i,%li or %i,%li\n",
	...
	{
		bprintf("C_Random got low > high boundary %li and %li\n",
			...
			int num_name_parms,parm_node name_parm_array[])
{
	...
	if (int_val.v.tag != TAG_INT)
	{
		bprintf("C_Bound can't use %i,%li\n",int_val.v.tag,int_val.v.data);
		return NIL;
	}
	...
	if (min_val.v.tag != TAG_NIL)
	{
		...
		{
			bprintf("C_Bound can't use min bound %i,%li\n",min_val.v.tag,min_val.v.data);
			return NIL;
		}
	...
	if (max_val.v.tag != TAG_NIL)
	{
		...
		{
			bprintf("C_Bound can't use max bound %i,%li\n",max_val.v.tag,max_val.v.data);
			return NIL;
		}
					...
					int num_name_parms,parm_node name_parm_array[])
{
	...
	if (int_val.v.tag != TAG_INT)
	{
		bprintf("C_AddTableEntry can't use table id %i,%li\n",int_val.v.tag,int_val.v.data);
		return NIL;
	}
					...
					int num_name_parms,parm_node name_parm_array[])
{
	...
	if (int_val.v.tag != TAG_INT)
	{
		bprintf("C_GetTableEntry can't use table id %i,%li\n",int_val.v.tag,int_val.v.data);
		return NIL;
	}
					   ...
					   int num_name_parms,parm_node name_parm_array[])
{
	...
	if (int_val.v.tag != TAG_INT)
	{
		bprintf("C_DeleteTableEntry can't use table id %i,%li\n",int_val.v.tag,int_val.v.data);
		return NIL;
	}
				  ...
				  int num_name_parms,parm_node name_parm_array[])
{
	...
	if (int_val.v.tag != TAG_INT)
	{
		bprintf("C_DeleteTable can't use table id %i,%li\n",int_val.v.tag,int_val.v.data);
		return NIL;
	}
				  ...
				  int num_name_parms,parm_node name_parm_array[])
{
	...
	if (object_val.v.tag != TAG_OBJECT)
	{
		bprintf("C_RecycleUser can't recycle non-object %i,%li\n",object_val.v.tag,object_val.v.data);
		return NIL;
	}
	...
	if (o == NULL)
	{
		bprintf("C_RecycleUser can't find object %li\n",object_val.v.data);
		return NIL;
	}
	...
	if (old_user == NULL)
	{
		bprintf("C_RecycleUser can't find user which is object %li\n",object_val.v.data);
		return NIL;
	}
				...
				int num_name_parms,parm_node name_parm_array[])
{
   ...
   int index, iTemp;
	...
	if (s1_val.v.tag != TAG_STRING)
	{
		bprintf("C_MinigameNumberToString can't set non-string %i,%li\n",
			s1_val.v.tag,s1_val.v.data);
		return NIL;
	}
	...
	if (snod == NULL)
	{
		bprintf("C_MinigameNumberToString can't set invalid string %i,%li\n",
			s1_val.v.tag,s1_val.v.data);
		return NIL;
	}
	...
	{
		bprintf("C_MinigameNumberToString can't set from non-int %i,%li\n",
			s2_val.v.tag,s2_val.v.data);
		return NIL;
	}
