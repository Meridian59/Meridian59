...
blak_int InsertListElem(int n, int list_id, val_type new_val)
{
   ...
   for (int i = 2; i <= n; i++)
   {
      ...
      if (l->rest.v.tag != TAG_LIST)
      {
         ...
         // Use temp id if element we're adding is a list, in case allocate resized memory.
         if (temp_list_id >= 0)
         {
   ...
   // This node is inserted in position of the existing one, so use its rest data points to the old node.
