...
val_type __inline RetrieveValue(int object_id,local_var_type *local_vars,int data_type,blak_int data)
{
   object_node *o;
   class_node *c;
   val_type ret_val;
   ...
   if (data_type == LOCAL_VAR)
   {
	 ...
	 if (local_vars->locals[data].v.tag == TAG_INVALID)
	    eprintf("[%s] RetrieveValue got value of local or parameter that was uninitialized (INVALID %lli)\n",
   ...
   if (data_type == PROPERTY)
   {
	 ...
	 if (o->p[data].val.v.tag == TAG_INVALID)
	    eprintf("[%s] RetrieveValue got value of property that was uninitialized (INVALID %lli)\n",
	       BlakodDebugInfo(),local_vars->locals[data].v.data);
      }
   ...
   if (data_type == CLASS_VAR)
   {
	 ...
	 return ret_val;
      ...
      {
	 eprintf("[%s] RetrieveValue can't retrieve invalid class var %lli in OBJECT %i CLASS %s (%i)\n",
