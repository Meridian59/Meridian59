...
void D3DRenderLMapsPostDraw(const WorldRenderParams &worldRenderParams,
                            const LightAndTextureParams &lightAndTextureParams, BSPnode *tree, bool transparent_pass);

void D3DRenderLMapsDynamicPostDraw(const WorldRenderParams &worldRenderParams,
                                   const LightAndTextureParams &lightAndTextureParams, BSPnode *tree,
 ...
 */
long D3DRenderWorld(const WorldRenderParams &worldRenderParams, const WorldPropertyParams &worldPropertyParams,
                    const LightAndTextureParams &lightAndTextureParams)
{
                            ...
                            const LightAndTextureParams &lightAndTextureParams, BSPnode *tree, bool transparent_pass)
{
   ...
   case BSPinternaltype: {
      ...
      side = (a = tree->u.internal.separator.a) * player->x + (b = tree->u.internal.separator.b) * player->y +
                                   ...
                                   bool transparent_pass)
{
   ...
   case BSPinternaltype: {
      ...
      /* first, traverse closer side */
      if (side > 0)
         D3DRenderLMapsDynamicPostDraw(worldRenderParams, lightAndTextureParams, tree->u.internal.pos_side,
                                       transparent_pass);
      else
         D3DRenderLMapsDynamicPostDraw(worldRenderParams, lightAndTextureParams, tree->u.internal.neg_side,
                                       transparent_pass);
      ...
      if (side != 0)
      {
         ...
         for (pWall = tree->u.internal.walls_in_plane; pWall != NULL; pWall = pWall->next)
         {
            ...
            {
               D3DRenderLMapPostWallAdd(pWall, pools.lMapPool, D3DRENDER_WALL_NORMAL, side,
                                        lightAndTextureParams.lightCacheDynamic, true);
            }
      ...
      if (unlit < 4)
      {
         ...
         if (normal.x > normal.y)
         {
            for (i = 0; i < 4; i++)
            {
               ...
               st[i].t = (xyz[i].z - pDLightCache->dLights[numLights].xyz.z) * invZScale + 0.5f;
 ...
 */
void D3DGeometryBuildNew(const WorldRenderParams &worldRenderParams, const WorldPropertyParams &worldPropertyParams,
                         const LightAndTextureParams &lightAndTextureParams, bool transparent_pass)
{
   int count;
   BSPnode *pNode = NULL;
   WallData *pWall;
         ...
         case BSPinternaltype:
            for (pWall = pNode->u.internal.walls_in_plane; pWall != NULL; pWall = pWall->next)
            {
               ...
               if ((flags & D3DRENDER_WALL_NORMAL) && ((short) pWall->z2 != (short) pWall->z1) ||
                   ((short) pWall->zz2 != (short) pWall->zz1))
               {
                  D3DRenderLMapPostWallAdd(pWall, pools.lMapPoolStatic, D3DRENDER_WALL_NORMAL, 1,
                                           lightAndTextureParams.lightCache, false);
                  D3DRenderLMapPostWallAdd(pWall, pools.lMapPoolStatic, D3DRENDER_WALL_NORMAL, -1,
                                           lightAndTextureParams.lightCache, false);
               }
               ...
               if ((flags & D3DRENDER_WALL_BELOW) && ((short) pWall->z1 != (short) pWall->z0) ||
                   ((short) pWall->zz1 != (short) pWall->zz0))
               {
                  D3DRenderLMapPostWallAdd(pWall, pools.lMapPoolStatic, D3DRENDER_WALL_BELOW, 1,
                                           lightAndTextureParams.lightCache, false);
                  D3DRenderLMapPostWallAdd(pWall, pools.lMapPoolStatic, D3DRENDER_WALL_BELOW, -1,
                                           lightAndTextureParams.lightCache, false);
               }
               ...
               if ((flags & D3DRENDER_WALL_ABOVE) && ((short) pWall->z3 != (short) pWall->z2) ||
                   ((short) pWall->zz3 != (short) pWall->zz2))
               {
                  D3DRenderLMapPostWallAdd(pWall, pools.lMapPoolStatic, D3DRENDER_WALL_ABOVE, 1,
                                           lightAndTextureParams.lightCache, false);
                  D3DRenderLMapPostWallAdd(pWall, pools.lMapPoolStatic, D3DRENDER_WALL_ABOVE, -1,
                                           lightAndTextureParams.lightCache, false);
               }
      ...
      case BSPinternaltype:
         for (pWall = pNode->u.internal.walls_in_plane; pWall != NULL; pWall = pWall->next)
         {
            ...
            if ((flags & D3DRENDER_WALL_NORMAL) && ((short) pWall->z2 != (short) pWall->z1) ||
                ((short) pWall->zz2 != (short) pWall->zz1))
            {
               D3DRenderPacketWallMaskAdd(pWall, pools.wallMaskPool, D3DRENDER_WALL_NORMAL, 1,
            ...
            if ((flags & D3DRENDER_WALL_BELOW) && ((short) pWall->z1 != (short) pWall->z0) ||
                ((short) pWall->zz1 != (short) pWall->zz0))
            {
               D3DRenderPacketWallMaskAdd(pWall, pools.wallMaskPool, D3DRENDER_WALL_BELOW, 1,
                                          worldPropertyParams.noLookThroughTexture, false);
               D3DRenderPacketWallMaskAdd(pWall, pools.wallMaskPool, D3DRENDER_WALL_BELOW, -1,
                                          worldPropertyParams.noLookThroughTexture, false);
            }
            ...
            if ((flags & D3DRENDER_WALL_ABOVE) && ((short) pWall->z3 != (short) pWall->z2) ||
                ((short) pWall->zz3 != (short) pWall->zz2))
            {
               D3DRenderPacketWallMaskAdd(pWall, pools.wallMaskPool, D3DRENDER_WALL_ABOVE, 1,
                                          worldPropertyParams.noLookThroughTexture, false);
               D3DRenderPacketWallMaskAdd(pWall, pools.wallMaskPool, D3DRENDER_WALL_ABOVE, -1,
                                          worldPropertyParams.noLookThroughTexture, false);
            }
         ...
         for (curChunk = 0; curChunk < pPacket->curChunk; curChunk++)
         {
            ...
            for (i = 0; i < pChunk->numVertices; i++)
            {
               ...
               if (shadeAmount != 0)
               {
                  ...
                  lightScale = (long) (a * sunVect.x + b * sunVect.y) >> LOG_FINENESS;
            ...
            for (i = 0; i < pChunk->numVertices; i++)
            {
               D3DCacheBGRASet(pChunk->pRenderCache, pChunk->startIndex + i, pChunk->bgra[i].b, pChunk->bgra[i].g,
                               pChunk->bgra[i].r, pChunk->bgra[i].a);
            }
         }
      }
   }
                         ...
                         custom_bgra *pBGRA, unsigned int type, int side)
{
   ...
   if (pXYZ)
   {
      ...
      if (side > 0)
      {
         ...
         case D3DRENDER_WALL_BELOW: {
            if ((pWall->bowtie_bits & BT_BELOW_POS) || (pWall->bowtie_bits & BT_BELOW_NEG))
            {
   ...
   *flags = 0;
   ...
   if ((pXYZ) && (pST))
   {
      ...
      if (!drawTopDown)
      {
         ...
         if (pXYZ[1].z == pXYZ[2].z)
         {
            pST[1].t = 1.0f - (((float) yOffset * (float) pDib->shrink) * invWidth);
            pST[2].t = 1.0f - (((float) yOffset * (float) pDib->shrink) * invWidth);
         }
         else
         {
            pST[1].t = 1.0f - (((float) yOffset * (float) pDib->shrink) * invWidth);
            pST[2].t = 1.0f - (((float) yOffset * (float) pDib->shrink) * invWidth);
            pST[1].t -= ((float) pXYZ[1].z - bottom) * (float) pDib->shrink * invWidthFudge;
            pST[2].t -= ((float) pXYZ[2].z - bottom) * (float) pDib->shrink * invWidthFudge;
         }

         pST[0].t = pST[1].t - ((float) (pXYZ[0].z - pXYZ[1].z) * (float) pDib->shrink * invWidthFudge);
         pST[3].t = pST[2].t - ((float) (pXYZ[3].z - pXYZ[2].z) * (float) pDib->shrink * invWidthFudge);
      }
      else  // else, need to place tex origin at top left
      {
         ...
         {
            pST[0].t = ((float) top - pXYZ[0].z) * (float) pDib->shrink * invWidthFudge;
            pST[3].t = ((float) top - pXYZ[3].z) * (float) pDib->shrink * invWidthFudge;
         }

         pST[0].t -= ((float) (yOffset * pDib->shrink) * invWidth);
         pST[3].t -= ((float) (yOffset * pDib->shrink) * invWidth);

         pST[1].t = pST[0].t + ((pXYZ[0].z - pXYZ[1].z) * (float) pDib->shrink * invWidthFudge);
         pST[2].t = pST[3].t + ((pXYZ[3].z - pXYZ[2].z) * (float) pDib->shrink * invWidthFudge);
      }
      ...
      if (pSideDef->animate != NULL && pSideDef->animate->animation == ANIMATE_SCROLL)
      {
         ...
         if (pSideDef->flags & WF_BACKWARDS)
         {
            for (i = 0; i < 4; i++)
            {
               pST[i].s -= pSideDef->animate->u.scroll.xoffset * pDib->shrink * invHeight;
               pST[i].t += pSideDef->animate->u.scroll.yoffset * pDib->shrink * invWidth;
            }
         }
         else
         {
      ...
      for (count = 0; count < pNode->u.leaf.poly.npts; count++)
      {
         if (pSector->sloped_floor)
         {
            ...
            pXYZ[count].z = (-pSector->sloped_floor->plane.a * pXYZ[count].x -
                             pSector->sloped_floor->plane.b * pXYZ[count].y - pSector->sloped_floor->plane.d) *
                            oneOverC;
         }
         else
         {
         ...
         if (shadeAmount != 0)
         {
                ...
                (pNode->u.leaf.sector->sloped_floor->flags & SLF_DIRECTIONAL))
            {
               ...
               lightscale = (long) (pNode->u.leaf.sector->sloped_floor->plane.a * sunVect.x +
                                    ...
                                    pNode->u.leaf.sector->sloped_floor->plane.c * sunVect.z) >>
                            LOG_FINENESS;
      ...
      for (count = 0; count < pNode->u.leaf.poly.npts; count++)
      {
         if (pSector->sloped_ceiling)
         {
            ...
            pXYZ[count].z = (-pSector->sloped_ceiling->plane.a * pXYZ[count].x -
                             pSector->sloped_ceiling->plane.b * pXYZ[count].y - pSector->sloped_ceiling->plane.d) *
                            oneOverC;
         }
         else
         {
