...
inline bool isTiny(float f) { return f < epsilon && f > -epsilon; }
...
static Bool AddObject(BSPnode *tree, ObjectData *object)
{
   ...
   while (1)
   {
      ...
      case BSPinternaltype:
         ...
         side1 = tree->u.internal.separator.a * object->x1 + tree->u.internal.separator.b * object->y1 +
                 tree->u.internal.separator.c;
...
long GetFloorHeight(long x, long y, Sector *sector)
{
   ...
   if (sector->sloped_floor == (SlopeData *) NULL)
      ...
      return roundf(
          (-sector->sloped_floor->plane.a * x - sector->sloped_floor->plane.b * y - sector->sloped_floor->plane.d) /
          sector->sloped_floor->plane.c);
}
...
long GetCeilingHeight(long x, long y, Sector *sector)
{
   ...
   if (sector->sloped_ceiling == (SlopeData *) NULL)
      ...
      return roundf((-sector->sloped_ceiling->plane.a * x - sector->sloped_ceiling->plane.b * y -
                     sector->sloped_ceiling->plane.d) /
                    sector->sloped_ceiling->plane.c);
}
...
static void merge_cone(ConeTreeNode *node)
{
   ...
   {
      if (prev->cone.top_a == node->cone.top_a && prev->cone.top_b == node->cone.top_b &&
          prev->cone.top_d == node->cone.top_d && prev->cone.bot_a == node->cone.bot_a &&
          prev->cone.bot_b == node->cone.bot_b && prev->cone.bot_d == node->cone.bot_d)
      {
...
static Bool add_up(DrawItem *item_template, long a, long b, long d, long col0, long col1)
{
   ...
   for (c = search_for_first(col0); c->cone.leftedge <= col1; c = next)
   {
      ...
      if (additem)
      {
         if (item && item->cone.rightedge == c0 - 1 && item->cone.top_a == c->cone.top_a &&
             item->cone.top_b == c->cone.top_b && item->cone.top_d == c->cone.top_d && item->cone.bot_a == a &&
             item->cone.bot_b == b && item->cone.bot_d == d)
         {
...
static Bool add_dn(DrawItem *item_template, long a, long b, long d, long col0, long col1)
{
   ...
   for (c = search_for_first(col0); c->cone.leftedge <= col1; c = next)
   {
      ...
      if (additem)
      {
         if (item && item->cone.rightedge == c0 - 1 && item->cone.bot_a == c->cone.bot_a &&
             item->cone.bot_b == c->cone.bot_b && item->cone.bot_d == c->cone.bot_d && item->cone.top_a == a &&
             item->cone.top_b == b && item->cone.top_d == d)
         {
...
static void WalkObjects(ObjectData *objects)
{
   ...
   // sort, closest first
   std::sort(sort, sort + num, [](ObjectData *a, ObjectData *b) { return a->draw.distance < b->draw.distance; });
...
static void WalkLeaf(BSPleaf *leaf)
{
   ...
   if (shade_amount != 0)
   {
      ...
      if ((sloped_floor != NULL) && process_floor && (sloped_floor->flags & SLF_DIRECTIONAL))
      {
         ...
         lightscale =
             (long) (surface_norm.x * sun_vect.x + surface_norm.y * sun_vect.y + surface_norm.z * sun_vect.z) >>
             LOG_FINENESS;
   ...
   for (i = 0; i < leaf->poly.npts; i++)
   {
      if (!world_to_screen(x0 = leaf->poly.p[i].x, y0 = leaf->poly.p[i].y, x1 = leaf->poly.p[i + 1].x,
                           y1 = leaf->poly.p[i + 1].y, &t0, &t1, &col0, &d0, &col1, &d1))
         continue;
...
static void outlinecone(ViewCone *c)
{
   ...
   if (minrow < 0)
      debug(("minrow right (%d %d): %d\n", c->leftedge, c->rightedge, minrow));

   if (minrow < 0)
      debug(("minrow right %d (%d %d), %d (%d %d)\n", c->leftedge, DIVUP(c->top_b * c->leftedge + c->top_d, c->top_a),
...
void doDrawWall(DrawWallStruct *wall, ViewCone *c)
{
   ...
   if (wall->side > 0)
   {
      ...
      if (BSPwall->pos_sector == NULL)
         ...
         light = BSPwall->pos_sector->light;
   }
   else
   {
      ...
      if (BSPwall->neg_sector == NULL)
         ...
         light = BSPwall->neg_sector->light;
   }
   ...
   case WALL_ABOVE:
      ...
      if (!grid)
      {
         debug(("Unable to draw texture %d (overflown cache?)\n", sidedef->above_type));
         return;
      }
   ...
   */
   ...
   for (col = c->leftedge; col <= c->rightedge; col++)
   {
      ...
      if (backwards)
         textpos = (long) ((((FRACTION - 1 - f) * length) / (FINENESS * 4)) + xGraphicOffset) % (bitmap_height);
      else
         textpos = (long) (((f * length) / (FINENESS * 4)) + xGraphicOffset) % (bitmap_height);
      ...
      while (total_steps > 0)
      {
         ...
         else
         {
            while (screen_ptr > end_screen_ptr)
            {
               *screen_ptr = palette[*(square_base_ptr - (ytex >> FIX_DECIMAL))];
 ...
 *
 */
void biLerp(FixedPoint u_1, FixedPoint u_2, FixedPoint v_1, FixedPoint v_2, long count, long bitmap_width, BYTE *bits,
            BYTE *screen_ptr, BYTE *palette)
{
   FixedPoint u;
...
static void doDrawSloped(ViewCone *c, BSPleaf *leaf, BYTE floor)
{
   ...
   for (row = minrow; row <= maxrow; row++)
   {
      ...
      while (len > 0)
      {
         ...
         {
            biLerp(u_1 * bitmap_width + tu, u_2 * bitmap_width + tu, v_1 * bitmap_width + tv, v_2 * bitmap_width + tv,
                   count, bitmap_width, bits, screen_ptr, palette);
...
#define TEXTURE_ACC (FIX_DECIMAL + 4)
      tx_inc = -((center_b * d) >>
                 (LOG_FINENESS + (FIX_DECIMAL - 6) + LOG_VIEWER_DISTANCE - TEXTURE_ACC));  // == LOG_VIEWER_DISTANCE
      ty_inc = +((center_a * d) >> (LOG_FINENESS + (FIX_DECIMAL - 6) + LOG_VIEWER_DISTANCE - TEXTURE_ACC));

      tx = ((viewer_x - xoffset) << (TEXTURE_ACC - LOG_FINENESS)) +
           ((center_a * d) << (TEXTURE_ACC - LOG_FINENESS - (FIX_DECIMAL - 6))) + ((mincol - screen_width2) * tx_inc);
      ty = ((viewer_y - yoffset) << (TEXTURE_ACC - LOG_FINENESS)) +
           ((center_b * d) << (TEXTURE_ACC - LOG_FINENESS - (FIX_DECIMAL - 6))) + ((mincol - screen_width2) * ty_inc);
      ...
      if (tex_width == BITMAP_WIDTH * 2)
         while (screen_ptr <= end_screen_ptr)
         {
            *screen_ptr = palette[*(bits + (((ty & ~((1L << (TEXTURE_ACC - (LOG_BITMAP_WIDTH + 1))) - 1)) >>
                                             (TEXTURE_ACC - 2 * (LOG_BITMAP_WIDTH + 1))) +
      ...
      else
      {
         while (screen_ptr <= end_screen_ptr)
         {
            *screen_ptr = palette[*(bits + (((ty & ~((1L << (TEXTURE_ACC - LOG_BITMAP_WIDTH)) - 1)) >>
                                             (TEXTURE_ACC - 2 * LOG_BITMAP_WIDTH)) +
...
static void SetMappingValues(SlopeData *slope)
{
   ...
   v0.x = (VIEWER_DISTANCE * (((slope->p0.x - viewer_x) * -center_b + (slope->p0.y - viewer_y) * center_a))) /
          FINENESS / BASE_DIF_FACTOR;
   ...
   v1.x = (VIEWER_DISTANCE * (((slope->p1.x - viewer_x) * -center_b + (slope->p1.y - viewer_y) * center_a))) /
          FINENESS / BASE_DIF_FACTOR;
   ...
   v2.x = (VIEWER_DISTANCE * (((slope->p2.x - viewer_x) * -center_b + (slope->p2.y - viewer_y) * center_a))) /
          FINENESS / BASE_DIF_FACTOR;
