...
Modified from the LambdaMOO distribution for use in Meridian 59
by Ed Halley, 20 January, 1997.

Emacs-specific code and anything else smelling of GNU implementations
has been cleansed to escape from the dreaded anti-profit copyleft.

Added a utility function which performs a global search and replace.
...
#define EDH 1 // some extensions similar to gnu
...
#ifdef EDH
  Cwordleft,		/* match if follows or is mid-word  */
  Cwordright,		/* match if precedes or is mid-word */
  Cnotwordleft,		/* match if doesn't follow or is mid-word  */
  Cnotwordright,	/* match if doesn't precede or is mid-word */
#endif // EDH
...
#ifdef EDH
  Rwordleft,		/* word on the left */
  Rwordright,	  	/* word on the right */
  Rnotwordleft,		/* not word on the left */
  Rnotwordright,	/* not word on the right */
#endif // EDH
  Rnum_ops
};
...
#ifdef EDH
      regexp_quoted_ops['l'] = Rwordleft;
      regexp_quoted_ops['L'] = Rnotwordleft;
      regexp_quoted_ops['r'] = Rwordright;
      regexp_quoted_ops['R'] = Rnotwordright;
#endif // EDH
    }
...
const char *re_compile_pattern(char *regex,int size,regexp_t bufp)
{
...
#define ALLOC(amount)				\
  MACRO_BEGIN					\
    if (pattern_offset+(amount) > alloc)	\
      {						\
	alloc += 256 + (amount);		\
	pattern = (char *) SafeRealloc(pattern, alloc);  \
	if (!pattern)				\
	  goto out_of_memory;			\
      }						\
  MACRO_END
...
static void re_compile_fastmap_aux(
   char *code, int pos, char *visited, char *can_be_null, char *fastmap)
{
  int a, b, syntaxcode;
...
static int re_do_compile_fastmap(
   char *buffer, int used, int pos, char *can_be_null, char *fastmap)
{
...
int re_match(regexp_t bufp,char *string,int size, int pos,regexp_registers_t regs)
{
  ...
  if (!re_comp_buf.buffer)
    {
      ...
      re_comp_buf.fastmap = SafeMalloc(256);
...
static int re_expandalloc(char **buffer, int *allocated, int atleast, int chunk)
{
	if (atleast >= *allocated)
	{
		char *tmp;
		*allocated = atleast + chunk;
		tmp = (char *) realloc(*buffer, *allocated);
		if (!tmp)
		{
			*buffer = NULL;
			*allocated = 0;
   ...
   int (*verifycallback)(char*,int,int))
{
	...
	{
		// any more matches?
		found = re_search(bufp, string, size, prevend, range, regs);
		if (found < 0)
			break;

		// append stuff from string, between prevend and found, to result
		if (!re_expandalloc(&result, &resultalloc, build + found - prevend, withlen))
			return NULL;
		for (i = prevend; i < found; )
			result[build++] = string[i++];

		// replace this match?
		if (!verifycallback || verifycallback(string, regs->start[0], regs->end[0]))
		{
			// go through the 'with' string and build it onto result
			parse = with;
		}
		else
		{
			// go with the found text which was rejected by verifycallback
			parse = "\\0";
		}

		while (*parse)
		{
			if (!re_expandalloc(&result, &resultalloc, build + 1, withlen))
				return NULL;
			if (*parse == '\\')
			{
				int r = 0;
				...
				case '5':	case '6':	case '7':	case '8':	case '9':
					...
					if (r < RE_NREGS && regs->start[r] >= 0)
					{
						if (!re_expandalloc(&result, &resultalloc, build + regs->end[r] - regs->start[r], withlen))
							return NULL;
						for (i = regs->start[r]; i < regs->end[r]; )
							result[build++] = string[i++];
					}
					break;
				case 'a':	result[build++] = '\a';	parse++;	break;
				case 'b':	result[build++] = '\b';	parse++;	break;
				case 'f':	result[build++] = '\f';	parse++;	break;
				case 'n':	result[build++] = '\n';	parse++;	break;
				case 'r':	result[build++] = '\r';	parse++;	break;
				case 't':	result[build++] = '\t';	parse++;	break;
				case 'v':	result[build++] = '\v';	parse++;	break;
				default:
					result[build++] = *parse++;
				}
			}
		...
		range -= (regs->end[0] - prevend);
		prevend = regs->end[0];

	} while (found >= 0);

	// append stuff from string, after prevend, to result
	if (!re_expandalloc(&result, &resultalloc, build + size - prevend, withlen))
		return NULL;
	for (i = prevend; i < size; )
		result[build++] = string[i++];
