...
void SidedefDoAnimation(room_type *room, Sidedef *s, int return_code)
{
   ...
   case ANIMATE_ROOM_BITMAP:
      ...
      if (s->above_type != 0)
      {
         ...
         s->above_bmap = BitmapsGetPdib(above_grid->bmaps, a->group % BitmapsNumGroups(above_grid->bmaps), 0);
      }
      if (s->below_type != 0)
      {
         ...
         s->below_bmap = BitmapsGetPdib(below_grid->bmaps, a->group % BitmapsNumGroups(below_grid->bmaps), 0);
      }
      if (s->normal_type != 0)
      {
         ...
         s->normal_bmap = BitmapsGetPdib(normal_grid->bmaps, a->group % BitmapsNumGroups(normal_grid->bmaps), 0);
      }
...
void SectorDoAnimation(room_type *room, Sector *s, int return_code)
{
   ...
   case ANIMATE_ROOM_BITMAP:
      ...
      if (s->floor_type != 0)
      {
         ...
         s->floor = BitmapsGetPdib(floor_grid->bmaps, a->group % BitmapsNumGroups(floor_grid->bmaps), 0);
      }
      if (s->ceiling_type != 0)
      {
         ...
         s->ceiling = BitmapsGetPdib(ceiling_grid->bmaps, a->group % BitmapsNumGroups(ceiling_grid->bmaps), 0);
      }
      break;
...
void MoveSector(BYTE type, WORD sector_num, WORD height, BYTE speed)
{
   ...
   for (i = 0; i < current_room.num_sectors; i++)
   {
      ...
      lift->increment = (((float) HeightKodToClient(speed)) / 1000.0) / abs(lift->dest_z - lift->source_z);
   }
}
/************************************************************************/
/*
