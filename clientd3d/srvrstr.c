...
/* Dark theme variants: brighter/softer colors readable on dark backgrounds. */
...
void DisplayMessage(const char *message, COLORREF start_color, BYTE start_style)
{
   ...
   for (ptr = new_message; *ptr != 0; ptr++)
   {
	 ...
	 break;
      ...
      new_type = 0;
      FormatCode *active_table;
      switch (config.theme)
      {
      case THEME_DARK:  active_table = code_table_dark;  break;
      default:          active_table = code_table;       break;
      }
