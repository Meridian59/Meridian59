...
typedef struct {
   ...
   const void   *data;        // Data associated with action
   int     action2;     // Action to perform when button untoggled (A_NOACTION if not a toggle)
   const void   *data2;       // Data associated with untoggle action
...
typedef struct {
   ...
   const void   *action_data;  // Data for action
   int     action2;      // Action to perform when button untoggled (A_NOACTION if not a toggle)
   const void   *action_data2; // Data associated with untoggle action
