perl dialogconvert.pl brax.tab
perl dialogconvert.pl kalior_murder.tab
perl dialogconvert.pl konima.tab
perl dialogconvert.pl necromancers.tab
perl dialogconvert.pl nodeattack_stage1.tab
perl dialogconvert.pl nodeattack_stage2.tab
perl dialogconvert.pl nodeattack_councilor.tab
perl dialogconvert.pl npc_new.tab
perl dialogconvert.pl npc_old.tab

