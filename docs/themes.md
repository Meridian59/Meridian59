...
3. The per-theme menu bar color in `ThemeMenuBarColor` in `clientd3d/color.c` (optional, two lines).  Skip to keep the system default.
4. The per-theme title bar style in `ThemeUsesDarkTitleBar` in `clientd3d/color.c` (optional, two lines).  Return `true` for light text on a dark title bar, `false` for dark text on a light title bar.  Match this to the menu bar color for the most consistent look.
