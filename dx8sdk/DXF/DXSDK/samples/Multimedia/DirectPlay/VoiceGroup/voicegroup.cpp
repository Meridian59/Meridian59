...
// File: VoiceGroup.cpp
//
// Desc: The main file for VoiceGroup that shows how use DirectPlay along 
//       with DirectPlayVoice to allow talking in to a specific group
//       of players.
//
...
// every instance of that game.  // {9F2DA206-D197-4871-AD1D-D12F59058B5B}
GUID g_guidApp = { 0x9f2da206, 0xd197, 0x4871, { 0xad, 0x1d, 0xd1, 0x2f, 0x59, 0x5, 0x8b, 0x5b } };

struct PLAYER_STATE
{
    DWORD        dwGroup;                          // Group # that player joined in
    DWORD        dwTarget;                         // Group # that player is targeting 
};
...
//-----------------------------------------------------------------------------
#define GAME_MSGID_PLAYERSTATECHANGE   1
...
struct GAMEMSG_PLAYERSTATECHANGE : public GAMEMSG_GENERIC
{
    PLAYER_STATE ps;
};
...
TCHAR              g_strAppName[256]             = TEXT("VoiceGroup");
...
DPNID              g_LocalGroupDPNID             = 0;       // DPNID of the group of the local player
...
HRESULT          SendLocalStateToAll( HWND hDlg );
HRESULT          SetGroupAndTarget();
    ...
    switch( msg ) 
    {
        case WM_INITDIALOG:
            {
                ...
                {
                    MessageBox( hDlg, TEXT("You are running in half duplex mode. In half duplex mode no recording takes place."), 
        ...
        case WM_COMMAND:
            switch( LOWORD(wParam) )
            {
                ...
                case IDC_CYCLEGROUP:
                    {
                        if( g_pPlayerLocal )
                        {
                            g_pPlayerLocal->ps.dwGroup++;
                            g_pPlayerLocal->ps.dwGroup %= MAX_GROUPS + 1;
                        }

                        if( FAILED( g_hrDialog = SetGroupAndTarget() ) )
                        {
                            DXTRACE_ERR( TEXT("SetGroupAndTarget"), g_hrDialog );
                            EndDialog( hDlg, 0 );
                            break;
                        }

                        if( FAILED( g_hrDialog = SendLocalStateToAll( hDlg ) ) )
                        {
                            DXTRACE_ERR( TEXT("SendLocalStateToAll"), g_hrDialog );
                            EndDialog( hDlg, 0 );
                            break;
                        }
                ...
                case IDC_CYCLETARGET:
                    {
                        if( g_pPlayerLocal )
                        {
                            g_pPlayerLocal->ps.dwTarget++;
                            g_pPlayerLocal->ps.dwTarget %= MAX_GROUPS + 1;
                        }
...
HRESULT OnInitDialog( HWND hDlg )
{
    ...
    if( g_bHostPlayer )
        SetWindowText( hDlg, TEXT("VoiceGroup (Session Host)") );
    else
        SetWindowText( hDlg, TEXT("VoiceGroup (Session Client)") );

    SetDlgItemText( hDlg, IDC_GROUP_TEXT, TEXT("Unassigned") );
    SetDlgItemText( hDlg, IDC_TARGET_TEXT, TEXT("Everyone") );
    ...
    column.cx         = dwListViewWidth * 8 / 21;
    ...
    column.cx         = dwListViewWidth * 4 / 21;
    column.pszText    = strHeader;
    column.cchTextMax = _tcslen( strHeader );
    ListView_InsertColumn( hListView, 1, &column );

    // Insert the Group column
    _tcscpy( strHeader, TEXT("Group") );
    column.cx         = dwListViewWidth * 5 / 21;
    column.pszText    = strHeader;
    column.cchTextMax = _tcslen( strHeader );
    ListView_InsertColumn( hListView, 2, &column );

    // Insert the Target column
    _tcscpy( strHeader, TEXT("Target") );
    column.cx         = dwListViewWidth * 4 / 21;
    column.pszText    = strHeader;
    column.cchTextMax = _tcslen( strHeader );
    ListView_InsertColumn( hListView, 3, &column );

    if( FAILED( hr = SetGroupAndTarget() ) )
        return DXTRACE_ERR( TEXT("SetGroupAndTarget"), hr );
    if( FAILED( hr = SendLocalStateToAll( hDlg ) ) )
        return DXTRACE_ERR( TEXT("SendLocalStateToAll"), hr );
    ...
    if( g_bHostPlayer )
    {
        ...
        for( dwIndex = 1; dwIndex < MAX_GROUPS + 1; dwIndex++ )
        {
            ...
            *pdwData = dwIndex;
            ...
            DPNHANDLE hAsync;
            hr = g_pDP->CreateGroup( &dpGroupInfo, NULL,
                                     NULL, &hAsync, 0 );
            if( FAILED( hr ) ) 
...
HRESULT DisplayPlayersInChat( HWND hDlg )
{
    ...
    for( DWORD i = 0; i<dwNumPlayers; i++ )
    {
        ...
        do
        {
            ...
            hr = g_pDP->GetPlayerContext( g_pPlayers[i], 
                                          (LPVOID* const) &pPlayerInfo,
									      0);
        } 
        ...
        if( pPlayerInfo->bHalfDuplex )
        {
            _tcscpy( strText, TEXT("Can't talk") );
        }
        else
        {
            if( pPlayerInfo->bTalking )
                _tcscpy( strText, TEXT("Talking") );
            else
                _tcscpy( strText, TEXT("Silent") );
        }
        ...
        lvItem.cchTextMax = _tcslen( strText );
        ListView_SetItem( hListView, &lvItem );

        if( pPlayerInfo->ps.dwGroup == 0 )
            _tcscpy( strText, TEXT("Unassigned") );
        else
            wsprintf( strText, TEXT("%d"), pPlayerInfo->ps.dwGroup );
        ...
        if( pPlayerInfo->ps.dwTarget == 0 )
            _tcscpy( strText, TEXT("Everyone") );
        else
            wsprintf( strText, TEXT("%d"), pPlayerInfo->ps.dwTarget );
    ...
    if( g_pPlayerLocal )
    {
        if( g_pPlayerLocal->ps.dwGroup == 0 )
            ...
            wsprintf( strText, TEXT("Group #%d"), g_pPlayerLocal->ps.dwGroup );
        SetDlgItemText( hDlg, IDC_GROUP_TEXT, strText );

        if( g_pPlayerLocal->ps.dwTarget == 0 )
            _tcscpy( strText, TEXT("Everyone") );
        else
            wsprintf( strText, TEXT("Group #%d"), g_pPlayerLocal->ps.dwTarget );
        SetDlgItemText( hDlg, IDC_TARGET_TEXT, strText );
    }
...
//-----------------------------------------------------------------------------
// Name: SendLocalStateToAll()
// Desc: Send the local player's state to all of the players
//-----------------------------------------------------------------------------
HRESULT SendLocalStateToAll( HWND hDlg )
{
    if( g_pPlayerLocal )
    {
        GAMEMSG_PLAYERSTATECHANGE msgPlayerStateChange;
        msgPlayerStateChange.dwType = GAME_MSGID_PLAYERSTATECHANGE;
        msgPlayerStateChange.ps     = g_pPlayerLocal->ps; 

        DPN_BUFFER_DESC bufferDesc;
        bufferDesc.dwBufferSize = sizeof(GAMEMSG_PLAYERSTATECHANGE);
...
HRESULT SetGroupAndTarget()
{
    ...
    if( pVoiceClient && g_pPlayerLocal->ps.dwTarget == 0 )
    {
        // Talk to everyone in the session 
        DVID dvid = DVID_ALLPLAYERS;
        if( FAILED( hr = pVoiceClient->SetTransmitTargets( &dvid, 1, 0 ) ) )
            return DXTRACE_ERR( TEXT("SetTransmitTargets"), hr );
    }

    if( g_LocalGroupDPNID != 0 )
    {
        // Remove player from current group
        DPNHANDLE hAsync;
        hr = g_pDP->RemovePlayerFromGroup( g_LocalGroupDPNID, 
                                           g_LocalPlayerDPNID,
                                           NULL, &hAsync, 0 );
        if( FAILED(hr) )
            return DXTRACE_ERR( TEXT("RemovePlayerFromGroup"), hr );
    ...
    do
    {
        ...
        hr = g_pDP->EnumPlayersAndGroups( aGroupsDPNID, &dwCount, DPNENUM_GROUPS );
    }
    ...
    for( DWORD i=0; i<dwCount; i++ )
    {
        ...
        hr = g_pDP->GetGroupInfo( aGroupsDPNID[i], pdpGroupInfo, &dwSize, 0 );
        ...
        if( dwGroupNumber == g_pPlayerLocal->ps.dwGroup )
        {
            g_LocalGroupDPNID = aGroupsDPNID[i];

            // Add player to this group
            DPNHANDLE hAsync;
            hr = g_pDP->AddPlayerToGroup( aGroupsDPNID[i], g_LocalPlayerDPNID, 
                                                      NULL, &hAsync, 0 );
            if( FAILED( hr ) ) 
                return DXTRACE_ERR( TEXT("AddPlayerToGroup"), hr );
        }

        if( dwGroupNumber == g_pPlayerLocal->ps.dwTarget )
        {
            // Talk to just this group
            hr = pVoiceClient->SetTransmitTargets( &aGroupsDPNID[i], 1, 0 );
            if( FAILED( hr ) ) 
                return DXTRACE_ERR( TEXT("SetTransmitTargets"), hr );
        }
    }
                                         ...
                                         PVOID pMsgBuffer )
{
    ...
    switch( dwMessageId )
    {
        case DPN_MSGID_CREATE_PLAYER:
        {
            ...
            if( hr == DPNERR_BUFFERTOOSMALL )
            {
                ...
                if( SUCCEEDED(hr) )
                {
                    ...
                    if( (pdpPlayerInfo->dwPlayerFlags & DPNPLAYER_LOCAL) != 0 )
                    {
                        ...
                        g_pPlayerLocal->ps.dwGroup  = 0;
                        g_pPlayerLocal->ps.dwTarget = 0;
            ...
            if( FAILED( hr = SendLocalStateToAll( g_hDlg ) ) )
                return DXTRACE_ERR( TEXT("SendLocalStateToAll"), hr );
        ...
        case DPN_MSGID_RECEIVE:
        {
            ...
            if( pMsg->dwType == GAME_MSGID_PLAYERSTATECHANGE )
            {
                GAMEMSG_PLAYERSTATECHANGE* pPlayerStateChangedMsg;
                pPlayerStateChangedMsg = (GAMEMSG_PLAYERSTATECHANGE*) pMsg;
        ...
        case DVMSGID_CREATEVOICEPLAYER:
            {
                ...
                PLAYER_ADDREF( pPlayerInfo ); // addref player, for voice context value
                PLAYER_UNLOCK(); // leave player context CS
