...
{
    const WCHAR g_szBookmarkFilename[] = L"c:\\DVDSample.bmk";
    const WCHAR g_szBookmarkName[] = L"DVDSampleBookmarks";
    HHOOK g_hMouse = 0;
...
bool CDvdCore::BuildGraph()
{
    ...
    {
#ifdef UNICODE
        lstrcpy(szwDiscPath, m_szDiscPath) ;
#else
        MultiByteToWideChar(CP_ACP, 0, m_szDiscPath, -1, szwDiscPath, MAX_PATH) ;
#endif // UNICODE
	...
	if (S_FALSE == hr) // partial success
	{
        ...
        if (0 == GetStatusText(&buildStatus, szStatusText, sizeof(szStatusText)))
        {
            lstrcpy(szStatusText, TEXT("An unknown error has occurred")) ;
        }
        lstrcat(szStatusText, TEXT("\n\nDo you still want to continue?")) ;
...
DWORD CDvdCore::GetStatusText(AM_DVD_RENDERSTATUS *pStatus, PTSTR pszStatusText, DWORD dwMaxText)
{
    ...
    ZeroMemory(szBuffer, sizeof(TCHAR) * 1000) ;
    ...
    {
        iChars = wsprintf(pszBuff, TEXT("* The video decoder doesn't produce closed caption data\n")) ;
        pszBuff += iChars ;
    }
...
HRESULT CDvdCore::Play()
{
    ...
    switch (m_eState)
    {
        ...
        case Graph_Stopped2:
            hr = m_pIMC->Run();
            m_pIDvdC2->PlayForwards(1.0, 0, NULL); // this call will likely fail due to timing issues.  Ignore it.
            if (SUCCEEDED(hr))
                SetState(Playing);
            return hr;
        ...
        case Nav_Stopped:
        case Graph_Stopped1:
            //hr = m_pIDvdC2->PlayTitle(1, DVD_CMD_FLAG_None, NULL); // play title 1
            hr = m_pIMC->Run();
            if (SUCCEEDED(hr))
                SetState(Playing);
            return hr; 
...
bool CDvdCore::Stop()
{
    ...
    case Nav_Paused:
    case Graph_Paused:
        //hr = m_pIDvdC2->Stop(); // put us in stop domain
        hr = m_pIMC->Pause();
...
bool CDvdCore::Pause(void)
{
    ...
    {
        if (FAILED(m_pIDvdC2->Pause(false)))
            return false;
        else
        {
            SetState(Playing);
    ...
    if (FAILED(m_pIDvdC2->Pause(TRUE)))
        return false;
    else
    {
        SetState(Nav_Paused);
...
LRESULT CDvdCore::OnDvdEvent(UINT uMessage, WPARAM wParam, LPARAM lParam)
{
	...
	while (SUCCEEDED(m_pIME->GetEvent(&lEvent, (LONG_PTR *) &lParam1, 
                    (LONG_PTR *) &lParam2, lTimeOut)))
	{
        ...
        case EC_DVD_CHAPTER_START:
            m_ulCurChapter = lParam1;
            break;

        case EC_DVD_TITLE_CHANGE:
            m_ulCurTitle = lParam1;
            break;
        ...
        case EC_DVD_DOMAIN_CHANGE:
            ...
            case DVD_DOMAIN_VideoTitleSetMenu: // = 3
                ...
                DbgLog((LOG_TRACE, 3, TEXT("DVD Event: Domain changed to VMGM(2)/VTSM(3) (%ld)"), lParam1)) ;
            ...
            case DVD_DOMAIN_Title:      // = 4
                ...
                DbgLog((LOG_TRACE, 3, TEXT("DVD Event: Title Domain (%ld)"), lParam1)) ;
        ...
        case EC_DVD_PARENTAL_LEVEL_CHANGE:
            if (IDYES == MessageBox(m_hWnd, TEXT("Accept Parental Level Change Request?"), 
                TEXT("Accept Change?"), MB_YESNO))
            {
        ...
        case EC_DVD_ERROR:
            DbgLog((LOG_TRACE, 3, TEXT("DVD Event: Error event received (code %ld)"), lParam1)) ;
            switch (lParam1)
            {
        ...
        case EC_DVD_WARNING:
            ...
            default:
                DbgLog((LOG_ERROR, 1, TEXT("DVD Warning: An unknown (%ld) warning received."), lParam1)) ;
                break ;
            }
            break ;
...
{
    DbgLog((LOG_TRACE, 5, TEXT("CDvdCore::ShowMouseCursor(%s)"), bShow ? TEXT("true") : 
        TEXT("false"))) ;
...
#ifdef UNICODE
    lstrcpy(szwFileName, szDirectory) ;
#else
    MultiByteToWideChar(CP_ACP, 0, szDirectory, -1, szwFileName, MAX_PATH) ;
#endif // UNICODE            
...
bool CDvdCore::ChangeDvdRegion()
{
	...
	{
        DbgLog((LOG_ERROR, 1, TEXT("No DVD drive was found with DVD-Video disc.")
	    TEXT("Cannot change DVD region of the drive."))); 
	    return false;
	}
	...
	{
		// Get path of \windows\dvdrgn.exe for command line
		// string
		...
		_tcsncat(szCmdLine, szDVDDrive, 1);
...
bool CDvdCore::GetDriveLetter(TCHAR *pszDrive)
{
    ...
    if (SUCCEEDED(m_pIDvdI2->GetDVDDirectory(szwPath, MAX_PATH, &ulActualSize)))
    {
        ...
        WideCharToMultiByte(CP_ACP, NULL, szwPath, -1, szDrive, MAX_PATH, NULL, NULL);
...
bool CDvdCore::GetDvdText()
{
	...
	hr = m_pIDvdI2->GetDVDTextLanguageInfo(ulLangIndex,
		&ulNumStrings, &LangCode, &CharSet); 
	...
	for (ULONG ulStringIndex = 0; ulStringIndex < ulNumStrings; ulStringIndex++)
	{
		...
		hr = m_pIDvdI2->GetDVDTextStringAsUnicode(ulLangIndex,
			ulStringIndex, buffer, sizeof(buffer)/sizeof(*buffer), &ulActualSize, &type);
