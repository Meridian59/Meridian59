...
/************************************************************************/
/*
 * DrawDarkScrollArrow:  Draw a filled triangle arrow centered in btn.
 *   down=true draws an arrow pointing down, false draws up.
 */
static void DrawDarkScrollArrow(HDC hdc, const RECT *btn, bool down, COLORREF col)
{
   int cx   = (btn->left + btn->right)  / 2;
   int cy   = (btn->top  + btn->bottom) / 2;
   int half = (btn->right - btn->left) / 5;
   if (half < 3) half = 3;
   POINT pts[3];
   if (down)
   {
      pts[0].x = cx;        pts[0].y = cy + (half + 1) / 2;
      pts[1].x = cx - half; pts[1].y = cy - half / 2;
      pts[2].x = cx + half; pts[2].y = cy - half / 2;
   }
   else
   {
      pts[0].x = cx;        pts[0].y = cy - (half + 1) / 2;
...
/************************************************************************/
/*
 * PaintDarkScrollbar:  Draw the dark scrollbar appearance onto hdc.
 */
static void PaintDarkScrollbar(HWND hwnd, HDC hdc)
{
   static const COLORREF COL_TRACK = RGB(30,  30,  30);
   static const COLORREF COL_THUMB = RGB(100, 100, 100);
   static const COLORREF COL_BTN   = RGB(45,  45,  48);
   static const COLORREF COL_ARROW = RGB(180, 180, 180);

   RECT rc;
   GetClientRect(hwnd, &rc);
   int w    = rc.right  - rc.left;
   int h    = rc.bottom - rc.top;
   int btnH = GetSystemMetrics(SM_CYVSCROLL);

   /* Track background */
   RECT track = {0, btnH, w, h - btnH};
   HBRUSH br  = CreateSolidBrush(COL_TRACK);
   ...
   int thumbH = trackH;
   int thumbY = btnH;
   if (si.nMax > si.nMin && trackH > 0)
   {
      int range = si.nMax - si.nMin + 1;
      thumbH = MulDiv(trackH, (int)si.nPage, range);
      int minThumb = GetSystemMetrics(SM_CYVSCROLL) / 2;
      if (thumbH < minThumb) thumbH = minThumb;
      if (thumbH > trackH)  thumbH = trackH;
      int scrollable = si.nMax - si.nMin - (int)si.nPage + 1;
      if (scrollable > 0)
         thumbY = btnH + MulDiv(si.nPos - si.nMin, trackH - thumbH, scrollable);
   }
   RECT thumb = {2, thumbY, w - 2, thumbY + thumbH};
   br = CreateSolidBrush(COL_THUMB);
   FillRect(hdc, &thumb, br);
   DeleteObject(br);

   /* Arrow buttons */
   RECT upBtn   = {0, 0,        w, btnH};
   RECT downBtn = {0, h - btnH, w, h};
   br = CreateSolidBrush(COL_BTN);
   FillRect(hdc, &upBtn,   br);
   FillRect(hdc, &downBtn, br);
   DeleteObject(br);
   DrawDarkScrollArrow(hdc, &upBtn,   false, COL_ARROW);
   DrawDarkScrollArrow(hdc, &downBtn, true,  COL_ARROW);
}
/************************************************************************/
/*
 * DarkScrollbarMaxPos:  Return the last valid scroll position for the
 *   current scrollbar range and page size.
 */
static int DarkScrollbarMaxPos(const SCROLLINFO *si)
{
  if (si->nPage > 0)
    return std::max(si->nMax - (int)si->nPage + 1, si->nMin);

  return std::max(si->nMax, si->nMin);
}
/************************************************************************/
/*
 * DarkScrollbarCanScroll:  Return true if the clicked scrollbar part can
 *   still move the position in that direction.
 */
static bool DarkScrollbarCanScroll(const SCROLLINFO *si, int hit)
{
  int minPos = si->nMin;
  int maxPos = DarkScrollbarMaxPos(si);

  switch (hit)
  {
  case 0:
  case 1:
    return si->nPos > minPos;
...
/************************************************************************/
/*
 * DarkScrollbarGetWheelSteps:  Convert accumulated wheel delta into whole
 *   scroll steps for a specific scrollbar control.
 */
static int DarkScrollbarGetWheelSteps(HWND hwnd, int zDelta)
{
   static const char PROP_WHEELDELTA[] = "DarkSB_WheelDelta";

   int wheel_delta = (int)(INT_PTR)GetPropA(hwnd, PROP_WHEELDELTA);
   wheel_delta += zDelta;

   int steps = wheel_delta / WHEEL_DELTA;
   wheel_delta -= steps * WHEEL_DELTA;

   SetPropA(hwnd, PROP_WHEELDELTA, (HANDLE)(INT_PTR)wheel_delta);
   return steps;
}
/************************************************************************/
/*
 * DarkSBHitTest:  Returns which part of a dark scrollbar was clicked.
 *   0 = up arrow, 1 = track above thumb, 2 = thumb, 3 = track below
 *   thumb, 4 = down arrow, -1 = none.
 */
static int DarkSBHitTest(HWND hwnd, int y)
{
   RECT rc;
   GetClientRect(hwnd, &rc);
   int h    = rc.bottom - rc.top;
   ...
   if (y < btnH)       return 0;   /* up arrow */
   if (y >= h - btnH)  return 4;   /* down arrow */
   ...
   if (y < thumbY)            return 1;   /* track above */
   if (y < thumbY + thumbH)   return 2;   /* thumb */
   return 3;                              /* track below */
}
/************************************************************************/
/*
 * DarkScrollbarProc:  Window procedure for dark theme scrollbars.
 *   Handles painting, mouse input, and auto-repeat scrolling.
 */
static LRESULT CALLBACK DarkScrollbarProc(HWND hwnd, UINT msg, WPARAM wp, LPARAM lp)
{
   WNDPROC orig = (WNDPROC)(LONG_PTR)GetWindowLongPtr(hwnd, GWLP_USERDATA);
   if (!IsDarkMode())
      return CallWindowProc(orig, hwnd, msg, wp, lp);

   /* Per-instance tracking stored via window properties */
   static const char PROP_HIT[]       = "DarkSB_Hit";
   static const char PROP_DRAGGING[]  = "DarkSB_Drag";
   static const char PROP_DRAGOFS[]   = "DarkSB_DragOfs";
   static const char PROP_WHEELDELTA[]= "DarkSB_WheelDelta";
   ...
   case SBM_SETPOS:
   {
      ...
      si.fMask = SIF_POS;
      si.nPos = (int)wp;
      SetScrollInfo(hwnd, SB_CTL, &si, (BOOL)lp);
   ...
   {
      LRESULT r = CallWindowProc(orig, hwnd, SBM_SETRANGE, wp, lp);
      InvalidateRect(hwnd, NULL, FALSE);
      return r;
   }
   ...
   {
      LRESULT r = CallWindowProc(orig, hwnd, msg, FALSE, lp);
      InvalidateRect(hwnd, NULL, FALSE);
      return r;
   }
   ...
   case WM_MOUSEWHEEL:
   {
      ...
      int steps = DarkScrollbarGetWheelSteps(hwnd, zDelta);
      ...
      for (int index = 0; index < abs(steps); ++index)
      {
         ...
         if ((code == SB_LINEUP && si.nPos <= si.nMin) ||
            (code == SB_LINEDOWN && si.nPos >= DarkScrollbarMaxPos(&si)))
            break;

         SendMessage(hParent, WM_VSCROLL, code, (LPARAM)hwnd);
      }
      return 0;
   }
   ...
   case WM_LBUTTONDOWN:
   {
      ...
      int hit = DarkSBHitTest(hwnd, y);
      SetPropA(hwnd, PROP_HIT, (HANDLE)(INT_PTR)(hit + 1));
      SetPropA(hwnd, PROP_DRAGGING, (HANDLE)0);

      HWND hParent = GetParent(hwnd);
      SCROLLINFO si = {sizeof(si), SIF_ALL};
      GetScrollInfo(hwnd, SB_CTL, &si);

      if (!DarkScrollbarCanScroll(&si, hit) && hit != 2)
      {
        InvalidateRect(hwnd, NULL, FALSE);
        return 0;
      }
      ...
      {
      case 0: /* up arrow */
         SendMessage(hParent, WM_VSCROLL, SB_LINEUP, (LPARAM)hwnd);
         SetTimer(hwnd, 1, GetDoubleClickTime(), NULL);
         break;
      case 4: /* down arrow */
         SendMessage(hParent, WM_VSCROLL, SB_LINEDOWN, (LPARAM)hwnd);
         SetTimer(hwnd, 1, GetDoubleClickTime(), NULL);
         break;
      case 1: /* track above thumb */
         SendMessage(hParent, WM_VSCROLL, SB_PAGEUP, (LPARAM)hwnd);
         SetTimer(hwnd, 2, GetDoubleClickTime(), NULL);
         break;
      case 3: /* track below thumb */
         SendMessage(hParent, WM_VSCROLL, SB_PAGEDOWN, (LPARAM)hwnd);
         SetTimer(hwnd, 2, GetDoubleClickTime(), NULL);
         break;
      case 2: /* thumb */
      {
         SetPropA(hwnd, PROP_DRAGGING, (HANDLE)1);
         /* Compute offset from top of thumb to click point */
         RECT rc;
         GetClientRect(hwnd, &rc);
         int h = rc.bottom - rc.top;
         int btnH = GetSystemMetrics(SM_CYVSCROLL);
         ...
         SetPropA(hwnd, PROP_DRAGOFS, (HANDLE)(INT_PTR)(y - thumbY));
         SendMessage(hParent, WM_VSCROLL, MAKEWPARAM(SB_THUMBTRACK, si.nPos), (LPARAM)hwnd);
         break;
      }
      }
      InvalidateRect(hwnd, NULL, FALSE);
   ...
   case WM_MOUSEMOVE:
   {
      if ((INT_PTR)GetPropA(hwnd, PROP_DRAGGING) != 1)
         return 0;

      int y = GET_Y_LPARAM(lp);
      int dragOfs = (int)(INT_PTR)GetPropA(hwnd, PROP_DRAGOFS);
      ...
      int scrollable = si.nMax - si.nMin - (int)si.nPage + 1;
      if (scrollable > 0 && (trackH - thumbH) > 0)
      {
         int thumbTop = y - dragOfs - btnH;
         int newPos = si.nMin + MulDiv(thumbTop, scrollable, trackH - thumbH);
         if (newPos < si.nMin) newPos = si.nMin;
         if (newPos > si.nMin + scrollable) newPos = si.nMin + scrollable;

         si.fMask = SIF_POS;
         si.nPos  = newPos;
         SetScrollInfo(hwnd, SB_CTL, &si, FALSE);
         SendMessage(GetParent(hwnd), WM_VSCROLL,
            MAKEWPARAM(SB_THUMBTRACK, newPos), (LPARAM)hwnd);
         InvalidateRect(hwnd, NULL, FALSE);
      }
      return 0;
   }
   ...
   case WM_LBUTTONUP:
   {
      ...
      if ((INT_PTR)GetPropA(hwnd, PROP_DRAGGING) == 1)
      {
         ...
         GetScrollInfo(hwnd, SB_CTL, &si);
         SendMessage(GetParent(hwnd), WM_VSCROLL,
            MAKEWPARAM(SB_THUMBPOSITION, si.nPos), (LPARAM)hwnd);
      }
      SendMessage(GetParent(hwnd), WM_VSCROLL, SB_ENDSCROLL, (LPARAM)hwnd);
      SetPropA(hwnd, PROP_HIT, (HANDLE)0);
      SetPropA(hwnd, PROP_DRAGGING, (HANDLE)0);
      ReleaseCapture();
      InvalidateRect(hwnd, NULL, FALSE);
      return 0;
   ...
   case WM_TIMER:
   {
      /* Repeat scroll while button is held */
      int hit = (int)(INT_PTR)GetPropA(hwnd, PROP_HIT) - 1;
      HWND hParent = GetParent(hwnd);

      SCROLLINFO si = {sizeof(si), SIF_RANGE | SIF_PAGE | SIF_POS};
      GetScrollInfo(hwnd, SB_CTL, &si);
      if (!DarkScrollbarCanScroll(&si, hit))
      {
         KillTimer(hwnd, 1);
         KillTimer(hwnd, 2);
         InvalidateRect(hwnd, NULL, FALSE);
      ...
      switch (hit)
      {
      case 0: SendMessage(hParent, WM_VSCROLL, SB_LINEUP,   (LPARAM)hwnd); break;
      case 4: SendMessage(hParent, WM_VSCROLL, SB_LINEDOWN, (LPARAM)hwnd); break;
      case 1: SendMessage(hParent, WM_VSCROLL, SB_PAGEUP,   (LPARAM)hwnd); break;
      case 3: SendMessage(hParent, WM_VSCROLL, SB_PAGEDOWN, (LPARAM)hwnd); break;
      }
      /* Shorten repeat interval after initial delay */
      if (wp == 1 || wp == 2)
      {
         KillTimer(hwnd, (UINT_PTR)wp);
         SetTimer(hwnd, (UINT_PTR)wp, 50, NULL);
      }
      InvalidateRect(hwnd, NULL, FALSE);
      return 0;
   }
   ...
   case WM_NCDESTROY:
   {
      RemovePropA(hwnd, PROP_HIT);
      RemovePropA(hwnd, PROP_DRAGGING);
      RemovePropA(hwnd, PROP_DRAGOFS);
      RemovePropA(hwnd, PROP_WHEELDELTA);
      return CallWindowProc(orig, hwnd, msg, wp, lp);
   }
...
/************************************************************************/
/*
 * DarkScrollbarSubclass:  Subclass a scrollbar control to use custom
 *   dark theme drawing.
 */
void DarkScrollbarSubclass(HWND hwndScroll)
{
   WNDPROC orig = SubclassWindow(hwndScroll, DarkScrollbarProc);
   SetWindowLongPtr(hwndScroll, GWLP_USERDATA, (LONG_PTR)orig);
}

/************************************************************************/
/*
 * DarkScrollbarSetInfo:  Configure a dark scrollbar's range, page, and
 *   position.  itemCount is the total scrollable items or rows, and
 *   pageSize is how many are visible at once.
 */
void DarkScrollbarSetInfo(HWND hwndScroll, int itemCount, int pageSize, int pos, bool redraw)
{
  SCROLLINFO si = { sizeof(si), SIF_RANGE | SIF_PAGE | SIF_POS };
  int clampedItemCount = std::max(itemCount, 0);
  int clampedPageSize = std::max(pageSize, 0);
  int maxPos = std::max(clampedItemCount - clampedPageSize, 0);

  si.nMin = 0;
  si.nMax = clampedItemCount > 0 ? clampedItemCount - 1 : 0;
  si.nPage = (UINT)clampedPageSize;
  si.nPos = std::min(std::max(pos, 0), maxPos);

  SetScrollInfo(hwndScroll, SB_CTL, &si, redraw ? TRUE : FALSE);
}
