...
#define DECLARE_RESPONSE_TABLE(cls)\
  ...
  public: bool Find(TEventInfo&, TEqualOperator = 0)

#define DECLARE_EXPORT_RESPONSE_TABLE(cls)\
  private: static ::owl::TResponseTableEntry __entries[];\
  private: typedef cls TMyClass;\
  public: bool __declspec(dllexport) Find(TEventInfo&, TEqualOperator = 0)

#define DECLARE_IMPORT_RESPONSE_TABLE(cls)\
  private: static ::owl::TResponseTableEntry __entries[];\
  private: typedef cls TMyClass;\
  public: bool __declspec(dllimport) Find(TEventInfo&, TEqualOperator = 0)
...
#define DEFINE_RESPONSE_TABLE(cls)\
  ...
  {\
    return SearchEntries(__entries, (eventInfo.Object = this, eventInfo), equal);\
  }\
...
#define OWL_ID_EV_(msgId, id, method)\
  {{msgId}, id, OWL_DISPATCH(::owl::TDispatch<msgId>::Decode, method)}
