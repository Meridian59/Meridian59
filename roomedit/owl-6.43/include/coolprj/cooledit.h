...
//{{TCoolTextBufferVIRTUAL_BEGIN}}
    ...
    virtual bool DeleteText(const TEditRange& range){ return false;}
    ...
    virtual bool Save(LPCTSTR filename, TCrLfStyle style = clStyleAutomatic,
                      bool clearDirty = true){ return false;}
    // simple search
...
#endif
    bool EvMouseWheel(owl::uint modKeys, int xDelta, const owl::TPoint& point);
...
//{{TCoolReplaceDlgXFER_DEF}}
  protected:
    owl::TComboBox*  ReplaceTextBox;
//{{TCoolReplaceDlgXFER_DEF_END}}
...
//{{TCoolSearchWndWndVIRTUAL_BEGIN}}
    virtual void SetupWindow();
    virtual void DoSearch();
    virtual bool Search(const TEditPos& startPos, LPCTSTR text,
                        TFindFlags flags = ffNone);
//{{TCoolSearchWndVIRTUAL_END}}
...
//{{TCoolEditFileRSP_TBL_END}}
//{{TCoolEditFileWndVIRTUAL_BEGIN}}
  public:
    virtual void FileNew();
    virtual bool FileOpen(LPCTSTR filename);
...
//
//
#define REGCOOLPARSER(parser) {"coolparser",{TRegItem::RegFlags((long)parser,__regHeap)}},

inline TParserCreator GetCoolParserCreator(owl::TDocTemplate* tpl)
{
  LPCTSTR str = tpl->GetRegList()["coolparser"];
  return (TParserCreator)_ttol(str?str:_T(""));
}
...
//{{TCoolSearchViewVIRTUAL_BEGIN}}
    bool     CanClose();
    LPCTSTR  GetViewName();
    owl::TWindow* GetWindow();
    bool     SetDocTitle(LPCTSTR docname, int index);
//{{TCoolSearchViewVIRTUAL_END}}

//{{TCoolSearchViewRSP_TBL_BEGIN}}
  protected:
    bool     VnIsWindow(HWND hWnd);
//{{TCoolSearchViewRSP_TBL_END}}
...
//{{TCoolEditViewVIRTUAL_BEGIN}}
    bool     CanClose();
    LPCTSTR  GetViewName();
    owl::TWindow* GetWindow();
    bool     SetDocTitle(LPCTSTR docname, int index);
//{{TCoolEditViewVIRTUAL_END}}

//{{TCoolEditViewRSP_TBL_BEGIN}}
  protected:
    bool     VnIsWindow(HWND hWnd);
//{{TCoolEditViewRSP_TBL_END}}
