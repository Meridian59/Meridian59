...
template <> struct TDocViewDispatch<WM_OWLNOTIFY>::TNotificationDispatch<vnInvalidate>
{
  ...
  {
    PRECONDITION(p2);
    return (p2 != 0 && (owl::ConvertVoidPtr<T>(i)->*M)(*reinterpret_cast<owl::TRect*>(p2))) ? TRUE : FALSE;
  }
};
...
template <> struct TDocViewDispatch<WM_OWLNOTIFY>::TNotificationDispatch<vnLinkView>
{
  ...
  {
    PRECONDITION(p2);
    return (p2 != 0 && (owl::ConvertVoidPtr<T>(i)->*M)(*reinterpret_cast<TOcLinkView*>(p2))) ? TRUE : FALSE;
  }
};
...
template <> struct TDocViewDispatch<WM_OWLNOTIFY>::TNotificationDispatch<vnLinkMoniker>
{
  ...
  {
    PRECONDITION(p2);
    return (p2 != 0 && (owl::ConvertVoidPtr<T>(i)->*M)(*reinterpret_cast<owl::TString*>(p2))) ? TRUE : FALSE;
  }
};
...
class _OCFCLASS TOleView : public TOleWindow, public owl::TView {
  ...
  protected:
    TOcView* CreateOcView(owl::TRegLink* link, bool isEmbedded, IUnknown* outer);
    void CleanupWindow();
...
class _OCFCLASS TOleLinkView : public owl::TView {
  ...
  //DECLARE_STREAMABLE_OCF(ocf::TOleLinkView,1);
};

// define streameable inlines (VC)
//DECLARE_STREAMABLE_INLINES( ocf::TOleLinkView );
