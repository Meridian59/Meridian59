...
public:
  ...
  //
  /// Returns the index of the next selected item after the given index. The default
  /// value of -1 will return the index of first selected item. If no item is selected,
  /// or there is no selected item after the given index, -1 is returned.
  //
  int GetNextSelIndex(int index = -1) const;
  ...
  //
  /// Stores in `str` at most `maxChars` characters (including the null
  /// terminator) of the string for the selected item `subitemIndex`, provided
  /// the list view has the LVS_SINGLESEL style. If no item is selected, or
  /// the list view does not have the LVS_SINGLESEL style, false is returned.
  //
  bool GetSelString(tchar* str, int maxChars, int subitemIndex = 0) const;

  //
  /// String-aware overload. Returns an empty string if no item is selected, or
  /// the list view does not have the LVS_SINGLESEL style.
  //
  tstring GetSelString(int subitemIndex) const;
  ...
  int GetStringWidth(LPCTSTR text);
  int GetStringWidth(const tstring& text);
  int CalculateColumnWidth(LPCTSTR text, int padding = 12);
  int CalculateColumnWidth(const tstring& text, int padding = 12);
