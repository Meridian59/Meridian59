...
class _OWLCLASS TWindow : virtual public TEventHandler,
  ...
  protected:
    ...
    //
    /// For code safety and 64-bit compatibility the old parameter to PerformCreate is no longer
    /// used, and the signature of PerformCreate has been changed to force a compilation error for
    /// legacy code. This is achieved by changing the return type but leaving the old parameter in
    /// place, although now unused.
    //
    virtual THandle PerformCreate(int deprecated_argument = 0);
...
{
  PRECONDITION(GetHandle());
  return reinterpret_cast<HTASK>(static_cast<UINT_PTR>(::GetWindowThreadProcessId(GetHandle(), 0)));
}
