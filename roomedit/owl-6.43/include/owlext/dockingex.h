...
class OWLEXTCLASS TDockableControlBarEx : public owl::TDockableControlBar
{
  ...
  public:
    ...
    TDockableControlBarEx(
      ...
      owl::TFont* font = new owl::TGadgetWindowFont,   // see default constructor
    ...
    owl::TResult WindowProc(owl::TMsgId, owl::TParam1, owl::TParam2);
    // Processes incoming messages when toolbar is hide

    void PaintGadgets(owl::TDC& dc, bool b, owl::TRect& rect);
...
public:
  ...
  void DockableInsert(owl::TDockable& dockable, const owl::TPoint* topLeft,
    owl::TRelPosition position, owl::TDockable* relDockable);
  void DockableRemoved(const owl::TRect& orgRect);
...
class OWLEXTCLASS TEdgeSlipEx : public owl::TWindow, public owl::TDockingSlip
{
  public:
    ...
    void     EvCommandEnable(owl::TCommandEnabler& commandEnabler);
    void     SetupWindow();

    // These are overriden for any window that wants to be notified on dockable
    // windows being moved or removed.
    void DockableInsert(owl::TDockable& dockable, const owl::TPoint* topLeft = 0,
      owl::TRelPosition position = owl::rpNone, owl::TDockable* relDockable = 0);
    void DockableMove(owl::TDockable& dockable, const owl::TPoint* topLeft = 0,
      owl::TRelPosition position = owl::rpNone, owl::TDockable* relDockable = 0);
    void DockableRemoved(const owl::TRect& orgRect);
    owl::TAbsLocation GetLocation() const;
