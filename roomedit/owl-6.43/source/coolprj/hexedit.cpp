...
//-----------------------------------------------------------------------------
static uint8*
binstrcd(uint8* str1, uint len1, uint8* str2, uint len2)
{
  PRECONDITION(str1 && str2);
  uint8* endp = str1 + len1 - len2 + 1;
...
//-----------------------------------------------------------------------------
static uint8*
binrstrcd(uint8* str1, uint len1, uint8* str2, uint len2)
{
  PRECONDITION(str1 && str2);
  uint8* p = str1 + len1 - len2;

  while (p >= str1){
    if (memcmp(p, str2, len2) == 0)
      return p;
    p--;
  }
  return 0;
...
//-----------------------------------------------------------------------------
uint32 THexBuffer::Search(const TEditPos& _searchRange, uint8* text,
...
/*
  ...
  TEditPos searchRange(_searchRange);
...
bool THexFileBuffer::Load(TFile& file)
{
  ...
  uint32 size = status.size;
  if((size = file.Read((uint8*)buffer, status.size)) == TFILE_ERROR)
    return false;
...
THexViewWnd::TTextSelection::Copy()
{
  ...
  if (clipboard.EmptyClipboard()){
    ostrstream ostrm;
    if(Copy(ostrm)){
      retval = TextToClipboard(clipboard, (uint8*)ostrm.str(), static_cast<int>(ostrm.pcount()));
      ostrm.rdbuf()->freeze(false);
    }
  }
  return retval;
}
...
//-----------------------------------------------------------------------------
TWindow::TGetClassNameReturnType THexViewWnd::GetClassName()
{
  return _T("HEXEDIT");
}
//
...
//-----------------------------------------------------------------------------
void
THexViewWnd::Paint(TDC& dc, bool erase, TRect& rect)
{
  TRect clientRect;
  GetClientRect(clientRect);
...
THexViewWnd::GetLineData(int index, uint8* buff, uint size) const
{
  ...
  index = index*GetCharsInLine();
  int retsize = buffer->GetDataSize() - index;
  retsize = std::min(retsize, GetCharsInLine());
  return buffer->GetBuffer(index, buff, retsize);
}
//-----------------------------------------------------------------------------
void
THexViewWnd::DrawMargin(TDC& dc, const TRect& rect, int lineIndex)
{
  // nothing for now
}
//-----------------------------------------------------------------------------
  ...
  if(IsFlagSet(cfDigit)){
    ...
    *p++ = _T (' ');
    ...
    if(width > w){
      TRect frect(rect);
      frect.left = origin.x;
      dc.ExtTextOut(origin, ETO_OPAQUE, &frect, _T(""), 0);
...
//-----------------------------------------------------------------------------
bool
THexViewWnd::Search(const TEditPos& _startPos, LPCTSTR text, uint len,
...
//
void
THexViewWnd::DropText(IDataObject* src, const TPoint& point, DROPEFFECT de)
{
...
THexViewWnd::SetDragData()
{
  if(Selection){
    ...
    char* text  = ostrm.str();
    std::streamsize len = ostrm.pcount();
