...
//
//
struct TJavaSyntaxParser: public TSyntaxParser {
  public:
    TJavaSyntaxParser(TCoolTextWnd* parent):TSyntaxParser(parent){}
    uint32 ParseLine(uint32 cookie, int index, TCoolTextWnd::TTextBlock* buf, int& items);
};
//
_COOLEDFUNC(TSyntaxParser*) JavaParserCreator(TCoolTextWnd* parent)
{
  return  new TJavaSyntaxParser(parent);
}
//
uint32 TJavaSyntaxParser::ParseLine(uint32 dwCookie, int nLineIndex,
