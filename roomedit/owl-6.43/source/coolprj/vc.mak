#
# vc.mak - Makefile for building CoolPrj with Microsoft Visual C++
#
...
#-------------------------------------------------
# Parse the CoolPrj version number from the source.
!if [..\parse_version.bat OWL "..\..\include\owl\version.h" >$(TEMP)\COOLPRJ_VERSION.tmp]
! error Unable to detect the CoolPrj version number.
!endif

!include $(TEMP)\COOLPRJ_VERSION.tmp
COOLPRJ_VERSION=$(OWL_VERSION)
COOLPRJ_PRERELEASE=$(OWL_PRERELEASE)

!if "$(COOLPRJ_VERSION)" == ""
! error COOLPRJ_VERSION is undefined (parsing failed).
!endif

!message COOLPRJ_VERSION = $(COOLPRJ_VERSION)
!message COOLPRJ_PRERELEASE = $(COOLPRJ_PRERELEASE)
...
DELIM=-
IMMEDIATENAME=$(BASENAME)$(DELIM)$(COMPILER)$(DELIM)$(TARGET_PLATFORM)$(DELIM)$(BUILD_MODES)$(IMPLIB_MODE)
TARGETNAME=$(BASENAME)$(DELIM)$(COOLPRJ_VERSION)$(DELIM)$(COMPILER)$(DELIM)$(TARGET_PLATFORM)$(DELIM)$(BUILD_MODES)
...
!ifndef OBJDIR
OBJDIR=$(ROOTDIR)\obj\$(IMMEDIATENAME)
!endif
...
LFLAGS=$(LFLAGS) /version:$(COOLPRJ_VERSION)
!endif

!if $(COOLPRJ_PRERELEASE) == 0
RCDEFINES=$(RCDEFINES) /DRELEASE
LFLAGS=$(LFLAGS) /release
!endif

LFLAGS=$(LFLAGS) /dll
DLL_OPT=/D_DLL /D_BUILDCOOLPRJDLL /D_OWLDLL
RCDEFINES=$(RCDEFINES) /D_DLL /D_BUILDCOOLPRJDLL /D_OWLDLL
...
# Compiler options
CL_OPT=/W3 /Zl /EHsc /GR $(CDEFINES) $(DEBOPTS) $(CVOPTS) $(TARGOPTS) /I$(ROOTDIR)\include /I.\ /D_CRT_NONSTDC_NO_DEPRECATE /D_CRT_SECURE_NO_DEPRECATE /D_CRT_NON_CONFORMING_SWPRINTFS /D_SCL_SECURE_NO_WARNINGS
...
CPPFLAGS=$(CPPFLAGS) $(CL_OPT) $(PDBOPTS) $(DEFS) $(OPT) $(DLL_OPT) $(LIB_OPT)
NOHDRFLAGS=$(CL_OPT) $(PDBOPTS) $(DEFS) $(OPT) $(DLL_OPT) $(LIB_OPT) /DNOHDR
...
PCH_HEADER="coolprj/pch.h"
CPPFLAGS=$(CPPFLAGS) /D_COOLPRJPCH /D_OWLPCH /Yu$(PCH_HEADER) /Fp$(PCH_FILE)
!endif
...
HDRS=$(ROOTDIR)\include\owl\*.h $(ROOTDIR)\include\owl\private\*.h $(ROOTDIR)\include\coolprj\*.h
...
$(PCH_TARGETS):: msc\$(PCH_CPP).cpp $(HDRS)
  $(MSCL) @<<
/Yc /Fp$(PCH_FILE) $(CL_OPT) $(DLL_OPT) $(LIB_OPT) $(PDBOPTS) $(DEFS) $(OPT) /D_OWLPCH /D_COOLPRJPCH /c msc\$(PCH_CPP).cpp
<<
!if $(BROWSE) == 1
...
PCH_STATIC=coolprjpchs
CPPFLAGS_STATIC=$(CL_OPT) $(DEFS) $(DEBDEF) /I$(ROOTDIR)\include /D_OWLDLL /D_OWLPCH /D_COOLPRJDLL /D_COOLPRJPCH $(OPT) $(EH) /Gy /Fd$(LIBDIR)\$(TARGETNAME)i.pdb /Fp$D\$(PCH_STATIC).pch
!if $(DEBUG) != 0
...
# Resources
$D\$(BASENAME).res: coolprj.rc $(ROOTDIR)\include\coolprj\version.h
  $(MSRC) $(RCFLAGS) $(RCDEFINES) /i$(ROOTDIR)\include;$(ROOTDIR)\include\owl;$(ROOTDIR)\include\coolprj /fo $D\$(BASENAME).res coolprj.rc
