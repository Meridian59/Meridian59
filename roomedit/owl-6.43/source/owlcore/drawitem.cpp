...
{
  TBtnBitmap* bitmap = new TBtnBitmap(*module, resId, TColor::LtGray);
 ...
 TDrawItemProxy(CreateProxyMenuFont())
{
  ...
  { // map colors
    TBtnBitmap bitmap(barRes.GetBitmap(), TColor::LtGray, NoAutoDelete);
  }
...
TDrawItem* TDrawMenuItemProxy::FindItem(uint cmdId)
{
  ...
  {
    TDrawMenuItem* const m = dynamic_cast<TDrawMenuItem*>(items[index]);
    if (m && m->GetCmdId() == static_cast<int>(cmdId))
      break;
  }
  return index < n ? items[index] : 0;
}
//
