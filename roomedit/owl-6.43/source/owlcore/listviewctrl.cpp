...
int
TListViewCtrl::GetNextSelIndex(int index) const
{
...
{
  return (GetStyle() & LVS_SINGLESEL) ? GetNextSelIndex() : -1;
}
...
bool
TListViewCtrl::GetSelString(tchar* str, int maxChars, int subitemIndex) const
{
  // This function is for single select list views only.
  //
  if (!(GetStyle() & LVS_SINGLESEL))
    return false;
...
tstring
TListViewCtrl::GetSelString(int subitemIndex) const
{
  // This function is for single select list views only.
  //
  if (!(GetStyle() & LVS_SINGLESEL))
...
//
/// Returns the exact width in pixels of the given string using the control's
/// current font. This value must be padded to avoid truncation if used to set
/// a column width using InsertColumn or SetColumnWidth.
//
/// Reference:
///   http://msdn.microsoft.com/en-us/library/windows/desktop/bb761073.aspx
//
int
TListViewCtrl::GetStringWidth(LPCTSTR text)
{
  PRECONDITION(GetHandle());
...
//
int
TListViewCtrl::GetStringWidth(const tstring& text)
{
  PRECONDITION(GetHandle());
...
//
/// Returns a suitable column width for the given string.
/// The value can be used to set a column width using InsertColumn or SetColumnWidth.
/// The default amount of padding is 12 pixels.
//
int
TListViewCtrl::CalculateColumnWidth(LPCTSTR text, int padding)
{
  PRECONDITION(GetHandle());
...
//
int
TListViewCtrl::CalculateColumnWidth(const tstring& text, int padding)
{
  PRECONDITION(GetHandle());
