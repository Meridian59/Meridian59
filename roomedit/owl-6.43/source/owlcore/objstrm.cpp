...
void __TStreamableCont::CreateTypes()
{
  ...
  // Note that we use double-checked locking here. This may have issues, see
...
{
  int lower = 0;
  int upper = static_cast<int>(Size()) - 1;
  while (lower <= upper)
  {
...
{
  const int res = Data->Find(TPWObj(((tchar*)(void*)d)+1,0));
  if (res == Data->NPOS)
    return 0;
  else
...
{
  const int res = Data->Find(TPWObj(d,0));
  if (res == Data->NPOS)
    return 0;
  else
    return (*Data)[res].Ident;
