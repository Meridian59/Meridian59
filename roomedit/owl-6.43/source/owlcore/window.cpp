...
{
  PRECONDITIONX(arg == 0, _T("The deprecated argument to PerformCreate is disallowed.")); InUse(arg);
  PRECONDITION(GetModule());
  //
...
TWindow::Create()
{
  ...
  WARNX(OwlWin, !GetHandle(), 0, _T("TWindow::Create failed"));
  if (!GetHandle())
...
TWindow::SetCursor(TModule* module, TResId resId)
{
  ...
  if (GetHandle()) {
    ...
    if (GetClientRect().Contains(p))
      ::SetCursor(HCursor ? HCursor : reinterpret_cast<HCURSOR>(::GetClassLongPtr(GetHandle(), GCLP_HCURSOR)));
  }
