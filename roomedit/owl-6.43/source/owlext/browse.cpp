    ...
    case ODA_SELECT: {
      ...
      while (i){
        ...
        if (header->Visible){
          ...
          const int loc = cols->Find(TColumn(header->Id, _T(""), 0));
          if (loc != cols->NPOS)
          {
            TStyle colStyle((*cols)[loc].StyleNo);
            int locStyle = Styles->Find(&colStyle);
...
bool TBrowse::GetColumnText(int id, owl::tstring& text, int index)
{
  ...
  if (itemData != LB_ERR){
    ...
    const int loc = cols->Find(TColumn(id, _T(""), 0));
    if (loc != cols->NPOS)
    {
      text = (*cols)[loc].Text;
      bOk = true;
...
bool TBrowse::GetColumnWidth(int id, int &width, bool& exactWidth)
{
  ...
  const int loc = Headers->Find(&idHeader);
  if (loc != Headers->NPOS)
  {
...
void TBrowse::Paint(TDC& dc, bool /*erase*/, TRect& rect)
{
  ...
  if (HIBYTE(LOWORD(GetVersion())) >= 95 || LOBYTE(LOWORD(GetVersion())) > 3) //??????????
    ...
    ViewportOrgX = (int)((float(HorzExtent) - List->GetClientRect().Width())
...
bool TBrowse::SetCompareItemProc(int colId, TCompareFunc compareFunc)
{
  ...
  const int loc = Headers->Find(&colIdHeader);
  if (loc != Headers->NPOS)
  {
...
{
  TStyle idStyle(id);
  const int loc = Styles->Find(&idStyle);
  if (loc != Styles->NPOS)
  {
