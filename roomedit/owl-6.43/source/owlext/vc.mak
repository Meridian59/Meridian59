...
#-------------------------------------------------
# Parse the OWLExt version number from the source.
!if [..\parse_version.bat OWL "..\..\include\owl\version.h" >$(TEMP)\OWLEXT_VERSION.tmp]
! error Unable to detect the OWLExt version number.
!endif

!include $(TEMP)\OWLEXT_VERSION.tmp
OWLEXT_VERSION=$(OWL_VERSION)
OWLEXT_PRERELEASE=$(OWL_PRERELEASE)

!if "$(OWLEXT_VERSION)" == ""
! error OWLEXT_VERSION is undefined (parsing failed).
!endif

!message OWLEXT_VERSION = $(OWLEXT_VERSION)
!message OWLEXT_PRERELEASE = $(OWLEXT_PRERELEASE)
...
TARGETNAME=$(BASENAME)$(DELIM)$(OWLEXT_VERSION)$(DELIM)$(COMPILER)$(DELIM)$(TARGET_PLATFORM)$(DELIM)$(BUILD_MODES)
...
LFLAGS=$(LFLAGS) /version:$(OWLEXT_VERSION)
!endif

!if $(OWLEXT_PRERELEASE) == 0
RCDEFINES=$(RCDEFINES) /DRELEASE
LFLAGS=$(LFLAGS) /release
!endif

LFLAGS=$(LFLAGS) /dll
DLL_OPT=/D_DLL /D_BUILDOWLEXTDLL /D_OWLDLL
RCDEFINES=$(RCDEFINES) /D_DLL /D_BUILDOWLEXTDLL /D_OWLDLL
...
CPPFLAGS=$(CPPFLAGS) /D_OWLEXTPCH /D_OWLPCH /Yu$(PCH_HEADER) /Fp$(PCH_FILE)
!endif
...
HDRS=$(ROOTDIR)\include\owl\*.h $(ROOTDIR)\include\owl\private\*.h $(ROOTDIR)\include\owlext\*.h
...
$(PCH_TARGETS): msc\$(PCH_CPP).cpp $(HDRS)
  $(MSCL) @<<
/Yc /Fp$(PCH_FILE) $(CL_OPT) $(DLL_OPT) $(LIB_OPT) $(PDBOPTS) $(DEFS) $(OPT) /D_OWLPCH /D_OWLEXTPCH /c msc\$(PCH_CPP).cpp
<<
!if $(BROWSE) == 1
...
PCH_STATIC=owlextpchs
CPPFLAGS_STATIC=$(CL_OPT) $(DEFS) $(DEBDEF) /I$(ROOTDIR)\include /D_OWLDLL /D_OWLPCH /D_OWLEXTDLL /D_OWLEXTPCH $(OPT) $(EH) /Gy /Fd$(LIBDIR)\$(TARGETNAME)i.pdb /Fp$D\$(PCH_STATIC).pch
!if $(DEBUG) != 0
...
# Resources
$D\$(BASENAME).res: owlext.rc $(ROOTDIR)\include\owlext\version.h
  $(MSRC) $(RCFLAGS) $(RCDEFINES) /i$(ROOTDIR)\include;$(ROOTDIR)\include\owl;$(ROOTDIR)\include\owlext /fo $D\$(BASENAME).res owlext.rc
