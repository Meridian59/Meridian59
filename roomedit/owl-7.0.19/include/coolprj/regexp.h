...
#ifndef COOLPRJ_REGEX_H
#define COOLPRJ_REGEX_H
...
_COOLFUNC(RxNode*) RxCompile(LPCTSTR expr);
_COOLFUNC(int) RxExec(RxNode* expr, LPCTSTR data, int len, LPCTSTR start,
                         RxMatchRes* match, owl::uint rxOpt = RX_CASE);
_COOLFUNC(int) RxReplace(LPCTSTR rep, LPCTSTR src, int len,
                            RxMatchRes match, LPTSTR* dest, int* dlen);
_COOLFUNC(void) RxFree(RxNode* node);
