...
//
/// Forces the check box to be checked or unchecked by calling SetCheck() with
/// the value BF_CHECKED or BF_UNCHECKED based on the value of `check`, which
/// defaults to true. Notifies the associated group box, if there is one, that
...
inline uint TCheckBox::GetCheck() const {
  ...
  return static_cast<uint>(CONST_CAST(TCheckBox*, this)->SendMessage(BM_GETCHECK));
}
