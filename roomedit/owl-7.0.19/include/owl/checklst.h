  ...
  : public TCheckList
{
  public:
    ...
    TTabCheckList(TWindow* parent, int id, int x, int y, int w, int h, TCheckListArray& items, TModule* module = 0);
    TTabCheckList(TWindow* parent, int resourceId, TCheckListItem* items=0, int numItems=0, TModule* module = 0);
    TTabCheckList(TWindow* parent, int resourceId, TCheckListArray& items, TModule* module = 0);
...
//
inline
TTabCheckList::TTabCheckList(TWindow* parent, int id, int x, int y, int w, int h, TCheckListItem* items, int numItems, TModule* module)
:
...
//
inline
TTabCheckList::TTabCheckList(TWindow* parent, int id, int x, int y, int w, int h, TCheckListArray& items, TModule* module)
:
...
//
/// Creates a tabbed checklist based on the given resource identifier.
/// \note The constructor does not take ownership of the items. The lifetime and memory of the 
/// items must be handled by the caller, which must ensure the items outlive the checklist.
//
inline
TTabCheckList::TTabCheckList(TWindow* parent, int resourceId, TCheckListItem* items, int numItems, TModule* module)
:
...
//
inline
TTabCheckList::TTabCheckList(TWindow* parent, int resourceId, TCheckListArray& items, TModule* module)
:
...
//
inline void TCheckListItem::SetData(UINT_PTR data)
{
  UserData = data;
}
...
//
inline UINT_PTR TCheckListItem::GetData()
{
  return UserData;
}
...
{
  PRECONDITION(index < static_cast<int>(Items.size())); return Items[index];
}
...
{
  PRECONDITION(index < static_cast<int>(Items.size()));
