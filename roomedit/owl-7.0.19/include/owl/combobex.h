...
inline HWND TComboBoxEx::GetEditControl(){
  ...
  return reinterpret_cast<HWND>(SendMessage(CBEM_GETEDITCONTROL,0));
}

//
inline HWND TComboBoxEx::GetComboBoxCtrl(){
  PRECONDITION(GetHandle());
  return reinterpret_cast<HWND>(SendMessage(CBEM_GETCOMBOCONTROL, 0));
}

//
inline uint32 TComboBoxEx::GetExtendedStyle() const{
  PRECONDITION(GetHandle());
  return static_cast<uint32>(CONST_CAST(TComboBoxEx*,this)->SendMessage(CBEM_GETEXTENDEDSTYLE, 0));
}

//
inline uint32 TComboBoxEx::SetExtendedStyle(uint32 mask, uint32 style){
  PRECONDITION(GetHandle());
  return static_cast<uint32>(SendMessage(CBEM_SETEXTENDEDSTYLE, TParam1(mask), TParam2(style)));
}

//
inline int TComboBoxEx::DeleteItem(int index){
  PRECONDITION(GetHandle());
  return static_cast<int>(SendMessage(CBEM_DELETEITEM, TParam1(index)));
}

//
inline bool TComboBoxEx::HasEditChanged(){
  PRECONDITION(GetHandle());
  return SendMessage(CBEM_HASEDITCHANGED,0) == TRUE;
}
...
//
inline void TComboBoxEx::DeleteItem (DELETEITEMSTRUCT &){
  DefaultProcessing();
}
