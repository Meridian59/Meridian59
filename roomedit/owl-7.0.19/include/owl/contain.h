...
void TCollection<T>::InsertAt(uint index, T* item)
{
  ...
  if (count == limit)
    SetLimit(count*2);
  if (count-index != 0)
    memmove(reinterpret_cast<void*>(&items[index+1]), reinterpret_cast<void*>(&items[index]),
...
void TCollection<T>::RemoveAt(uint index)
{
  PRECONDITION(index < count);
  if (index != count-1)
    memmove(reinterpret_cast<void*>(&items[index]), reinterpret_cast<void*>(&items[index+1]),
