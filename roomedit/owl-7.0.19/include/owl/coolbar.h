...
TCoolBand::BeginDrag(const TPoint& point)
{
  ...
  const auto index = Parent->FBands->Find(this);
  if (index != Parent->FBands->NPOS)
    Parent->SendMessage(RB_BEGINDRAG, static_cast<TParam1>(static_cast<uint>(index)), MkUint32(static_cast<uint16>(point.x),static_cast<uint16>(point.y)));
}
...
{
  PRECONDITION(Parent && Parent->GetHandle());
  Parent->SendMessage(RB_DRAGMOVE, 0, MkUint32(static_cast<uint16>(point.x),static_cast<uint16>(point.y)));
}
...
TCoolBand::GetBorders(TRect& rect)
{
  ...
  const auto index = Parent->FBands->Find(this);
  if (index != Parent->FBands->NPOS)
    Parent->SendMessage(RB_GETBANDBORDERS, static_cast<TParam1>(index), reinterpret_cast<TParam2>(static_cast<LPRECT>(&rect)));
}
...
TCoolBand::GetRect(TRect& rect)
{
  ...
  const auto index = Parent->FBands->Find(this);
  if (index != Parent->FBands->NPOS)
    Parent->SendMessage(RB_GETRECT, static_cast<TParam1>(index), reinterpret_cast<TParam2>(static_cast<LPRECT>(&rect)));
}
//
//
//
...
TCoolBand::Maximize(bool ideal)
{
  ...
  const auto index = Parent->FBands->Find(this);
  if (index != Parent->FBands->NPOS)
    Parent->SendMessage(RB_MAXIMIZEBAND, static_cast<TParam1>(index), static_cast<TParam2>(ideal));
}
//
//
//
...
TCoolBand::Minimize()
{
  ...
  const auto index = Parent->FBands->Find(this);
  if (index != Parent->FBands->NPOS)
    Parent->SendMessage(RB_MINIMIZEBAND,static_cast<TParam1>(index));
}
//
//
//
...
TCoolBar::SetBgColor(const TColor& clr)
{
  FBgColor = clr;
  if (GetHandle())
    SendMessage(RB_SETBKCOLOR, 0, TParam2(static_cast<COLORREF>(clr)));
}
//
inline TColor&
TCoolBar::GetBgColor()
{
  if (GetHandle())
    FBgColor = static_cast<COLORREF>(SendMessage(RB_GETBKCOLOR,0,0));
  return FBgColor;
}
//
inline void
TCoolBar::SetTextColor(const TColor& clr)
{
  FTextColor = clr;
  if (GetHandle())
    SendMessage(RB_SETTEXTCOLOR, 0, TParam2(static_cast<COLORREF>(clr)));
}
//
inline TColor&
...
{
  if (GetHandle())
    FTextColor = static_cast<COLORREF>(SendMessage(RB_GETTEXTCOLOR,0,0));
  return FTextColor;
}
...
TCoolBar::GetRowCount()
{
  ...
  return static_cast<uint>(SendMessage(RB_GETROWCOUNT));
}
...
{
  return static_cast<uint>(SendMessage(RB_GETROWHEIGHT, index,0));
}
...
{
  PRECONDITION(GetHandle());
  return static_cast<uint>(SendMessage(RB_GETBARHEIGHT));
}
...
{
  IDropTarget* iface;
  SendMessage(RB_GETDROPTARGET,0,reinterpret_cast<TParam2>(&iface));
...
{
  PRECONDITION(GetHandle());
  return static_cast<uint>(SendMessage(RB_HITTEST,0,TParam2(&info)));
}
//
//
//
...
{
  PRECONDITION(GetHandle());
  return static_cast<int>(SendMessage(RB_IDTOINDEX,id));
}
//
//
//
