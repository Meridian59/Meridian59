...
//    TUIBorder(const TRect& frame, TStyle style, uint flags = 0);
    ...
    bool        PlgBlt(const TPoint* points, const TDC& srcDC,
    ...
    int         GetDIBits(const TBitmap& bitmap, uint startScan, uint numScans,
                          void * bits, const BITMAPINFO & info,
                          uint16 usage);
    int         GetDIBits(const TBitmap& bitmap, TDib& dib);
    int         SetDIBits(TBitmap& bitmap, uint startScan, uint numScans,
                          const void * bits, const BITMAPINFO & Info,
                          uint16 usage);
    int         SetDIBits(TBitmap& bitmap, const TDib& dib);
    int         SetDIBitsToDevice(const TRect& dst, const TPoint& src,
                                  uint startScan, uint numScans,
                                  const void * bits,
                                  const BITMAPINFO & bitsInfo, uint16 usage);
    int         SetDIBitsToDevice(const TRect& dst, const TPoint& src,
                                  const TDib& dib);
    int         StretchDIBits(const TRect& dst, const TRect& src,
                              const void * bits,
    ...
    virtual bool ExtTextOut(int x, int y, uint16 options, const TRect* r,
        const tstring& str, int count = -1, const int * dx = nullptr);
    ...
    virtual int DrawText(const tstring& str, int count, TRect&, uint16 format = 0);
     ...
     virtual int DrawTextEx(LPTSTR str, int count, TRect* = nullptr,
        uint format=0, LPDRAWTEXTPARAMS = nullptr);
...
//
class _OWLCLASS TNCPaintDC : public TWindowDC {
public:
  //hPaintRgn is the WParam of WM_NCPAINT
  TNCPaintDC(HWND wnd, HRGN hPaintRgn);

private:
  TNCPaintDC(const TNCPaintDC&);
  TNCPaintDC& operator =(const TNCPaintDC&);
};
...
class _OWLCLASS TMetaFileDC : public TDC {
  public:
    ...
    TMetaFileDC(const tstring& filename);
     TMetaFileDC(const TDC& dc, LPCTSTR filename = nullptr, TRect* rect = nullptr,
...
class _OWLCLASS TCreatedDC : public TDC {
  public:
    ...
    TCreatedDC(LPCTSTR driver, LPCTSTR device, LPCTSTR output, const DEVMODE* initData = nullptr);
    TCreatedDC(const tstring& driver, const tstring& device, const tstring& output, const DEVMODE* = nullptr);
...
/// \class TIC
// ~~~~~ ~~~
/// Derived from TCreatedDC, TIC is a device context (DC) class that provides a constructor
...
class _OWLCLASS TIC : public TCreatedDC {
  public:
    ...
    TIC(const tstring& driver, const tstring& device, const DEVMODE* = nullptr);

#if defined(OWL5_COMPAT)

    TIC(LPCTSTR driver, LPCTSTR device, LPCTSTR output, const DEVMODE* initData = nullptr);
    TIC(const tstring& driver, const tstring& device, const tstring& output, const DEVMODE* = nullptr);
...
/// \class TMemoryDC
// ~~~~~ ~~~~~~~~~
/// A device context (DC) class derived from TCreatedDC, TMemoryDC provides access to a
/// memory DC.
...
class _OWLCLASS TMemoryDC : public TCreatedDC {
  ...
  protected:
    ...
    TMemoryDC(LPCTSTR driver, LPCTSTR device, const DEVMODE* initData = nullptr);
    TMemoryDC(const tstring& driver, const tstring& device = tstring{}, const DEVMODE* = nullptr);

#if defined(OWL5_COMPAT)

    TMemoryDC(LPCTSTR driver, LPCTSTR device, LPCTSTR output, const DEVMODE* initData = nullptr);
    TMemoryDC(const tstring& driver, const tstring& device, const tstring& output, const DEVMODE* = nullptr);
...
class _OWLCLASS TPrintDC : public TCreatedDC {
  public:
    ...
    TPrintDC(LPCTSTR driver, LPCTSTR device, LPCTSTR output, const DEVMODE* initData = nullptr);
    TPrintDC(const tstring& driver, const tstring& device, const tstring& output, const DEVMODE* = nullptr);

#endif

     int         Escape(int escape, int count=0, const void* inData=nullptr,
    ...
    {return StartDoc(docName.c_str(), output.empty() ? nullptr : output.c_str());}
...
{
  return ::EnumObjects(GetHDC(), objectType, proc, reinterpret_cast<LPARAM>(data));
}
...
{
  return ::DPtoLP(GetAttributeHDC(), reinterpret_cast<LPPOINT>(&rect), 2);
}
...
{
  return ::LPtoDP(GetAttributeHDC(), reinterpret_cast<LPPOINT>(&rect), 2);
}
...
inline int TDC::EnumMetaFile(const TMetaFilePict& metafile, MFENUMPROC callback,
                             void * data) const
{
  return ::EnumMetaFile(GetHDC(), metafile, callback, reinterpret_cast<LPARAM>(data));
}
...
{
  return ::PlayMetaFile(GetHDC(), const_cast<TMetaFilePict&>(metafile));
}
...
{
  return ::MoveToEx(GetHDC(), x, y, nullptr);
}
...
{
  return ::MoveToEx(GetHDC(), point.x, point.y, nullptr);
}
...
{
  TRect r(x1, y1, x2, y2);
  return ::ExtTextOut(GetHDC(), 0, 0, ETO_OPAQUE, &r, nullptr, 0, nullptr);
}
...
{
  return ::ExtTextOut(GetHDC(), 0, 0, ETO_OPAQUE, &rect, nullptr, 0, nullptr);
}
...
/// brush, brush. The width and height of the border is specified by the brushSize argument.
/// Returns true if the call is successful; otherwise, returns false.
//
inline bool TDC::FrameRgn(const TRegion& region, const TBrush& brush, const TSize& brushSize)
{
  return ::FrameRgn(GetHDC(), region, brush, brushSize.cx, brushSize.cy);
}
...
{
  return ::Polygon(GetHDC(), const_cast<TPoint*>(points), count);
}
...
{
  return ::PolyPolygon(GetHDC(), const_cast<TPoint*>(points), polyCounts, count); // API typecast
}
...
{
  return ::PolyPolyline(GetHDC(), const_cast<TPoint*>(points), reinterpret_cast<const DWORD*>(polyCounts), count); // API typecast
}
...
{
  return ::PolyBezier(GetHDC(), const_cast<TPoint*>(points), count);
}
...
{
  return ::PolyBezierTo(GetHDC(), const_cast<TPoint*>(points), count);
}
...
{
  return ::PolyDraw(GetHDC(), const_cast<TPoint*>(points), types, count);
}
...
{
  return ::PolylineTo(GetHDC(), const_cast<TPoint*>(points), count);
}
...
//
inline bool TDC::PlgBlt(const TPoint* points,
                        const TDC& srcDC, const TRect& src,
                        const TBitmap& maskBm, const TPoint& maskPos)
{
  return ::PlgBlt(GetHDC(), const_cast<TPoint*>(points),
...
//
inline int TDC::GetDIBits(const TBitmap& bitmap, uint startScan, uint numScans,
                           void * bits, const BITMAPINFO & info,
                           uint16 usage)
{
  return ::GetDIBits(GetHDC(), bitmap, startScan, numScans, bits,
    const_cast<BITMAPINFO*>(&info), usage);
}
...
/// GetDIBits returns true if the call is successful; otherwise, it returns false.
//
inline int TDC::GetDIBits(const TBitmap& bitmap, TDib& dib)
{
...
//
inline int TDC::SetDIBits(TBitmap& bitmap, uint startScan, uint numScans,
                           const void * bits, const BITMAPINFO & info,
                           uint16 usage)
{
  return ::SetDIBits(GetHDC(), bitmap, startScan, numScans,
    bits, const_cast<BITMAPINFO*>(&info), usage);
  // API typecast
}
...
/// SetDIBits returns true if the call is successful; otherwise, it returns false.
//
inline int TDC::SetDIBits(TBitmap& bitmap, const TDib& dib)
{
  return ::SetDIBits(GetHDC(), bitmap, dib.StartScan(), dib.NumScans(),
    dib.GetBits(), const_cast<BITMAPINFO*>(dib.GetInfo()), dib.Usage());
  // API typecast
}
...
/// false.
//
inline int TDC::SetDIBitsToDevice(const TRect& dst, const TPoint& src,
                                   uint startScan, uint numScans,
                                   const void * bits,
                                   const BITMAPINFO & info, uint16 usage)
{
  return ::SetDIBitsToDevice(
    ...
    startScan, numScans,
    const_cast<void*>(bits), const_cast<BITMAPINFO*>(&info), usage
  ); // API typecast
}
                                   ...
                                   const TDib& dib)
{
  return ::SetDIBitsToDevice(
    ...
    const_cast<void*>(dib.GetBits()),
    const_cast<BITMAPINFO*>(dib.GetInfo()), dib.Usage()
  ); // API typecast
}
...
//
inline int TDC::StretchDIBits(const TRect& dst, const TRect& src,
                               const void * bits,
                               ...
                               uint16 usage, uint32 rop)
{
  return ::StretchDIBits(
    ...
    bits, const_cast<BITMAPINFO*>(&info), usage, rop
  );
  // API typecast
}
                               ...
                               const TDib& dib, uint32 rop)
{
  return ::StretchDIBits(
    ...
    src.left, src.top, src.Width(), src.Height(),
    dib.GetBits(),
    const_cast<BITMAPINFO*>(dib.GetInfo()),
    dib.Usage(), rop
  );
  // API typecast
}
...
{
#ifdef WINELIB
  return ::CreateDIBSection(GetHDC(), const_cast<BITMAPINFO*>(&info), usage, bits, section, offset);
#else
...
{
  return GetTextFace(0, nullptr) - 1;
}
...
//
/// Closes the enhanced-metafile device context and returns a handle that identifies an enhanced-format metafile.
//
...
inline bool TMetaFileDC::Comment(uint bytes, const void* data)
{
  if (IsEnhanced())
  {
    return ::GdiComment(GetHDC(), bytes, reinterpret_cast<const BYTE*>(data));
  }
  return false;
}
...
{
  return ::Escape(GetHDC(), esc, count, reinterpret_cast<LPCSTR>(inData), outData);
  // API type cast
}
...
{
  DocInfo.lpszDocName = const_cast<LPTSTR>(docName);
  DocInfo.lpszOutput = const_cast<LPTSTR>(output);

#if (WINVER >= 0x0400) //sga added
  DocInfo.lpszDatatype = nullptr; //sga added
#endif //sga added
...
{
  return Escape(BANDINFO, sizeof(TBandInfo), nullptr, reinterpret_cast<LPTSTR>(&bandInfo));
}
...
{
  return Escape(NEXTBAND, 0, nullptr, reinterpret_cast<void*>(&rect));
}
