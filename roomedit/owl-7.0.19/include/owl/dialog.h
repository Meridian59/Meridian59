...
class _OWLCLASS TDialog : virtual public TWindow {
  public:
    // standard constructor
     ...
     explicit TDialog(TWindow* parent, const DLGTEMPLATE& dlgTemplate, TAutoDelete = AutoDelete, TModule* module = nullptr);
     // Not usual,to avoid ambiquaity, construct from handle to template
     explicit TDialog(HGLOBAL hTemplate, TWindow* parent, TAutoDelete = AutoDelete, TModule* module=nullptr);
...
//
inline bool
TDialog::MapDialogRect(TRect& rect) const
{
  PRECONDITION(GetHandle());
  return ::MapDialogRect(GetHandle(), &rect) != FALSE;
}
