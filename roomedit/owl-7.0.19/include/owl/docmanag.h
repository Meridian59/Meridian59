...
class _OWLCLASS TDocManager : virtual public TEventHandler,
  ...
  public:
    ...
    //
     int                   GetTemplateCount(TDocument* ofDocType = nullptr,
    ...
    virtual int           GetTemplateDescription(TDocTemplate** tpllist,
                                                                 int tplcount, tchar* buff = nullptr,
                                                                 int size = 0);
    ...
    virtual TDocument*    CreateDoc(TDocTemplate* tpl, LPCTSTR path,
                                                TDocument* parent=nullptr, long flags=0);
  ...
  protected:
    ...
    virtual int           SelectDocPath(TDocTemplate** tpllist, int tplcount,
                                        ...
                                        long flags, bool save=false, TDocument* doc = nullptr);
...
class _OWLCLASS TDvFileOpenDialog : public TFileOpenDialog, public TDvOpenSaveDataOwner {
  public:
    TDvFileOpenDialog(TWindow* parent, TDvOpenSaveData& data,
                             LPCTSTR title = nullptr, TModule* module = nullptr);
  protected:
...
{
  ::InterlockedIncrement(&tpl.RefCnt);  //Staso: ++ is very vulnerable under multi-threading
}
...
{
  if (::InterlockedDecrement(&tpl.RefCnt) == 0)//Staso: -- is very vulnerable under multi-threading
    delete &tpl;
}
...
{
  return tpl ? static_cast<TDocTemplate*>(tpl->GetNextTemplate()) : TemplateList;
}
