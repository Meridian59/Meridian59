...
/// Makes a copy of a document template.
  ...
  private:
    ...
    LONG          RefCnt;        ///< Documents attached + 1 if attached to docmgr
...
class TDocTemplateT : public TDocTemplate {
  public:
...
{
  return static_cast<TDocTemplate*>(Next);
}
...
{
  return new V(static_cast<D&>(doc));
}
