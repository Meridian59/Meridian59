...
class _OWLCLASS TEdit : public TStatic {
  public:
    TEdit(TWindow* parent, int id, LPCTSTR text, int x, int y, int w, int h,
      uint textLimit = 0, bool multiline = false, TModule* = nullptr);
  ...
  DECLARE_STREAMABLE_OWL(TEdit, 2);
};
...
{
  PRECONDITION(GetHandle());
  return static_cast<int>(CONST_CAST(TEdit*,this)->SendMessage(EM_GETLINECOUNT));
}
...
{
  PRECONDITION(GetHandle());
  return static_cast<bool>(CONST_CAST(TEdit*,this)->SendMessage(EM_GETMODIFY));
}
...
{
  PRECONDITION(GetHandle());
  return static_cast<int>(CONST_CAST(TEdit*,this)->SendMessage(EM_LINEFROMCHAR, charPos));
}
...
{
  PRECONDITION(GetHandle());
  return static_cast<int>(CONST_CAST(TEdit*,this)->SendMessage(EM_LINEINDEX, lineNumber));
}
...
{
  PRECONDITION(GetHandle());
  CONST_CAST(TEdit*,this)->SendMessage(EM_GETRECT, 0, TParam2(&frmtRect));
}
...
{
  PRECONDITION(GetHandle());
  SendMessage(EM_SETRECT, 0, TParam2(&frmtRect));
}
...
{
  PRECONDITION(GetHandle());
  SendMessage(EM_SETRECTNP, 0, TParam2(&frmtRect));
}
...
{
  PRECONDITION(GetHandle());
  return reinterpret_cast<HLOCAL>(CONST_CAST(TEdit*,this)->SendMessage(EM_GETHANDLE));
}
...
{
  PRECONDITION(GetHandle());
  return static_cast<int>(CONST_CAST(TEdit*,this)->SendMessage(EM_GETFIRSTVISIBLELINE));
}
...
{
  PRECONDITION(GetHandle());
  return static_cast<uint>(CONST_CAST(TEdit*,this)->SendMessage(EM_GETPASSWORDCHAR));
}
...
{
  PRECONDITION(GetHandle());
  return reinterpret_cast<EDITWORDBREAKPROC>(CONST_CAST(TEdit*,this)->SendMessage(EM_GETWORDBREAKPROC));
}
...
{
  PRECONDITION(GetHandle());
  return static_cast<bool>(CONST_CAST(TEdit*,this)->SendMessage(EM_CANUNDO));
}
...
{
  PRECONDITION(GetHandle());
  SendMessage(EM_SETMARGINS, EC_LEFTMARGIN, MkUint32(margin, 0));
}
...
{
  PRECONDITION(GetHandle());
  SendMessage(EM_SETMARGINS, EC_RIGHTMARGIN, MkUint32(0, margin));
}
...
{
  PRECONDITION(GetHandle());
  return static_cast<uint32>(CONST_CAST(TEdit*,this)->SendMessage(EM_GETMARGINS));
}
...
{
  PRECONDITION(GetHandle());
  return static_cast<int>(CONST_CAST(TEdit*,this)->SendMessage(EM_GETLIMITTEXT));
}

//
inline uint32 TEdit::CharFromPos(int16 x, int16 y) {
  PRECONDITION(GetHandle());
  return static_cast<uint32>(SendMessage(EM_CHARFROMPOS, 0, MkUint32(x, y)));
}
