...
#include <mmiscapi.h> // FOURCC_RIFF
#include <fstream>
...
{
  uint16 w=0x0001;
  uint8 *b=reinterpret_cast<uint8*>(&w);
...
#if !defined(owlFCC)
#  if 0
#    define owlFCC(ch4) (((static_cast<DWORD>(ch4) & 0xFF) << 24) |    \
                       ((static_cast<DWORD>(ch4) & 0xFF00) << 8) |    \
                       ((static_cast<DWORD>(ch4) & 0xFF0000) >> 8) |  \
                       ((static_cast<DWORD>(ch4) & 0xFF000000) >> 24))
#  else
//#  define extFCC(ch4) ((DWORD)(ch4))
#    define owlFCC(ch0, ch1, ch2, ch3)                                \
      (static_cast<uint32>(static_cast<uint8>(ch0)) | (static_cast<uint32>(static_cast<uint8>(ch1)) << 8) |          \
      (static_cast<uint32>(static_cast<uint8>(ch2)) << 16) | (static_cast<uint32>(static_cast<uint8>(ch3)) << 24 ))
#  endif
#endif
...
class _OWLCLASS TStreamFile : public TFile, public _tfstream {
  public:
    TStreamFile();
...
inline TFile::TFile()
:
  Handle(FileNull),ShouldClose(false),Buffer(nullptr),BufSize(0)
{
}
...
:
  Handle(FileNull),  ShouldClose(true), Buffer(nullptr), BufSize(0)
{
  Open(fileName, mode);
}
...
:
  Handle(handle), ShouldClose(shouldClose), Buffer(nullptr), BufSize(0)
{
}
//
/// Returns Handle.
...
inline uint64 TFile::SeekToBegin64(){
  return Seek(static_cast<int64>(0), beg);
}
//
...
inline uint64 TFile::SeekToEnd64(){
  return Seek(static_cast<int64>(0), end);
}
...
/// information, call LastError.
inline uint32 TFile::Seek(long offset, TSeekDir origin){
  PRECONDITION(Handle);
...
inline uint64 TFile::Seek(int64 offset, TSeekDir origin)
{
  PRECONDITION(Handle);
  return Handle->Seek(offset, origin);
}
...
inline TFile&  operator >> ( TFile& file, int8& c){
  c = static_cast<int8>(file.readUint8());
  return file;
}
//
inline TFile&  operator >> ( TFile& file, uint8& c){
  c = static_cast<uint8>(file.readUint8());
  return file;
}
//
inline TFile&  operator >> ( TFile& file, int16& i){
  i = static_cast<int16>(file.readUint16());
  return file;
}
//
...
inline TFile&  operator >> ( TFile& file, signed int& i){
  i = static_cast<signed int>(file.readUint32());
  return file;
}
//
inline TFile&  operator >> ( TFile& file, unsigned int& i){
  i = static_cast<uint>(file.readUint32());
  return file;
}
//
...
inline TFile&  operator >> ( TFile& file, int32& i){
  i = static_cast<int32>(file.readUint32());
  return file;
}
...
inline TFile&  operator >> ( TFile& file, int64& i){
  i = static_cast<int64>(file.readUint64());
  return file;
}
...
// writing support
inline TFile&  operator << ( TFile& file, int8 c){
  file.writeUint8(static_cast<uint8>(c));
  return file;
}
inline TFile&  operator << ( TFile& file, uint8 c){
  file.writeUint8(c);
...
inline TFile&  operator << ( TFile& file, int16 i){
  file.writeUint16(static_cast<int16>(i));
  return file;
}
inline TFile&  operator << ( TFile& file, uint16 i){
  file.writeUint16(static_cast<uint16>(i));
  return file;
}
inline TFile&  operator << ( TFile& file, signed int i){
  file.writeUint32(static_cast<signed int>(i));
  return file;
}
inline TFile&  operator << ( TFile& file, unsigned int i){
  file.writeUint32(static_cast<uint>(i));
  return file;
}
inline TFile&  operator << ( TFile& file, bool b){
  file.writeUint32(static_cast<uint32>(b));
  return file;
}
inline TFile&  operator << ( TFile& file, int32 i){
  file.writeUint32(static_cast<int32>(i));
  return file;
}
inline TFile&  operator << ( TFile& file, uint32 i){
  file.writeUint32(static_cast<uint32>(i));
  return file;
}
inline TFile&  operator << ( TFile& file, int64 i){
  file.writeUint64(*reinterpret_cast<uint64*>(&i));
  return file;
}
inline TFile&  operator << ( TFile& file, uint64 i){
  file.writeUint64(static_cast<uint64>(i));
  return file;
}
inline TFile&  operator << ( TFile& file, float f){
  file.writeFloat(f);
...
: File(nullptr),LineBuffer(nullptr),LineNumber(0),Done(false)
{
}
//
...
inline const tchar* TFileLineIterator::operator *() const{
  return Done ? nullptr : LineBuffer;
}
//
/// Returns a pointer to the start of the buffer if the file has not been completely
/// iterated through; otherwise returns 0.
...
//
inline uint TFileLineIterator::Line() const{
  return LineNumber;
}
