...
class _OWLCLASS TIPAddressBits {
  public:
    ...
    static uint32 MakeIPAddress(int f0, int f1, int f2, int f3)
     {  return static_cast<uint32>(((static_cast<uint32>(f0)<<24)+(static_cast<uint32>(f1)<<16)+(static_cast<uint32>(f2)<<8)+(static_cast<uint32>(f3)))); }
...
inline void TIPAddress::SetAddress(const TIPAddressBits& address){
  PRECONDITION(GetHandle());
  SendMessage(IPM_SETADDRESS, 0, TParam2(static_cast<uint32>(const_cast<TIPAddressBits&>(address))));
}

//
inline int TIPAddress::GetAddress(TIPAddressBits& address) const{
  PRECONDITION(GetHandle());
  return static_cast<int>(const_cast<TIPAddress*>(this)->SendMessage(IPM_GETADDRESS, 0, TParam2(&address.Address)));
}

//
inline bool TIPAddress::IsBlank() const{
  PRECONDITION(GetHandle());
  return ToBool(const_cast<TIPAddress*>(this)->SendMessage(IPM_ISBLANK, 0, 0));
}
