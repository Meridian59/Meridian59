...
class _OWLCLASS TMenu {
  public:
    ...
    /// @{
    bool        AppendMenu(uint flags, TMenuItem newItem=static_cast<unsigned int>(-1), LPCTSTR newStr=nullptr); //JJH added static cast
    ...
    int         GetMenuString(uint item, LPTSTR str, int count, uint flags) const;
    ...
    bool        InsertMenu(uint item, uint flags, TMenuItem newItem, const tstring& newStr) {return InsertMenu(item, flags, newItem, newStr.c_str());}
    ...
    bool        ModifyMenu(uint item, uint flags, TMenuItem newItem, const tstring& newStr) {return ModifyMenu(item, flags, newItem, newStr.c_str());}
    bool        ModifyMenu(uint item, uint flags, TMenuItem newItem, const TBitmap& newBmp);
    bool        RemoveMenu(uint item, uint flags);
    bool        SetMenuItemBitmaps(uint item, uint flags,
                                              const TBitmap* bmpUnchecked=nullptr,
                                              const TBitmap* bmpChecked=nullptr);
    /// @}
...
/// that require a TMenuItem menu parameter.
//
inline TMenu::operator TMenuItem() const
{
  return reinterpret_cast<TMenuItem>(GetHandle());
}
...
inline bool TMenu::AppendMenu(uint flags, TMenuItem newItem, const TBitmap& newBmp)
{
  ...
  return ::AppendMenu(Handle, flags|MF_BITMAP, newItem,
         reinterpret_cast<LPCTSTR>(newBmp.GetHandle()));
}
...
/// (MF_UNCHECKED).
inline bool TMenu::CheckMenuItem(uint item, uint check)
{
  PRECONDITION(Handle);
...
inline bool TMenu::DeleteMenu(uint item, uint flags)
{
  PRECONDITION(Handle);
...
/// successful.
inline bool TMenu::EnableMenuItem(uint item, uint enable)
{
  PRECONDITION(Handle);
...
//
inline uint TMenu::GetMenuState(uint item, uint flags) const
{
  PRECONDITION(Handle);
...
inline int TMenu::GetMenuString(uint item, LPTSTR str, int count, uint flags) const
{
  PRECONDITION(Handle);
...
inline bool TMenu::InsertMenu(uint item, uint flags, TMenuItem newItem, LPCTSTR newStr)
{
  PRECONDITION(Handle);
...
inline bool TMenu::InsertMenu(uint item, uint flags, TMenuItem newItem, const TBitmap& newBmp)
{
...
inline bool TMenu::ModifyMenu(uint item, uint flags, TMenuItem newItem, LPCTSTR newStr)
{
  PRECONDITION(Handle);
...
inline bool TMenu::ModifyMenu(uint item, uint flags, TMenuItem newItem, const TBitmap& newBmp)
{
  PRECONDITION(Handle);
...
inline bool TMenu::RemoveMenu(uint item, uint flags)
{
  PRECONDITION(Handle);
...
inline bool TMenu::SetMenuItemBitmaps(uint item, uint flags,
                                      const TBitmap* bmpUnchecked,
                                      const TBitmap* bmpChecked)
{
  PRECONDITION(Handle);
  return ::SetMenuItemBitmaps(Handle, item, flags,
                              bmpUnchecked ? HBITMAP(*bmpUnchecked) : nullptr,
                              bmpChecked ? HBITMAP(*bmpChecked) : nullptr);
}
