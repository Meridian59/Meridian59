...
TMetaFilePict::SetPlaceableHeader(TRect& bounds, uint16 unitsPerInch) {
  ...
  MFHeader.bbox.left   = static_cast<int16>(bounds.left);
  MFHeader.bbox.top    = static_cast<int16>(bounds.top);
  MFHeader.bbox.right  = static_cast<int16>(bounds.right);
  MFHeader.bbox.bottom = static_cast<int16>(bounds.bottom);
...
inline uint TEnhMetaFilePict::GetBits(uint bytes, void* buffer) {
  return ::GetEnhMetaFileBits(*this, bytes, static_cast<LPBYTE>(buffer));
}
...
inline uint TEnhMetaFilePict::GetDescription(uint bytes, void* buffer) {
  return ::GetEnhMetaFileDescription(*this, bytes, static_cast<LPTSTR>(buffer));
}
