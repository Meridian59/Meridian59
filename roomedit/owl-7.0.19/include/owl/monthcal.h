...
inline bool TMonthCalendar::SetFirstDayOfWeek(TDaysOfWeek day){
  PRECONDITION(GetHandle());
   return ToBool(HiUint16(SendMessage(MCM_SETFIRSTDAYOFWEEK,0,TParam2(static_cast<int>(day)))));
}
...
inline int TMonthCalendar::GetMaxSelCount() const{
  PRECONDITION(GetHandle());
  return static_cast<int>(CONST_CAST(TMonthCalendar*,this)->SendMessage(MCM_GETMAXSELCOUNT, 0, 0));
}
...
inline int TMonthCalendar::SetMonthDelta(int delta){
  PRECONDITION(GetHandle());
  return static_cast<int>(SendMessage(MCM_SETMONTHDELTA, TParam1(delta), 0));
}

//
inline int TMonthCalendar::GetMonthDelta() const{
  PRECONDITION(GetHandle());
  return static_cast<int>(CONST_CAST(TMonthCalendar*,this)->SendMessage(MCM_GETMONTHDELTA, 0, 0));
}
...
inline void TMonthCalendar::SetUnicode(bool unicode){
  PRECONDITION(GetHandle());
  SendMessage(MCM_SETUNICODEFORMAT, TParam1(static_cast<BOOL>(unicode)), 0);
}

//
inline uint32 TMonthCalendar::HitTest(TMCHitTestInfo& hitinfo){
  PRECONDITION(GetHandle());
  return static_cast<uint32>(SendMessage(MCM_HITTEST, 0, TParam2(&hitinfo)));
}
