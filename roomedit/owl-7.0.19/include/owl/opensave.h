...
/// opened or saved.
    class _OWLCLASS TData {
      public:
        ...
        /// This is the return value from calling CommDlgExtendedError after executing the dialog
        /// in the implementation of TOpenSaveDialog::Execute.
        ///
        /// \note An error may have occured, even if Execute returns IDOK. You should therefore
        /// always test this error code, regardless of the return value. Also note that, in
        /// earlier versions of OWLNext, the error code was always incorrectly set to 0 when IDOK
        /// was returned. Make sure that you test the error code before the return value.
        ///
        /// Usage:
        ///
        /// \code
        ...
        /// if (data.Error == FNERR_BUFFERTOOSMALL)
        /// {
        ...
        /// \endcode
        /// 
        /// \sa http://docs.microsoft.com/en-gb/windows/desktop/api/commdlg/nf-commdlg-commdlgextendederror
        uint32  Error;
        ...
        TData(
          uint32 flags = 0,
          LPCTSTR filter = nullptr, ///< Note: Copied; the string is stored in an internal buffer.
          LPTSTR customFilter = nullptr, ///< Note: Not used; dummy for backward compatibility.
          LPCTSTR initialDir = nullptr, ///< Note: Not copied; the string pointed to must outlive this object.
          LPCTSTR defExt = nullptr, ///< Note: Not copied; the string pointed to must outlive this object.
          int maxPath = 0,
          int filterIndex = 0,
          uint32 flagsEx = 0);
  ...
  protected:
    TOpenSaveDialog(TWindow* parent, TData& data, TModule*   module = nullptr);
...
/// TOpenSave::TData structure to initialize the file open dialog box.
///
/// \note Simply regard the return value from Execute as an indication of confirmation (IDOK) or
/// cancellation (IDCANCEL), and test TOpenSaveDialog::TData::Error for failure in both cases. The
/// error code may indicate issues, such as buffer overflow.
///
/// Usage:
///
/// \code
...
{
  return ::GetFileTitle(fileName, nullptr, 0);
}
...
inline int TOpenSaveDialog::GetFileTitle(LPCTSTR fileName,
                                         ...
                                         int fileTitleLen)
{
  return ::GetFileTitle(fileName, fileTitle, static_cast<WORD>(fileTitleLen));
}
