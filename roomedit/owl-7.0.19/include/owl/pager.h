...
inline void TPageScroller::ForwardMouse(bool forward){
  ...
  SendMessage(PGM_FORWARDMOUSE, TParam1(static_cast<BOOL>(forward)), 0);
}

//
inline void TPageScroller::SetBkColor(const TColor& clr){
  PRECONDITION(GetHandle());
  SendMessage(PGM_SETBKCOLOR, 0, TParam2(static_cast<COLORREF>(clr)));
}
...
inline int TPageScroller::GetBorder() const{
  PRECONDITION(GetHandle());
   return static_cast<int>(CONST_CAST(TPageScroller*,this)->SendMessage(PGM_GETBORDER,0,0));
}
...
inline int TPageScroller::GetButtonSize() const{
  PRECONDITION(GetHandle());
   return static_cast<int>(CONST_CAST(TPageScroller*,this)->SendMessage(PGM_GETBUTTONSIZE,0,0));
}

inline int TPageScroller::GetButtonState(TPagerButton button) const{
  PRECONDITION(GetHandle());
   return static_cast<int>(CONST_CAST(TPageScroller*,this)->SendMessage(PGM_GETBUTTONSTATE,0,TParam2(button)));
}
...
inline int TPageScroller::GetPosition() const{
  PRECONDITION(GetHandle());
   return static_cast<int>(CONST_CAST(TPageScroller*,this)->SendMessage(PGM_GETPOS,0,0));
}
