...
template<class T> class TPointerBase {
  public:
    T&   operator  *() {return *P;}
         operator T*() {return P;}
    int  operator  !() const {return P==nullptr;}
    T*   Relinquish() {T* p = P; P = nullptr; return p;}

  protected:
    TPointerBase(T* pointer) : P(pointer) {}
    TPointerBase() : P(nullptr) {}
  ...
  private:
    ...
    void operator delete(void* p) {(static_cast<TPointerBase<T>*>(p))->P=0;}
#endif
...
class TVoidPointer {
  public:
    TVoidPointer( const void *p = nullptr ) : Ptr(p) {}
...
template<class T> class TComRefBase {
  public:
    ...
    int     operator !() const    {return I==nullptr;}
    void operator delete(void* p) {(static_cast<TComRefBase<T>*>(p))->I=0;}
  protected:
    TComRefBase(const TComRefBase<T>& i) : I(i.I) {if (I) I->AddRef();}
    TComRefBase(T* i) : I(i) {}
    TComRefBase()     : I(nullptr) {}
    ~TComRefBase()   {Clear();}

    void Clear() {if(I){I->Release(); I = nullptr;}}
    T* I;
  private:
