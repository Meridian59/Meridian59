...
class _OWLCLASS TPrintDialog : public TCommonDialog
{
  public:
    ...
    TPrintDialog(TWindow* parent, TData&,
      TResId printTemplate = TResId{},
      TResId setupTemplate = TResId{},
      LPCTSTR title = 0,
      TModule* = 0);
  ...
  private:
    ...
    // NOTE: We store both a TResId and a string for each template. If the TResId, passed to the
    // primary constructor, is a string pointer, then a copy of the string is made, and the TResID
    // is set to refer to the buffered string (see Init). This prevents issues with dangling string
    // pointers. If the TResId is a short integer, or NULL, then the corresponding string is empty.
    // For the secondary constructor, that takes an owl::tstring template name, a copy of the
    // string is made, and the corresponding TResId is set to refer to the copy (again, see Init).
    //
    TResId PrintTemplate;
    tstring PrintTemplateName;
    TResId SetupTemplate;
