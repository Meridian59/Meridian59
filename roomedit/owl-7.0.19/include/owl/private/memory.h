...
inline LPSTR OwlW2AHelper(LPSTR lpa, LPCWSTR lpw, int nChars, unsigned int acp)
{
  ...
  // don't worry about the number of chars
  lpa[0] = '\0';
  WideCharToMultiByte(acp, 0, lpw, -1, lpa, nChars, nullptr, nullptr);
  return lpa;
}
...
template<class T> class TTmpBuffer: public TTmpBufferBase {
  public:
    ...
    T&   operator  *()              {return *static_cast<T*>(Buffer);}
         operator T*()              {return static_cast<T*>(Buffer);}
    bool operator  !() const        {return Buffer==0;}
    T& operator[](int i)            {return (static_cast<T*>(Buffer))[i];}
};
