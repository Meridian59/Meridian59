...
// We target Windows API version 0x0600 (Windows Vista).
//
#ifndef _WIN32_WINNT
...
// Omit legacy stuff from the Windows header.
// For instance, without WIN32_LEAN_AND_MEAN, "Windows.h" will include Windows Sockets ("Winsock")
// version 1.1, which is incompatible with version 2.
//
// See http://msdn.microsoft.com/en-us/library/windows/desktop/ms737629.aspx
//
// So to make OWLNext most broadly compatible we need to be lean here.
//
...
// Avoid mapping MessageBoxIndirect to MessageBoxIndirect[A/W]
#if defined(MessageBoxIndirect)
#undef MessageBoxIndirect
inline auto MessageBoxIndirect(const MSGBOXPARAMS* p) -> int
#if defined(UNICODE)
  { return ::MessageBoxIndirectW(p); }
#else
  { return ::MessageBoxIndirectA(p); }
#endif
#endif
...
#if defined(GetIconInfoEx)
#undef GetIconInfoEx
inline auto GetIconInfoEx(HICON hIcon, PICONINFOEX piconinfo) -> BOOL
#if defined(UNICODE)
{ return ::GetIconInfoExW(hIcon, piconinfo); }
#else
{ return ::GetIconInfoExA(hIcon, piconinfo); }
#endif
#endif
