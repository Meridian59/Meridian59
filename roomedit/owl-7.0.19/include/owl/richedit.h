...
// !BB    bool    GetFont(LOGFRONT& lf);
    ...
    void GetSubText(LPTSTR textBuf, int startPos, int endPos) const override; // deprecated
    ...
    auto GetLineFromPos(int charPos) const -> int override; // EM_EXLINEFROMCHAR
...
{
  cbSize = sizeof(::CHARFORMAT);//LB, for VCL build
  dwMask = mask;
}
...
{
  memset(reinterpret_cast<FORMATRANGE*>(this), 0, sizeof(FORMATRANGE));
}
