...
class _OWLCLASS TStatic : public TControl {
  public:
    ...
    TStatic(TWindow* parent, int id, LPCTSTR title, int x, int y, int w, int h,
      uint textLen = 0, TModule* = nullptr);
...
inline HICON TStatic::GetIcon() const {
  ...
  return reinterpret_cast<HICON>(CONST_CAST(TStatic*,this)->SendMessage(STM_GETICON));
}
...
inline HICON TStatic::SetIcon(HICON icon) {
  PRECONDITION(GetHandle());
  return reinterpret_cast<HICON>(SendMessage(STM_SETICON, TParam1(icon)));
}
...
{
  PRECONDITION(GetHandle());
  return reinterpret_cast<HANDLE>(CONST_CAST(TStatic*,this)->SendMessage(STM_GETIMAGE,static_cast<TParam2>(imageType)));
}
...
{
  PRECONDITION(GetHandle());
  return reinterpret_cast<HANDLE>(SendMessage(STM_SETIMAGE, static_cast<TParam1>(imageType), reinterpret_cast<TParam2>(image)));
}
