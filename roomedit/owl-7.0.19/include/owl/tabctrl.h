...
class _OWLCLASS TTabControl : public TControl {
  public:
   ...
   ~TTabControl() override;
    ...
    void        SetMinTabWidth(int width); // Version 4.71
    /// @}
...
{
  *(reinterpret_cast<TC_ITEM*>(this)) = tbItem;
}
...
inline void TTabControl::DeselectAll(bool exclFocus)
{
  ...
  SendMessage(TCM_DESELECTALL, TParam1(static_cast<BOOL>(exclFocus)));
}
...
{
  PRECONDITION(GetHandle());
  return static_cast<uint32>(SendMessage(TCM_SETEXTENDEDSTYLE, mask, style));
}
...
{
  PRECONDITION(GetHandle());
  return SendMessage(TCM_HIGHLIGHTITEM,item, MkUint32(static_cast<uint16>(static_cast<BOOL>(highlight)),0));
}

// Version 4.71
inline void TTabControl::SetMinTabWidth(int width)
{
  PRECONDITION(GetHandle());
