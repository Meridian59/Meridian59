...
class _OWLCLASS TCountedSemaphore : public TSemaphore{
  public:
     TCountedSemaphore(int initialCount, int maxCount, LPCTSTR name = nullptr,
...
class _OWLCLASS TEventSemaphore : public TSemaphore{
  public:
    TEventSemaphore(bool manualReset=false, bool initialState=false,
                          LPCTSTR name = nullptr, LPSECURITY_ATTRIBUTES sa = nullptr);
...
class _OWLCLASS TWaitableTimer : public TSemaphore {
  public:
    ...
    bool Set(const TFileTime& duetime,  int32 period=0,PTIMERAPCROUTINE compFunc=nullptr,
                 void* param=nullptr,  bool resume=false);
...
{
  return (index >= 0 && index < Count) ? Sems[index] : nullptr;
}
...
{
  uint32 code;
  ::GetExitCodeThread(Handle, static_cast<DWORD*>(&code));
  return code;
}
