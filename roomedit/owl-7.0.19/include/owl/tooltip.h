...
class _OWLCLASS TTooltip : public TControl {
  public:
    ...
    //
     TTooltip(TWindow* parent, TWindowFlag, TModule* module = nullptr);

     TTooltip(HWND hWnd, TModule* module = nullptr);
    ...
    {return AddTool(::GetDlgItem(toolWnd, ctrlId), tip, enableSubclassing);}

    //
    /// Adds a tip for the given control.
    /// The tip is given as a resource string id.
    //
     bool AddTool(HWND ctrl, int tipResId, TModule* m = nullptr, bool enableSubclassing = true);
    ...
    {return AddTool(::GetDlgItem(toolWnd, ctrlId), tipResId, m, enableSubclassing);}

    //
    /// Adds a tip for the given area in the given tool window.
    /// The tip area (tool) is identified by the given tool ID.
    //
    bool AddTool(HWND toolWnd, const TRect& tipArea, uint toolId, const tstring& tip, bool enableSubclassing = true);
    ...
    //
     bool AddTool(HWND toolWnd, const TRect& tipArea, uint toolId, int tipResId, TModule* m = nullptr, bool enableSubclassing = true);
    ...
    { UpdateTipText({::GetParent(ctrl), ctrl, tip}); }
    ...
    { UpdateTipText({toolWnd, {}, ctrlId, tip}); }
    ...
    { UpdateTipText({::GetParent(ctrl), ctrl, tipResId, m ? m->GetHandle() : GetModule() ? GetModule()->GetHandle() : nullptr}); }
    ...
    { UpdateTipText({toolWnd, {}, ctrlId, tipResId, m ? m->GetHandle() : GetModule() ? GetModule()->GetHandle() : nullptr}); }
...
/// The given TToolInfo must have been initialized with the information needed for the tooltip
/// control to display tip text for the tool.
/// \note Encapsulates the TTM_ADDTOOL message in the Windows API.
/// \sa https://docs.microsoft.com/en-us/windows/win32/controls/ttm-addtool
//
...
/// The given TToolInfo must have been initialized to identify the tool to update.
/// \note Encapsulates the TTM_UPDATETIPTEXT message in the Windows API.
/// \sa https://docs.microsoft.com/en-us/windows/win32/controls/ttm-updatetiptext
//
