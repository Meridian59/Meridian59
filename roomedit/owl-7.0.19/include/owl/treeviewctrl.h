...
/// \sa TTreeNode::SortChildren.
//
class _OWLCLASS TTvComparator {
  public:
    /// Returns a value < 0 if `item1` < `item2`, 0 if `item1` == `item2`, or > 0 if `item1` > `item2`.
    /// Parameter `lParam` is the user-defined value, passed to TTreeNode::SortChildren.
    /// Note that `item1` and `item2` are the used-defined values associated with the items.
    /// Values can be associated with items using TTreeNode::SetItemData.
    //
    /// \sa http://msdn.microsoft.com/en-us/library/windows/desktop/bb773782.aspx
    //
...
#else
      First           = reinterpret_cast<INT_PTR>(TVI_FIRST), ///< First child
      Last            = reinterpret_cast<INT_PTR>(TVI_LAST), ///< Last child
      Sort            = reinterpret_cast<INT_PTR>(TVI_SORT) ///< Sort order
#endif
    };
...
{
  return TreeView->ExpandItem(flag, static_cast<HTREEITEM>(*this));
}
...
{
  return reinterpret_cast<HIMAGELIST>(SendMessage(TVM_SETIMAGELIST, TParam1(type), TParam2(newList)));
}
...
{
  return reinterpret_cast<HIMAGELIST>(SendMessage(TVM_GETIMAGELIST, TParam1(type)));
}
...
{
  return reinterpret_cast<HIMAGELIST>(SendMessage(TVM_CREATEDRAGIMAGE, 0, TParam2(item)));
}
...
{
  return reinterpret_cast<HWND>(SendMessage(TVM_GETEDITCONTROL));
}
...
{
  return static_cast<int>(CONST_CAST(TTreeViewCtrl*,this)->SendMessage(TVM_GETITEMHEIGHT));
}
...
{
  return static_cast<int>(SendMessage(TVM_SETITEMHEIGHT, TParam1(static_cast<int16>(height))));
}
...
{
  return ToBool(SendMessage(TVM_SETINSERTMARK, TParam1(static_cast<BOOL>(after)), TParam2(item)));
}
...
{
  return reinterpret_cast<HWND>(SendMessage(TVM_EDITLABEL, 0, TParam2(item)));
}
...
TTreeViewCtrl::GetNextItem(uint32 nc, HTREEITEM hItem)
{
  if (hItem == TVI_ROOT)  // Workaround for Win95 bug.
    hItem = nullptr;
  return reinterpret_cast<HTREEITEM>(SendMessage(TVM_GETNEXTITEM, nc, TParam2(hItem)));
}
...
{
  return reinterpret_cast<HTREEITEM>(SendMessage(TVM_INSERTITEM, 0, TParam2(tvis)));
}
...
{
  return static_cast<uint>(SendMessage(TVM_GETCOUNT));
}
...
{
  return static_cast<uint>(SendMessage(TVM_GETVISIBLECOUNT));
}
...
{
  return static_cast<uint>(SendMessage(TVM_GETINDENT));
}
