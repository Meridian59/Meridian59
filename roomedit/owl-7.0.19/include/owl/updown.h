...
inline int TUpDown::GetAccel(int count, TUDAccel * accels) const {
  return static_cast<int>(CONST_CAST(TUpDown*,this)->SendMessage(UDM_GETACCEL, count, TParam2(accels)));
}
...
inline int TUpDown::GetBase() const {
  return static_cast<int>(CONST_CAST(TUpDown*,this)->SendMessage(UDM_GETBASE));
}

/// Retrieves handle of buddy window of underlying up-down control
//
inline HWND TUpDown::GetBuddy() const {
  return reinterpret_cast<HWND>(CONST_CAST(TUpDown*,this)->SendMessage(UDM_GETBUDDY));
}
...
inline int32 TUpDown::GetPos() const {
  return static_cast<int32>(LoUint16(CONST_CAST(TUpDown*,this)->SendMessage(UDM_GETPOS)));
}
...
inline uint32 TUpDown::GetRange() const {
  return static_cast<uint32>(CONST_CAST(TUpDown*,this)->SendMessage(UDM_GETRANGE));
}
