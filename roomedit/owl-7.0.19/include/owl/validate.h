...
/// the associated edit control. The constructor or SetPic sets Pic to a string that
/// is passed.
    tstring Pic;
...
class _OWLCLASS TRangeValidator : public TFilterValidator
{
  public:
    ...
    // Range setting and retrieval functions.
    //
...
//
/// Constructs a range validator object using `minValue` and `maxValue` as
/// the range of possible values.
//
inline TRangeValidator::TRangeValidator(TRangeValidator::TExtent minValue, TRangeValidator::TExtent maxValue)
:
  TRangeValidator(TRange {minValue, maxValue}) {}

//
/// Returns the minimum number the validator can accept.
//
inline auto TRangeValidator::GetMin() const -> TExtent
{
  return Range.Min;
}
...
//
inline auto TRangeValidator::GetMax() const -> TExtent
{
  return Range.Max;
}
...
//
inline auto TRangeValidator::GetRange() const -> TRange
{
  return Range;
}
