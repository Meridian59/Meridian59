...
public:
  ...
  static void ReadString(ipstream& is, wchar_t*& s)
  {
    ...
    int size = ::MultiByteToWideChar(CP_ACP, 0, src, -1, nullptr, 0);
    s = new wchar_t[size];
  ...
  {
     int size = ::WideCharToMultiByte(CP_ACP, 0, s, -1, nullptr, 0, nullptr, nullptr);
    std::vector<char> dst(size);
     int r = ::WideCharToMultiByte(CP_ACP, 0, s, -1, &dst[0], size, nullptr, nullptr);
    WARN(r == 0, _T("String conversion failed, GetLastError() == ") << GetLastError());
...
class _OWLCLASS TFileTime: public FILETIME
{
  public:
    ...
    bool ToLocalTime(TFileTime& tm) const
          { return ::FileTimeToLocalFileTime(const_cast<TFileTime*>(this), &tm); }
...
template <class T, int resType> class /*_OWLCLASS*/ TResource {
  public:
   ...
   ~TResource();
    ...
    DWORD     GetSize() const;  ///< Returns the size in bytes of the loaded resource
...
//JJH
#ifdef __GNUC__
  return ::CompareFileTime(const_cast<TFileTime*>(this), const_cast<TFileTime*>(&t)) > 0;
#else
  return ::CompareFileTime(const_cast<TFileTime*>(this), const_cast<TFileTime*>(&t)) > 0;
#endif
}
...
{
  wYear=static_cast<uint16>(y);
  wMonth=static_cast<uint16>(m);
  wDay=static_cast<uint16>(d);
  wHour=static_cast<uint16>(h);
  wMinute=static_cast<uint16>(mn);
  wSecond=static_cast<uint16>(s);
  wDayOfWeek=static_cast<uint16>(dw);
  wMilliseconds=static_cast<uint16>(ms);
}
...
:
  ...
  Size(0)
{
  HRSRC resHandle = FindResource(hModule, resId.GetPointerRepresentation(), reinterpret_cast<tchar *>(resType));
  if (resHandle) {
    Size = SizeofResource(hModule, resHandle);
    MemHandle = LoadResource(hModule, resHandle);
    if (MemHandle)
      MemPtr = static_cast<T*>(LockResource(MemHandle));
  }
}
...
:
  ...
  Size(0)
{
  HRSRC resHandle = FindResourceEx(hModule, reinterpret_cast<tchar *>(resType), resId.GetPointerRepresentation(), lcid);
  if (resHandle) {
    Size = SizeofResource(hModule, resHandle);
    MemHandle = LoadResource(hModule, resHandle);
    if (MemHandle)
...
{
  return ((MemHandle != nullptr) && (MemPtr != nullptr));
}
...
//
//template <class T, const tchar * resType> DWORD
template <class T, int resType> DWORD
TResource<T, resType>::GetSize() const
{
  PRECONDITION(IsOK());
  return Size;
}
