...
//
//
struct TBatchSyntaxParser: public TSyntaxParser {
  public:
    TBatchSyntaxParser(TCoolTextWnd* parent):TSyntaxParser(parent){}
    uint32 ParseLine(uint32 cookie, int index, TCoolTextWnd::TTextBlock* buf, int& items);
};
//
_COOLEDFUNC(TSyntaxParser*) BatchParserCreator(TCoolTextWnd* parent)
{
  return  new TBatchSyntaxParser(parent);
}
//
uint32 TBatchSyntaxParser::ParseLine(uint32 dwCookie, int nLineIndex,
