...
//
//
struct TDclSyntaxParser: public TSyntaxParser {
  public:
    TDclSyntaxParser(TCoolTextWnd* parent):TSyntaxParser(parent){}
    uint32 ParseLine(uint32 cookie, int index, TCoolTextWnd::TTextBlock* buf, int& items);
};
//
_COOLEDFUNC(TSyntaxParser*) DclParserCreator(TCoolTextWnd* parent)
{
  return  new TDclSyntaxParser(parent);
}
//
uint32 TDclSyntaxParser::ParseLine(uint32 dwCookie, int nLineIndex,
