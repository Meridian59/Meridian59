...
//
//
struct TISSyntaxParser: public TSyntaxParser {
  public:
    TISSyntaxParser(TCoolTextWnd* parent):TSyntaxParser(parent){}
    uint32 ParseLine(uint32 cookie, int index, TCoolTextWnd::TTextBlock* buf, int& items);
};
//
_COOLEDFUNC(TSyntaxParser*) ISParserCreator(TCoolTextWnd* parent)
{
  return  new TISSyntaxParser(parent);
}
//
uint32 TISSyntaxParser::ParseLine(uint32 dwCookie, int nLineIndex,
