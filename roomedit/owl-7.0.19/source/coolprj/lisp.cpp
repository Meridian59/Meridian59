...
//
//
struct TLispSyntaxParser: public TSyntaxParser {
  public:
    TLispSyntaxParser(TCoolTextWnd* parent):TSyntaxParser(parent){}
    uint32 ParseLine(uint32 cookie, int index, TCoolTextWnd::TTextBlock* buf, int& items);
};
//
_COOLEDFUNC(TSyntaxParser*) LispParserCreator(TCoolTextWnd* parent)
{
  return  new TLispSyntaxParser(parent);
}
//
uint32 TLispSyntaxParser::ParseLine(uint32 dwCookie, int nLineIndex,
