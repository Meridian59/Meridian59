...
//
//
struct TPerlSyntaxParser: public TSyntaxParser {
  public:
    TPerlSyntaxParser(TCoolTextWnd* parent):TSyntaxParser(parent){}
    uint32 ParseLine(uint32 cookie, int index, TCoolTextWnd::TTextBlock* buf, int& items);
};
//
_COOLEDFUNC(TSyntaxParser*) PerlParserCreator(TCoolTextWnd* parent)
{
  return  new TPerlSyntaxParser(parent);
}
//
uint32 TPerlSyntaxParser::ParseLine(uint32 dwCookie, int nLineIndex,
