...
//
//
struct TPythonSyntaxParser: public TSyntaxParser {
  public:
    TPythonSyntaxParser(TCoolTextWnd* parent):TSyntaxParser(parent){}
    uint32 ParseLine(uint32 cookie, int index, TCoolTextWnd::TTextBlock* buf, int& items);
};
//
_COOLEDFUNC(TSyntaxParser*) PythonParserCreator(TCoolTextWnd* parent)
{
  return  new TPythonSyntaxParser(parent);
}
//
uint32 TPythonSyntaxParser::ParseLine(uint32 dwCookie, int nLineIndex,
