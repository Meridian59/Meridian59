...
struct TShSyntaxParser: public TSyntaxParser {
  public:
    TShSyntaxParser(TCoolTextWnd* parent):TSyntaxParser(parent){}
    uint32 ParseLine(uint32 cookie, int index, TCoolTextWnd::TTextBlock* buf, int& items);
};
//
_COOLEDFUNC(TSyntaxParser*) ShParserCreator(TCoolTextWnd* parent)
{
  return  new TShSyntaxParser(parent);
}
//
uint32 TShSyntaxParser::ParseLine(uint32 dwCookie, int nLineIndex,
