...
//
//
struct TSiodSyntaxParser: public TSyntaxParser {
  public:
    TSiodSyntaxParser(TCoolTextWnd* parent):TSyntaxParser(parent){}
    uint32 ParseLine(uint32 cookie, int index, TCoolTextWnd::TTextBlock* buf, int& items);
};
//
_COOLEDFUNC(TSyntaxParser*) SiodParserCreator(TCoolTextWnd* parent)
{
  return  new TSiodSyntaxParser(parent);
}
//
uint32 TSiodSyntaxParser::ParseLine(uint32 dwCookie, int nLineIndex,
