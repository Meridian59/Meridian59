...
class TAppDictInstImp : public TAppDictImp {
  public:
    TAppDictInstImp() {Entry.App = nullptr;}

    void                       Add(unsigned, TApplication* app) {Entry.App = app;}
    void                       Remove(unsigned) {Entry.App = nullptr;}
    TAppDictionary::TEntry*    Lookup(unsigned) {return &Entry;}
    TAppDictionary::TEntry*    Lookup(TApplication*) {return &Entry;}
...
TAppDictionary::GetApplication(uint pid)
{
  ...
  return entry ? entry->App : nullptr;
}
...
_OWLFUNC(TApplication*) GetApplicationObject(uint pid)
{
