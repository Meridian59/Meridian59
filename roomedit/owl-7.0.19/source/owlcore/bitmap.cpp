...
TBitmap::TBitmap(const TDC& dc, int width, int height, bool discardable)
{
  if (discardable) {
    ...
    WARNX(OwlGDI, !Handle, 0, "Cannot create discardable bitmap (" << width <<
          "x" << height << ") for " << static_cast<void*>(dc.GetHDC()));
  }
  else {
  ...
  if (discardable) {
    TRACEX(OwlGDI, OWL_CDLEVEL, "Discardable TBitmap constructed @" << (void*)this << " (" <<
           dec << width << "x" << height << ") for " << static_cast<void*>(dc.GetHDC()) << ".");
  }
  else  {
...
TBitmap::TBitmap(const TDC& dc, const TDib& dib, uint32 usage)
{
  Handle = ::CreateDIBitmap(dc,
                            const_cast<BITMAPINFOHEADER *>(dib.GetInfoHeader()),
                            usage, static_cast<const uint8 *>(dib.GetBits()),
                            const_cast<BITMAPINFO *>(dib.GetInfo()),
                            dib.Usage());
                            // API casts
  WARNX(OwlGDI, !Handle, 0, "Cannot create bitmap from DIB " << static_cast<void*>(dib.GetHandle())
    << " for " << static_cast<void*>(dc.GetHDC()));
  CheckValid();
  RefAdd(Handle, Bitmap);
  TRACEX(OwlGDI, OWL_CDLEVEL, "TBitmap constructed @" << (void*)this <<
         " from DIB for " << static_cast<void*>(dc.GetHDC()) << ".");
}
...
TBitmap::TBitmap(HINSTANCE instance, TResId resId)
{
  ...
  WARNX(OwlGDI, !Handle, 0, "Cannot load bitmap " << resId <<
    " from " <<  static_cast<void*>(instance));
  CheckValid();
  RefAdd(Handle, Bitmap);
...
TBitmap::TBitmap(const TMetaFilePict& metaFile, TPalette& palette, const TSize& defSize)
{
  ...
  if (nColors > 2) {
    ...
    Handle = ::CreateBitmap(size.cx, size.cy, 1, 1, nullptr);
...
TBitmap::TBitmap(const TDib& dib, const TPalette* palette)
{
  if (palette)
    Create(dib, *palette);
  else if (NColors(dib.GetInfo()->bmiHeader.biBitCount) > 0)
    Create(dib, TPalette(dib));
  else
  {
    //this is the case when the DIB is full-colored and therefore has no palette
    Handle = ::CreateDIBitmap(TScreenDC(),
      const_cast<LPBITMAPINFOHEADER>(dib.GetInfoHeader()),
      CBM_INIT,
      static_cast<const uint8*>(dib.GetBits()),
      const_cast<LPBITMAPINFO>(dib.GetInfo()),
      dib.Usage()
      );
    CheckValid();
    RefAdd(Handle, Bitmap);
...
//
BITMAP
TBitmap::GetObject() const
{
  BITMAP b;
  auto r = GetObject(b);
  if (!r) throw TXOwl(_T("TBitmap::GetObject failed"));
  return b;
}
...
//
TSize
TBitmap::GetBitmapDimension() const
{
  TSize s;
  const auto ok = GetBitmapDimension(s);
  if (!ok) throw TXOwl(_T("TBitmap::GetBitmapDimension failed"));
  return s;
}
...
TBitmap::Create(const TBitmap& src)
{
  ...
  if (bm.bmPlanes != 1 || bm.bmBitsPixel != 1) {
    ...
    Handle = ::CreateBitmap(bm.bmWidth, bm.bmHeight, 1, 1, nullptr);
