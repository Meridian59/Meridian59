...
:
  ...
  ImageArray(nullptr),            // Created on first call to GetDesiredSize
  ...
  BitmapOrigin(0)
{
  if (sharedCels) {
    ResId = 0;
    ImageIndex = imageResIdOrIndex.GetInt();
  }
  else {
...
{
  TCelArray* a = ResId ? CreateCelArray(ResId, NumImages) : nullptr;
  delete ImageArray;
  ImageArray = a;
}
