...
{
  return static_cast<typename std::make_unsigned<T>::type>(item) / 8;
}
...
{
  return static_cast<uint8>(1 << (static_cast<typename std::make_unsigned<T>::type>(item) % 8));
}
  ...
  for (LPCTSTR p = str; *p; p++) {
    if (*p == _T('\\'))
      ...
      for (tchar c = tchar(p[-2]+1); c < *p; c++) // replace "-" with "B..Y"
        EnableItem(static_cast<uint8>(c));
    }
    EnableItem(static_cast<uint8>(*p));
  }
}
