...
TBrushCache::Float(int i)
{
  if(i > 0){
    ...
    TRACEX(OwlGDI, 2, _T("TBrushCache::Float(") << i << _T(") (") <<
      static_cast<void*>(handle) << _T(", ") << hex << color << _T(")"));
    {
      LOCKCACHE(Lock,false);
...
TBrushCache::AddRef(HANDLE handle, COLORREF color)
{
  ...
  TRACEX(OwlGDI, 2, _T("TBrushCache::Add(") << static_cast<void*>(handle) <<
    _T(", ") << hex << color << _T(")"));
  CHECK(Size > 1);
  ...
  for (int i = 0; i < Size && Entries[i].Handle; i++){
    if (handle == Entries[i].Handle) {
      TRACEX(OwlGDI, 2, _T("TBrushCache::CheckHandle(") <<
        static_cast<void*>(Entries[i].Handle) << _T(" @") << i);
      Entries[i].RefCnt++;
      return true;
    }
  }
  TRACEX(OwlGDI, 2, _T("TBrushCache::CheckHandle(") << static_cast<void*>(handle) << _T("): not found"));
  return 0;
}
  ...
  for (int i = 0; i < Size && Entries[i].Handle; i++){
    if (color == Entries[i].Color) {
      TRACEX(OwlGDI, 2, _T("TBrushCache::Lookup(") << hex << color <<
        _T("): found ") << static_cast<void*>(Entries[i].Handle) << _T(" @") << i);
      Entries[i].RefCnt++;
      Float(i);
...
TBrush::TBrush(const TColor& color, bool useCache)
{
  ...
  COLORREF cr = color;
  if(useCache && (Handle = GetBrushCache().Lookup(cr)) != nullptr)
    return;
  ...
  TRACEX(OwlGDI, OWL_CDLEVEL, _T("TBrush constructed @") << (void*)this <<
    _T(" with color ") << static_cast<COLORREF>(color));
}
...
TBrush::TBrush(const TBitmap& pattern)
{
  ...
  WARNX(OwlGDI, !Handle, 0, _T("Cannot create pattern TBrush from bitmap ") <<
    static_cast<void*>(pattern.GetHandle()));
  CheckValid();
  RefAdd(Handle, Brush);
  TRACEX(OwlGDI, OWL_CDLEVEL, _T("TBrush constructed @") << (void*)this <<
    _T(" from bitmap ") << static_cast<const void*>(pattern.GetHandle()));
}
...
TBrush::TBrush(const TDib& pattern)
{
  ...
  RefAdd(Handle, Brush);
  TRACEX(OwlGDI, OWL_CDLEVEL, _T("TBrush constructed @") << (void*)this <<
    _T(" from DIB ") << static_cast<void*>(pattern.GetHandle()));
}
