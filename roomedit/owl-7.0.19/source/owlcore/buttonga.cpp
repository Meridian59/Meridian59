...
TButtonGadget::CheckExclusively()
{
  ...
  if (state != Down) {
#else
  if (static_cast<TState>(State) != Down) {
#endif
    if (GetGadgetWindow()) {
...
TButtonGadget::SetButtonState(TState newState)
{
  ...
  if (newState != state) {
#else
  if (newState != static_cast<TState>(State)) {
#endif
...
TButtonGadget::PaintBorder(TDC& dc)
{
  ...
  if (flat)
  {
    ...
    if (xpstyle)
    {
      ...
      TThemePart part(GetGadgetWindow()->GetHandle(), static_cast<LPCWSTR>(L"TOOLBAR"), TP_BUTTON, state);
      TRect rect(0, 0, GetBounds().Width(), GetBounds().Height());
      part.DrawBackground(dc, rect);
    }
    else
    {
...
#else
  if (!((Type == Exclusive || Type == SemiExclusive) && static_cast<TState>(State) != Down) &&
#endif
      GetEnabled())
