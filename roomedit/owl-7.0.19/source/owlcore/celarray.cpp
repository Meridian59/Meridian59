...
TCelArray::TCelArray(const TDib& dib, int numCels, int numRows)
{
  ...
  TPalette palette(static_cast<HPALETTE>(::GetStockObject(DEFAULT_PALETTE)));
...
//    return -1;
  ...
  OwlCopyBmp(*Bitmap, static_cast<const TBitmap&>(src), CelOffset(NCelsUsed,0),
...
{
  if (index >= 0 && index < NCelsUsed) {
    OwlCopyBmp(*Bitmap, static_cast<const TBitmap&>(src), CelOffset(index,0),
