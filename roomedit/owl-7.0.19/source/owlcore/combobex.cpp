...
TComboBoxEx::SetupWindow()
{
  ...
  if(ImageList)
    SendMessage(CBEM_SETIMAGELIST, 0, reinterpret_cast<LPARAM>(static_cast<HIMAGELIST>(*ImageList)));
}
...
TComboBoxEx::InsertItem(const TComboBoxExItem& item)
{
  ...
  return static_cast<int>(SendMessage(CBEM_INSERTITEM, 0, TParam2(&exitem)));
}
...
{
  if(GetHandle())
    SendMessage(CBEM_SETIMAGELIST, 0, reinterpret_cast<LPARAM>(static_cast<HIMAGELIST>(*list)));
...
{
  PRECONDITION(Items->Size() > static_cast<uint>(index));
  return (*Items)[index];
}
