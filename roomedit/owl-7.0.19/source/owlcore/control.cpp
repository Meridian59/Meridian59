...
TControl::TControl(THandle handle, TModule* module)
{
  ...
  TRACEX(OwlControl, OWL_CDLEVEL, "TControl aliased @" << (void*)this <<
    " to handle" << static_cast<void*>(handle));
}
...
/// Predefined Windows controls handle erasure in their WM_PAINT handler, so if the flag
/// wfPredefinedClass is set, this function will defer to DefaultProcessing. The colors for
/// controls are set by handling the WM_CTLCOLOR... messages (see TWindow::EvCtlColor).
//
auto TControl::EvEraseBkgnd(HDC dc) -> bool
{
  return IsFlagSet(wfPredefinedClass) ? 
