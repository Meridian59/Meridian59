...
//
void TControlGadget::SetVisible(bool visible)
{
  TGadget::SetVisible(visible);
  auto c = GetControl();
  if (c)
    c->ShowWindow(visible ? SW_SHOW : SW_HIDE);
}
...
TControlGadget::SetBounds(const TRect& bounds)
{
  ...
  Control->SetWindowPos(nullptr, Bounds, SWP_NOACTIVATE|SWP_NOZORDER|SWP_NOSIZE);
