...
TCoolBar::Init()
{
  InitializeCommonControls(ICC_COOL_CLASSES);
...
auto TCoolBar::GetWindowClassName() -> TWindowClassName
{
  return TWindowClassName{REBARCLASSNAME};
}
...
TCoolBar::SetupWindow()
{
  ...
  if(FBgColor != TColor::None)
    SendMessage(RB_SETBKCOLOR, 0, TParam2(static_cast<COLORREF>(FBgColor)));
  if(FTextColor != TColor::None)
    SendMessage(RB_SETTEXTCOLOR, 0, TParam2(static_cast<COLORREF>(FTextColor)));
  ...
  for(int i = 0; i < static_cast<int>(FBands->Size()); i++){
    REBARBANDINFO info;
...
{
  if(GetHandle())
    return static_cast<uint>(SendMessage(RB_GETBANDCOUNT));
  return FBands->Size();
}
...
TCoolBar::ToggleBandBorders()
{
  ...
  if(GetHandle()){
    ...
    SetWindowPos(nullptr,0,0,rect.Width(),rect.Height()+1,SWP_NOZORDER|SWP_NOCOPYBITS);
  }
...
TCoolBar::ToggleVarHeight()
{
  ...
  if(GetHandle()){
    ...
    SetWindowPos(nullptr,0,0,rec.Width(),rec.Height()+1,SWP_NOZORDER|SWP_NOCOPYBITS);
  }
  // Return the new setting
  return !result;
}
...
TCoolBar::ToggleBorder()
{
  ...
  if(GetHandle()){
    ...
    if(Parent){
      ...
      Parent->HandleMessage(WM_NOTIFY, GetId(), reinterpret_cast<TParam2>(&notify));
    }
  }
  // Return the new setting
  return !result;
