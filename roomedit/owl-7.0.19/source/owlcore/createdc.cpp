...
/// (driver name) and device (device name). The optional initData argument provides
...
//
TCreatedDC::TCreatedDC(LPCTSTR driver, LPCTSTR device, const DEVMODE* initData)
  : TDC{}
{
  Handle = ::CreateDC(driver, device, nullptr, initData);
  CheckValid();
  TRACEX(OwlGDI, OWL_CDLEVEL, _T("TCreatedDC constructed @") << static_cast<void*>(this)
    << _T(" with driver ") << (driver ? driver : _T("NULL")));
}

//
/// String-aware overload
//
TCreatedDC::TCreatedDC(const tstring& driver, const tstring& device, const DEVMODE* initData)
  : TCreatedDC{driver.empty() ? nullptr : driver.c_str(), device.empty() ? nullptr : device.c_str(), initData}
{}
...
{
  TRACEX(OwlGDI, OWL_CDLEVEL, _T("TCreatedDC constructed @") << (void*)this <<
    _T(" with handle ") << static_cast<void*>(handle));
}
...
//
/// Creates a device information context (IC) for the device specified by driver
/// (driver name) and device (device name). The optional initData argument provides
/// a DEVMODE structure containing device-specific initialization data for this DC.
/// Pass `nullptr` for initData to use the default initializations specified by the user.
//
TIC::TIC(LPCTSTR driver, LPCTSTR device, const DEVMODE* initData)
  : TCreatedDC{}
{
  Handle = static_cast<HANDLE>(::CreateIC(driver, device, nullptr, initData));
  CheckValid();
  TRACEX(OwlGDI, OWL_CDLEVEL, _T("TIC constructed @") << static_cast<void*>(this)
    << _T(" with driver ") << (driver ? driver : _T("NULL")));
}
...
/// String-aware overload
//
TIC::TIC(const tstring& driver, const tstring& device, const DEVMODE* initData)
  : TIC{driver.empty() ? nullptr : driver.c_str(), device.empty() ? nullptr : device.c_str(), initData}
{}
