...
{
  if( year )
    Julnum = Jday( 12, 31, year-1 ) + static_cast<JulTy>(day);
  else
    Julnum = jul1901                + static_cast<JulTy>(day);
}
...
{
  Julnum = t.IsValid() ? jul1901 + static_cast<JulTy>(t.LocalSecs()/SECONDS_IN_DAY) : 0;
}
...
{
    return AssertWeekDayNumber(weekDayNumber) ? WeekDayNames[weekDayNumber-1] : nullptr;
}
...
{
  return static_cast<DayTy>(FindMatch( nameOfDay.c_str(), UCWeekDayNames, 7 )+1);
}
...
{
  return static_cast<MonthTy>(FindMatch(nameOfMonth.c_str(), UCMonthNames, 12 )+1);
}
...
{
    return AssertIndexOfMonth(monthNumber) ? MonthNames[monthNumber-1] : nullptr;
}
...
{
  return static_cast<unsigned>(Julnum);
}
...
void TDate::ParseFrom(LPCTSTR str, LPCTSTR format)
{
  ...
  while (*picture && *str) {
    tchar c = static_cast<tchar>(_totlower(*picture));
    switch (c) {
      case _T('d'): {
