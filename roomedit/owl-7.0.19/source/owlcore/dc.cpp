...
TDC::SelectObject(const TPen& pen)
{
  TRACEX(OwlGDI, 1, _T("TDC::SelectPen @") << (void*)this <<
    _T(" pen @") << (void*)&pen);
  HPEN oldPen = static_cast<HPEN>(::SelectObject(GetHDC(), pen));
  if (oldPen) {
...
TDC::SelectObject(const TBrush& brush)
{
  TRACEX(OwlGDI, 1, _T("TDC::SelectBrush @") << (void*)this <<
    _T(" brush @") << (void*)&brush);
  HBRUSH oldBrush = reinterpret_cast<HBRUSH>(::SelectObject(GetHDC(), brush));
  if (oldBrush) {
...
TDC::SelectObject(const TFont& font)
{
  TRACEX(OwlGDI, 1, _T("TDC::SelectFont @") << (void*)this <<
    _T(" font @") << (void*)&font);
  HFONT oldFont = reinterpret_cast<HFONT>(::SelectObject(GetHDC(), font));
  if (oldFont) {
...
TDC::SelectObject(const TBitmap& bitmap)
{
  ...
  HBITMAP oldBitmap = static_cast<HBITMAP>(::SelectObject(GetHDC(), bitmap));
  if (oldBitmap) {
...
{
  return ::ResetDC(GetHDC(), &devMode) != nullptr;
}
...
TDC::DrawFrameControl(TRect lpRect, UINT nType, UINT nState)
{
  ...
  return ::DrawFrameControl(static_cast<HDC>(Handle), &rect, nType, nState);
}
...
TDC::DrawEdge(const TRect& frame, uint edge, uint flags)
{
  ...
  //
  if (hasDrawEdge) {
    if (::DrawEdge(*this, static_cast<LPRECT>(const_cast<TRect*>(&frame)), edge, flags))
      return true;
