...
TDecoratedFrame::GetHintText(uint id, THintText hintType) const
{
  ...
  if (hintType == htStatus) {
    ...
    if (numBytes == 0 && MergeModule != nullptr)
...
TDecoratedFrame::EvMenuSelect(uint menuItemId, uint flags, HMENU hMenu)
{
  if (TrackMenuSelection) {
    if (flags == 0xFFFF && hMenu == nullptr) {  // Menu closing
...
TDecoratedFrame::TLocation GetLocation_(TWindow& w)
{
  return TDecoratedFrame::TLocation(uint16((w.GetWindowAttr().Style & locMask) >> locShift));
}

void SetLocation_(TWindow& w, TDecoratedFrame::TLocation loc)
{
  ...
  if (hWndCtl == nullptr) {
    ...
    if (decoration) {
      ...
      if (visible) {
        ...
        //
        TLocation location = GetLocation_(*decoration);
        ...
        struct TSiblingSearch
        {
          ...
          TSiblingSearch(TWindow* d, TDecoratedFrame::TLocation loc)
            : Decoration(d), Sibling(nullptr), Location(loc)
          {}
        };
        ...
        {
          auto c = GetChildren();
          const auto i = std::find_if(c.begin(), c.end(), [&](TWindow& w)
            { return ((&w != search.Decoration) && (GetLocation_(w) == search.Location)); });
          search.Sibling = (i != c.end()) ? &(*i) : nullptr;
        }
        else
          for (auto& w : GetChildren())
          {
            if (&w == search.Decoration)
              break;

            if (GetLocation_(w) == search.Location && w.IsWindowVisible())
              search.Sibling = &w;
          }
...
TDecoratedFrame::Insert(TWindow& decoration, TLocation location)
{
  ...
  //
  SetLocation_(decoration, location);
...
TDecoratedFrame::GetControlBar()
{
  ...
  if (!wnd)
  {
    // Look for grand-child toolbar.
    //
    auto c = GetChildren();
    const auto i = find_if(c.begin(), c.end(), [](TWindow& w) { return w.ChildWithId(IDW_TOOLBAR); });
    if (i == c.end()) return nullptr;
    wnd = i->ChildWithId(IDW_TOOLBAR);
    CHECK(wnd);
  }
