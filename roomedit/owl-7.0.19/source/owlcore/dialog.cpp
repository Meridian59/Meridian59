...
static
#endif
_OWLDATA(TDialog*) DlgCreationWindow = nullptr;
...
TDialog::TDialog(TWindow* parent, const DLGTEMPLATE& dlgTemplate, TAutoDelete del, TModule* module)
{
  ...
  DialogAttr.Name = reinterpret_cast<LPTSTR>(const_cast<DLGTEMPLATE*>(&dlgTemplate));
...
TDialog::TDialog(HGLOBAL hTemplate, TWindow* parent, TAutoDelete del, TModule* module)
{
  ...
  DialogAttr.Name = reinterpret_cast<LPTSTR>(GlobalLock(hTemplate));
...
TDialog::PreProcessMsg(MSG& msg)
{
  ...
  THandle child;
  THandle client;
  if ((child = GetParentH()) != nullptr &&
      (client = ::GetParent(child)) != nullptr &&
      child == reinterpret_cast<THandle>(::SendMessage(client, WM_MDIGETACTIVE, 0, 0)) &&
...
TDialog::GetWindowClass(WNDCLASS& wndClass)
{
  ...
  if (!TUser::GetClassInfo(nullptr, dlgClass.GetPointerRepresentation(), &wndClass))
...
TDialog::DialogFunction(TMsgId msg, TParam1 param1, TParam2 param2)
{
  ...
  switch (msg) {
    ...
    //
    case WM_INITDIALOG:
      return EvInitDialog(reinterpret_cast<THandle>(param1));
...
TDialog::PerformCreate()
{
  ...
  THandle h = !IsFlagSet(dfTmplMask) ?
    ::CreateDialogParam (
      *GetModule(),
      DialogAttr.Name,
      GetParentO() ? GetParentO()->GetHandle() : nullptr,
...
TDialog::DoExecute()
{
  ...
  TWindow::THandle p = GetParent() ? GetParent()->GetHandle() : nullptr;
