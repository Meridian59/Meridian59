...
//  PRECONDITION(Bits == 0);
  ...
  TPointer<TPalette> p;
  if (NumColors() != 0)
    p = new TPalette(*reinterpret_cast<TDib*>(this));
  else
    p = new TPalette(reinterpret_cast<HPALETTE>(::GetStockObject(DEFAULT_PALETTE)));

  dc.SelectObject(*static_cast<TPalette*>(p));
  dc.RealizePalette();
  void *bits;
...
TDiBitmap::~TDiBitmap()
{
  Bits = nullptr;  // Make sure that TDib doesn't mess with our unusual bits
}
  ...
  TPalette(nullptr, NoAutoDelete)
{
  ...
  logPal->palNumEntries = static_cast<uint16>(nColors);
