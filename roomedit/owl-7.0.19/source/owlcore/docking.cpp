...
TDockableGadgetWindow::LayoutSession()
{
  ...
  if (GetParentO()) {
    ...
    if (slip) {
      ...
      SetWindowPos(nullptr, 0, 0, Attr.W, Attr.H, SWP_NOACTIVATE | SWP_NOMOVE);
      if (!stc)
...
TFloatingSlip::TFloatingSlip(TWindow*        parent,
                             ...
                             TModule*        module)
:
  TFloatingFrame(parent, clientWnd->GetCaption(), nullptr/*clientWnd*/, shrinkToClient,
...

{
  ...
  if (w) {
    ...
    if (winRect.Height() != newWinRect.Height() ||
      ...
      SetWindowPos(nullptr, newWinRect, SWP_NOACTIVATE | SWP_NOMOVE | SWP_NOZORDER);
    }
  }
}
...
{
  if (topLeft)
    SetWindowPos(nullptr, topLeft->x, topLeft->y, 0, 0, SWP_NOACTIVATE | SWP_NOSIZE);
}
...
vector<TDecoration> MakeDecorationList_(TEdgeSlip& s, TGridType g)
{
  ...
  for (TWindow* w = s.GetFirstChild(); w; w = (w == s.GetLastChild()) ? nullptr : w->Next())
...
TEdgeSlip::SetupWindow()
{
  ...
  if (startWindowChange != -1) {
    for (size_t i = startWindowChange; i < decoList.size(); ++i) {
      ...
      //
      ::MapWindowPoints(nullptr, *this, (TPoint*)&pt, 1);
      decoItem.Window->SetWindowPos(nullptr, pt.x, pt.y, 0, 0,
...
{
  TRect rect = w->GetWindowRect();
  ::MapWindowPoints(nullptr, w->GetParentH(), reinterpret_cast<TPoint*>(&rect), 2); // map to slip coordinates
                          ...
                          TRelPosition position, TDockable* relDockable)
{
  ...
  dockableWindow->DispatchMsg(WM_OWLWINDOWDOCKED, 0, TParam1(GridType == YCoord ? alTop : alLeft),
                                TParam2(static_cast<TDockingSlip*>(this)));
#endif

  // Slam dockable over to 0,0 since Move assumes we own it already
  //
  dockableWindow->SetWindowPos(nullptr, 0, 0, 0, 0,
                               SWP_NOACTIVATE | SWP_NOSIZE | SWP_NOZORDER);
                        ...
                        TRelPosition position, TDockable* relDockable)
{
  ...
  break;
    ...
    if (relWindow)
      ...
      ::MapWindowPoints(nullptr, *this, reinterpret_cast<TPoint*>(&relWindowRect), 2);
  ...
  //
  ::MapWindowPoints(nullptr, *this, &newPos, 1);
  if (newPos.x < 0)
    newPos.x = 0;
  if (newPos.y < 0)
    newPos.y = 0;
  ...
  //
  dockableWindow->SetWindowPos(nullptr, newPos.x, newPos.y, 0, 0,
                               SWP_NOACTIVATE|SWP_NOSIZE|SWP_NOZORDER);
  dockableWindow->ShowWindow(SW_SHOWNA);
        ...
        if (Location == alTop || Location == alBottom) {
          if (draggedRect.left >= currRect.left && draggedRect.left <= currRect.right &&
            ...
            if (currRect.left + midPt > draggedRect.left) {
              ...
              ::MapWindowPoints(nullptr, *this, &pt, 1);
              draggedWindow->SetWindowPos(nullptr, pt.x, pt.y, 0, 0,
                                          SWP_NOACTIVATE|SWP_NOSIZE|SWP_NOZORDER);
              ...
              pt.y = currRect.top;
              ::MapWindowPoints(nullptr, *this, &pt, 1);
              w->SetWindowPos(nullptr, pt.x, pt.y, 0, 0,
                              SWP_NOACTIVATE|SWP_NOSIZE|SWP_NOZORDER);
              return w;
            }
            else {
  ...
  if(windowList.Size()) {
    ...
    if (i == 0 || i != windowList.Size()) {
      ...
      while (i < windowList.Size()) {
        ...
        ::MapWindowPoints(nullptr, *this, &pt, 1);
        windowList[i].DockedWindow->SetWindowPos(nullptr, pt.x, pt.y, 0, 0,
                              SWP_NOACTIVATE|SWP_NOSIZE|SWP_NOZORDER);
                           ...
                           TAbsLocation location, TDockingSlip* dragNotificatee)
{
  ...
  //
  DragHSize = DragDockable->ComputeSize(alTop, nullptr);
  DragVSize = DragDockable->ComputeSize(alLeft, nullptr);
  DragNSize = DragDockable->ComputeSize(alNone, nullptr);
                ...
                TRelPosition position, TDockable* relDockable)
{
  ...
  if (location == alNone) {
    ...
    if (DecFrame.GetHandle()) {
      slip->Create();
      slip->DockableInsert(dockable, where, rpNone, nullptr);
    ...
    if (!slip) {
      ...
      DecFrame.Insert(*slip, static_cast<TDecoratedFrame::TLocation>(location));
