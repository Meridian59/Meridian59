...
TDocManager::~TDocManager()
{
  ...
  //
  TDocument* doc;
  while ((doc = DocList.Next(nullptr)) != nullptr) {
    if (doc->IsOpen())
      doc->Close();
  ...
  for (TDocTemplate* tpl = TemplateList; tpl; tpl = tpl->GetNextTemplate()) {
    if (tpl->IsMyKindOfDoc(doc)) {
      ...
      if (tpl->IsFlagSet(dtSingleView)) {
        TView* pview = 0;
        while ((pview = doc.NextView(pview)) != nullptr)
...
TDocManager::CreateAnyDoc(LPCTSTR path, long flags)
{
  ...
  //
  int tplcount = GetNewTemplates(nullptr, 0, newDoc);
  ...
  //
  return CreateDoc(tpllist[index-1], filepath, nullptr, flags);
}
...
///
/// \sa See \ref docview for valid flags, such as dtAutoDelete and dtNewDoc.
///
...
TDocManager::InitDoc(TDocument* doc, LPCTSTR path, long flags)
{
  ...
  if (flags & dtNewDoc) {
    ...
    if (!doc->InitDoc()) {
      TRACEX(OwlDocView, 0, _T("TDocManager::InitDoc: Initialization failed"));
      PostDocError(*doc, IDS_UNABLEOPEN);
      doc->SetDocPath(nullptr); // fix by suggested Greg Chicares 06/04/98
      delete doc;
      return nullptr;
    }
...
{
  int tplcount = GetSaveTemplates(nullptr, 0, doc, samedoc);
  ...
  while (doc.IsDirty()) {
    ...
    switch (saveOrNot) {
      case IDYES:
        ...
        //
        if (!doc.IsEmbedded() && doc.GetDocPath() == nullptr) {
...
TDocManager::MatchTemplate(LPCTSTR path)
{
  ...
  //
  for (TDocTemplate* tpl = TemplateList; tpl != nullptr; tpl = tpl->GetNextTemplate()) {
...
TDocManager::MatchFilter(LPCTSTR path, LPCTSTR filters)
{
  ...
  while (fltr) {
    ...
    // Try next filter
    //
    fltr = (nxtFltr && *nxtFltr) ? nxtFltr : nullptr;
  }
...
#else
  if (hWnd && ::IsWindow(hWnd)) {
    while ((doc = DocList.Next(doc)) != nullptr ) {
...
TDocManager::AttachTemplate(TDocTemplate& tpl)
{
  ...
  TDocTemplate::AddLink(reinterpret_cast<TRegLink**>(&TemplateList), &tpl);
}
...
TDocManager::DeleteTemplate(TDocTemplate& tpl)
{
  ...
  //
  if (TDocTemplate::RemoveLink(reinterpret_cast<TRegLink**>(&TemplateList), &tpl)) {
    UnRefTemplate(tpl);
    return;
  }
                           ...
                           bool save, TDocument* doc)
{
  ...
  //
  int index, count;
  TDocument* curDoc = save ? (doc ? doc : GetCurrentDoc()) : nullptr;
  CHECK(!save || curDoc);
  if (save) {
  ...
  //--- Sirma (Krasi)
  {
    LPCTSTR fName = path && *path ? path : (doc ? doc->GetTitle() : nullptr);
    if (fName && *fName)
...
{
  if (Mode & dmSDI) {
    TDocument* doc = DocList.Next(nullptr);
    if (doc) {
      if (!doc->CanClose())
        return false;
      if (!doc->Close()) {
...
{
  ce.Enable(TemplateList != nullptr);
}
...
{
  ce.Enable(GetCurrentDoc() != nullptr);
}
