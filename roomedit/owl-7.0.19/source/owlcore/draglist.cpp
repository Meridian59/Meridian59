...
{
  DRAGLISTINFO * info = reinterpret_cast<DRAGLISTINFO *>(lp);
  if (info) {
...
{
  PRECONDITION(GetHandle() && GetParent() && GetParent()->GetHandle());
  ::DrawInsert(GetParent()->GetHandle(), GetHandle(), item);
}
...
TDragList::ItemFromPoint(const TPoint& p, bool scroll)
{
  ...
  return LBItemFromPt(GetHandle(), p, scroll);
}
...
TDragListEventHandler::DragNotify(TParam1 wp, TParam2 lp)
{
  ...
  //
  return ::SendMessage(info->hWnd, ::RegisterWindowMessage(DRAGLISTMSGSTRING), wp, lp);
}
