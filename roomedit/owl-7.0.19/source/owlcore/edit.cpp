...
//
TEdit* TEdit::ValidatorReFocus = nullptr;
...
: TStatic(parent, id, text, x, y, w, h, textLen, module), Validator(nullptr), Requirement(roNone)
{
...
: TStatic(parent, resourceId, textLen, module), Validator(nullptr), Requirement(roNone)
{
  EnableTransfer();
}
...
: TStatic(hWnd, module), Validator(nullptr), Requirement(roNone)
{
  EnableTransfer();
}
...
{
  uint retVal = static_cast<uint>(TStatic::EvGetDlgCode(msg));
  if (!IsValid(false))
  {
    retVal |= DLGC_WANTTAB;
  }
...
TEdit::EvChar(uint key, uint repeatCount, uint flags)
{
  if (Validator && key != VK_BACK) {
    ...
    //
    LPTSTR buff = LockBuffer(std::max(static_cast<int>(TextLimit), std::max(oldBuffLen, buffLen)) + sizeof(tchar) + 1024);
    ...
    SendMessage(EM_SETMODIFY, static_cast<TParam1>(postMsgModify));
  }
  else
...
{
  return static_cast<int>(CONST_CAST(TEdit*,this)->SendMessage(EM_LINELENGTH,
...
TEdit::GetLine(LPTSTR textString, int strSize, int lineNumber) const
{
  ...
  (reinterpret_cast<int16 *>(textString))[0] = int16(strSize);

  int bytesCopied = static_cast<int>(CONST_CAST(TEdit*,this)->
                    SendMessage(EM_GETLINE, lineNumber, TParam2(textString)));
  int n = min(strSize - 1, bytesCopied); // null-terminator index
  textString[n] = 0; // Windows returns non-0 terminated string
  if (n != lineLength)
    return false;
  return success;
}
...
strstrcd(LPCTSTR str1, LPCTSTR str2, bool caseSens)
{
  ...
  LPTSTR p = const_cast<LPTSTR>(str1);
           ...
           bool caseSens)
{
  PRECONDITION(str1 && str2);
  int len2 = static_cast<int>(::_tcslen(str2));
  LPTSTR p = const_cast<LPTSTR>(str1 + len1 - len2);
  ...
  for (;;) {
    ...
    if (wholeWord && pos) {
      ...
      if ((pos > buffer && _istalnum(pos[-1])) || // Match is in preceding word
        (textLen < static_cast<int>(::_tcslen(pos)) && _istalnum(pos[textLen])))
      {
        startPos = static_cast<uint>(pos-buffer) + !up;
        continue;  // Skip this match and keep searching
      }
#endif
    }
...
TEdit::IsValid(bool reportError)
{
  ...
  if (Validator || IsRequired())
  {
    ...
    {
      bool Dismissed;
      ~TRefocusGuard() { if (!Dismissed) ValidatorReFocus = nullptr; }
    }
    reFocusGuard = {false};
