...
{
  FileName = fileName ? strnewdup(fileName) : nullptr;
}
...
TEditFile::TEditFile(
  ...
  : TEditSearch(parent, id, text, x, y, w, h, module),
  FileName (fileName.empty() ? nullptr : strnewdup(fileName.c_str()))
{}
...
TEditFile::SetupWindow()
{
  ...
  if (FileName && !Read())
  {
    FormatMessageBox(LoadString(IDS_UNABLEREAD), _T(""), MB_ICONEXCLAMATION | MB_OK, FileName);
    SetFileName(nullptr);
  }
}
...
TEditFile::SetFileName(LPCTSTR fileName)
{
  ...
  SetDocTitle(FileName ? FileName : untitled.c_str(), 0);
}
...
TEditFile::Save()
{
  if (IsModified())
  {
    ...
    if (!Write())
    {
      FormatMessageBox(LoadString(IDS_UNABLEWRITE), _T(""), MB_ICONEXCLAMATION | MB_OK, FileName);
      return false;
    }
  }
...
TEditFile::CanClear()
{
  if (IsModified())
  {
    ...
    int result = FormatMessageBox(LoadString(IDS_FILECHANGED), _T(""), MB_YESNOCANCEL|MB_ICONQUESTION,
      FileName ? FileName : untitled.c_str());
