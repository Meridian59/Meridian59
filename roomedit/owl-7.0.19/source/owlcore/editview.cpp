...
:
  TEditSearch(parent, GetNextViewId(), nullptr),
  TView(doc),
  Origin(0)
{
...
TEditView::LoadData()
{
  unique_ptr<tistream> inStream (Doc->InStream(ios::in | ios::binary));
  if (!inStream.get())
  {
...
TEditView::VnCommit(bool force)
{
  ...
  unique_ptr<tostream> outStream(Doc->OutStream(ios::out | ios::binary));
  if (!outStream.get())
  {
    Doc->PostError(IDS_UNABLEOPEN, MB_OK);
    return false;
