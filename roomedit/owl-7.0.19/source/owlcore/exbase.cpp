...
namespace {
  ...
  {return WideCharToMultiByte(CP_UTF8, dwFlags, lpWideCharStr, cchWideChar, lpMultiByteStr, cbMultiByte, nullptr, nullptr);}
  ...
  Result convert(const Source& source, Converter* const converter, const DWORD flags = 0)
  {
    ...
    if (!source.empty())
    {
      ...
      int n = (*converter)(flags, &source[0], -1, nullptr, 0);
      CHECK(n >= 0);
...
const char* TXBase::what() const noexcept
{
  return str.c_str ();
}
