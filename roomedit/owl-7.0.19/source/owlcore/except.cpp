...
HandleGlobalException(TXBase& x, LPCTSTR caption, LPCTSTR canResume)
{
  ...
  const int r = ::MessageBox(nullptr, msg.c_str(), caption ? caption : defCaption, flags);
  return (r == IDYES) ? 0 : -1;
}
...
:
  TXBase(ResourceIdToString(nullptr, resId, module)),
  ResId(resId)
{
}
...
{
  tstring rscMsg = ResourceIdToString(nullptr, resId, module);
  tchar buf[255];
