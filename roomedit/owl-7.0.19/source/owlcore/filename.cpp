...
tstring __formServer(const tchar* serverName)
{
  ...
  return tstring(serverName, 0, static_cast<int>(ds - serverName));
}
...
tstring __formDevice(const tchar* deviceName, bool isUNC)
{
  ...
  return tstring(deviceName, 0, static_cast<int>(deviceNameEnd - deviceName));
}
...
tstring __formPath(const tchar* path)
{
  ...
  return tstring(path, 0, static_cast<int>(pathEnd - path));
}
...
tstring __formFile(const tchar* file)
{
  ...
  return tstring(file, 0, static_cast<int>(fileEnd - file));
}
...
TFileName::Parse(LPCTSTR freeform, bool pathOnly)
{
  ...
  if (nextp >= freeform+serverNamePrefixLen) {
    ...
    ServerStr = tstring(freeform, 0, static_cast<int>(nextp-freeform));
    freeform = nextp + 1;               // skip past "\"

    nextp = fn_find(freeform, freeformEnd, dirSeparator);
    DeviceStr = tstring(freeform, 0, static_cast<int>(nextp-freeform));
    freeform = nextp + 1;               // skip past "\"
  }
  else {
    Unc = false;
  ...
  //    report an erroneous error here
    ...
    if (nextp > dirp) {
      ExtStr = tstring(nextp, 0, static_cast<int>(freeformEnd-nextp));
      freeformEnd = nextp;
    }
    else
      ExtStr = _T("");

    FileStr = tstring(dirp+1, 0, static_cast<int>(freeformEnd - (dirp+1)));
    freeformEnd = dirp;
  }
  else{
    FileStr = _T("");
  ...
  if (freeformEnd > freeform)
    PathStr = tstring(freeform, 0, static_cast<int>(freeformEnd-freeform));
  else
    PathStr = _T("");
  FormCanonicalName();
}
...
TFileName::TFileName(TFileName::TSpecialType type)
{
  switch (type) {
    case TempFile:
    case TempDir: {
      int tmpLen = ::GetTempPath(0,nullptr);
      ...
      if (type == TempDir) {
        ...
        ::GetTempFileName(static_cast<tchar*>(tmpPath), _T("TFN"), 0, tmpFile);
        Parse(static_cast<tchar*>(tmpFile));
      }
    }
    break;
    case CurrentDir: {
      TTmpBuffer<tchar> buffer(WIN95_LONGFILENAME_MAX);
      ::GetCurrentDirectory(WIN95_LONGFILENAME_MAX, static_cast<tchar*>(buffer));
      _tcscat(static_cast<tchar*>(buffer), dirSeparatorStr);
      Parse((tchar*)buffer,true);
    }
    break;
    case HomeDir: {
      TTmpBuffer<tchar> buffer(WIN95_LONGFILENAME_MAX);
      ::GetWindowsDirectory(static_cast<tchar*>(buffer), WIN95_LONGFILENAME_MAX);
      _tcscat(static_cast<tchar*>(buffer),dirSeparatorStr);
      Parse(static_cast<tchar*>(buffer),true);
    }
    break;
    case BootDir:{
...
{
  return const_cast<tchar*>(CanonicalStr());
}
...
//  else
  ...
  if (_tcscmp(NameBuffer,_T(""))) //if file name is not blank
    ...
    tchar buffer[WIN95_LONGFILENAME_MAX];
    ::GetFullPathName(NameBuffer, WIN95_LONGFILENAME_MAX, buffer, nullptr);
...
{
  return const_cast<tchar*>(GetParts(File));
}

tchar* TFileName::GetNameAndExt() const
{
  return const_cast<tchar*>(GetParts(File|Ext));
}

tchar* TFileName::GetFullFileNameNoExt()
{
  return const_cast<tchar*>(GetParts(Server|Device|Path|File)); //fills NameBuffer
}
...
TFileName::ShortName(bool forceUNC) const
{
  ...
  return static_cast<tchar*>(NameBuffer);
}
...
const tchar* TFileName::Title() const
{
  ...
  tchar tmp[WIN95_LONGFILENAME_MAX];
  if (GetFileTitle(canonical.c_str(), tmp, COUNTOF(tmp)) == 0)
    _tcscpy(NameBuffer,tmp);
  else
    NameBuffer[0] = _T('\0'); 
  return static_cast<tchar*>(NameBuffer);
...
static tstring& trimPath(tstring& s, uint len)
{
  ...
  tstring subpath = s;
  const auto pos = subpath.find_last_of(dirSeparatorStr);
  if (pos == subpath.npos || len <= 5)
  {
    s = addPath;
    return s;
  }
...
{
  return ::CopyFile(CanonicalStr(), newName.CanonicalStr(), failIfExists) != FALSE;
}
...
{
  return ::CreateDirectory(CanonicalStr(), 0) != FALSE;  //LPSECURITY_ATTRIBUTES
}
...
{
  return ::SetCurrentDirectory(CanonicalStr()) != FALSE;
}

TFileName&
TFileName::AddSubDir(const tstring& subdir)
{
  if (_tcschr(dirSeparator, subdir[0]) == nullptr) // The subdir does not begin with the separator
    PathStr += dirSeparatorStr;

  PathStr += subdir;

  if (_tcschr(dirSeparator, PathStr[PathStr.length() - 1]) != nullptr)  // The resultring string ends with the separator
  ...
  Done(false)
{
  ...
  Data = data;
  tchar* p = const_cast<tchar*>(_tcsrchr(wildName.c_str(),_T('\\')));
  if(p){
    tchar c = *p;
    *p = _T('\0');
...
{
  if (Data && (static_cast<__TFindFileStr*>(Data))->Handle != INVALID_HANDLE_VALUE)
    ::FindClose((static_cast<__TFindFileStr*>(Data))->Handle);

  delete static_cast<__TFindFileStr*>(Data);
}
//------------------------------------------------------------------------------
...
{
  __TFindFileStr& data = *static_cast<__TFindFileStr*>(findData);
