...
TFindReplaceDialog::Init(TResId templateId)
{
  ...
  fr.hwndOwner = GetParentO() ? GetParentO()->GetHandle() : nullptr;
  ...
  fr.wFindWhatLen = static_cast<uint16>(Data.BuffSize);
  fr.lpstrReplaceWith = Data.ReplaceWith;
  fr.wReplaceWithLen = static_cast<uint16>(Data.BuffSize);
}
...
//
TWindow::THandle
TFindReplaceDialog::PerformCreate()
{
  PRECONDITIONX(false, _T("Incorrect use of base class."));
  return nullptr;
}
...
{
  if (param2)
    Data.Flags = (reinterpret_cast<LPFINDREPLACE>(param2))->Flags;
  else
    Data.Flags = fr.Flags;
}
