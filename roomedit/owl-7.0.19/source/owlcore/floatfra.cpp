...
{
  TRACEX(OwlCmd, 1, "TFloatingFrame::GetCommandTarget - returns "
    << (GetParentO() ? static_cast<void*>(GetParentO()->GetHandle()) : 0));
  return GetParentO() ? HWND(*GetParentO()) : HWND(nullptr);
}
...
void TFloatingFrame::EvNCPaint(HRGN)
{
  ...
  //
  if (DragFrame) {
    TWindowDC dc(*static_cast<TWindow*>(this));  // Cast is an MSVC bug workaround  Jogy: Is the workaround still needed?
