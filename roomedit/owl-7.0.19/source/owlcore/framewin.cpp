...
TFrameWindow::SetIcon(TModule* module, TResId resId)
{
  ...
  HINSTANCE hInstance = IconModule ? HINSTANCE(*IconModule) : HINSTANCE(nullptr);

  if (IconResId != 0)
    CurIcon = TUser::LoadIcon(hInstance, IconResId);

  if (CurIcon && IsWindow())
    SendMessage(WM_SETICON, true, reinterpret_cast<LPARAM>(static_cast<HICON>(CurIcon)));
...
TFrameWindow::SetIconSm(TModule* module, TResId resId)
{
  ...
  HINSTANCE hInstance = IconSmModule ? HINSTANCE(*IconSmModule) : HINSTANCE(nullptr);
  if (IconSmResId != 0) {
    CurIconSm = reinterpret_cast<HICON>(::LoadImage(hInstance, IconSmResId.GetPointerRepresentation(), IMAGE_ICON,
  ...
  if (CurIconSm && IsWindow())
    SendMessage(WM_SETICON, false, reinterpret_cast<LPARAM>(static_cast<HICON>(CurIconSm)));
...
TFrameWindow::SetClientWindow(TWindow* clientWnd)
{
  ...
  HWND oldWnd = oldClientWnd ? oldClientWnd->GetHandle() : static_cast<HWND>(nullptr);
...
getAppIcon()
{
  ...
  return hRealAppIcon ? hRealAppIcon : ::LoadIcon(nullptr, IDI_APPLICATION);
}
...
// !BB Please, leave this code AS IS?
  ...
  if (CurIcon)
    SendMessage(WM_SETICON, true, reinterpret_cast<LPARAM>(CurIcon));

  if (CurIconSm)
    SendMessage(WM_SETICON, false, reinterpret_cast<LPARAM>(CurIconSm));
}
...
{
  if (&menuDescr != MenuDescr) // Guard against self-assignment (the latter is useful for refreshing the menu).
  {
    delete MenuDescr;
