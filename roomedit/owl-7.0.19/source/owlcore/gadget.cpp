  ...
  if (BorderStyle != None) {
    ...
    if (BorderStyle == Plain) {
      ...
      if(Window->GetFlatStyle()&TGadgetWindow::FlatStandard){
        TUIBorder(boundsRect, static_cast<uint>(BorderStyle) == static_cast<uint>(TUIBorder::Recessed) ?
...
TGadget::MouseLeave(uint /*modKeys*/, const TPoint&)
{
  ...
  if (tooltip && tooltip->IsWindow()) {
    ...
    msg.wParam  = 0;
    msg.lParam  = MkUint32(static_cast<uint16>(crsPoint.x), static_cast<uint16>(crsPoint.y));
...
TSeparatorGadget::Paint(TDC& dc)
{
  ...
  if (flat && ShowSeparator)
  {
    ...
    if (xpstyle)
    {
      ...
      TThemePart part(GetGadgetWindow()->GetHandle(), static_cast<LPCWSTR>(L"TOOLBAR"), part_id, TS_NORMAL);
