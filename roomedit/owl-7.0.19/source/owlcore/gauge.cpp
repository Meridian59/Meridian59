...
//
auto TGauge::GetWindowClassName() -> TWindowClassName
{
  return TWindowClassName{_T("OWL_Gauge")};
}
...
{
  TWindow::SetBkgndColor(color != TColor{CLR_DEFAULT} ? color : TColor::None);
}
...
TGauge::Paint(TDC& dc, bool /*erase*/, TRect&)
{
  ...
  if (LedSpacing) {
    ...
    if (c && *c) {
      std::vector<tchar> buff(::_tcslen(c) + 20);
      HRESULT r = StringCbPrintf(&buff[0], buff.size(), c, Value);
      CHECK(r == S_OK); InUse(r);

      int   len = static_cast<int>(::_tcslen(&buff[0]));
      ...
      dc.ExtTextOut(x, y, ETO_CLIPPED, &barRect, &buff[0], len);
      dc.SetTextColor(ledcolor);
      dc.ExtTextOut(x, y, ETO_CLIPPED, &emptyRect, &buff[0], len);
    }
  }
}
