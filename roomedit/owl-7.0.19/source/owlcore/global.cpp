...
{
  return (version&CompatibleVersionMask) == (OWLGetVersion()&CompatibleVersionMask) ? &DocTemplateStaticHead : nullptr;
}
TModule** PASCAL __declspec(dllexport)
GetModulePtr(uint32 version)
{
  return (version&CompatibleVersionMask) == (OWLGetVersion()&CompatibleVersionMask) ? &owl::Module : nullptr;
}
}
