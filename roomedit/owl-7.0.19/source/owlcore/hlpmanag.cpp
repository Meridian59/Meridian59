  ...
  if(UseHTMLHelp){
    if (app)
      HelpState = ToBool(HtmlHelp(app->GetMainWindow(), GetHelpFile().c_str(),
                                  helpCmd, helpFileContextId) != nullptr);
    else
      HelpState = ToBool(HtmlHelp(nullptr, GetHelpFile().c_str(),
                                  helpCmd, helpFileContextId) != nullptr);
  }
  else{
    if (app)
      ...
      HelpState = ToBool(::WinHelp(nullptr, GetHelpFile().c_str(),
                         helpCmd, helpFileContextId));
  }
}
...
THelpFileManager::DeactivateHelp()
{
  ...
  if(UseHTMLHelp){
    if (app)
      ...
      HtmlHelp(nullptr, GetHelpFile().c_str(), HELP_QUIT, 0);
  }
  else{
    if (app)
      app->GetMainWindow()->WinHelp(GetHelpFile().c_str(), HELP_QUIT, 0);
    else
      ::WinHelp(nullptr, GetHelpFile().c_str(), HELP_QUIT, 0);
  }
}
...
//
HWND
THelpFileManager::HtmlHelp(TWindow* wnd, LPCTSTR lpszHelp, uint uCmd,  DWORD_PTR data)
{
      ...
      case HELP_CONTEXTPOPUP: {
          ...
          if(THtmlHelp::IsAvailable()){
                  ...
                  else
                  {
                    ...
                    hPop.pt               = LastHit;
                    hPop.idString         = static_cast<UINT>(data);
                    hPop.pszText          = nullptr;
                    hPop.pszFont          = nullptr;

                    const HWND w = wnd ? wnd->GetHandle() : nullptr;
                    return THtmlHelp::Dll()->HtmlHelp(w, lpszHelp, command, reinterpret_cast<DWORD_PTR>(&hPop));
                  }
  ...
  if(THtmlHelp::IsAvailable()){
    if(wnd)
      ...
      return THtmlHelp::Dll()->HtmlHelp(nullptr, lpszHelp,command,data);
  }
  return nullptr;
}
...
THelpFileManager::CmWhatIsThis()
{
  ...
  if (hWnd == nullptr || hWnd == LastParent->GetHandle()){
    Info.iContextType = HELPINFO_MENUITEM;
    Info.iCtrlId      = IDHELP;
...
THelpFileManager::EvHelp(const HELPINFO& hi)
{
  ...
  if (hi.iContextType == HELPINFO_MENUITEM){
    ...
    success = GetHelpContextFromControl(context, hi.iCtrlId, static_cast<HWND>(hi.hItemHandle));
      ...
      case WM_KEYDOWN :
        if (msg.wParam == VK_F1) {
          ...
          if (ContextHelp && (msg.wParam == VK_ESCAPE)) {
            ...
            TApplication* app = TYPESAFE_DOWNCAST(this, TApplication);
            if (app)
              app->GetMainWindow()->SetCursor(nullptr, IDC_ARROW); //?????????????????
            return true;    // Gobble up the message.
          }
        }
        break;
                                            ...
                                            int controlId, HWND hwnd) const
{
  ...
  for(TContextListIterator iterator(*ContextTable); iterator; iterator++){
    ...
    if (IsIndirectHelpContext(context)) {
      THelpContext* indirect = reinterpret_cast<THelpContext*>(context.GetWindow());
...
{
  TWindow* targetWindow = static_cast<TWindow*>(ptr);
