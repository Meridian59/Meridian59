...
inline tostream& operator <<(tostream& os, const TFixedPoint& p){
  return os <<  static_cast<int>(const_cast<TFixedPoint&>(p));
}
inline tostream& operator <<(tostream& os, const TLayoutConstraint& c){
  return os <<  _T('(') << static_cast<void*>(c.RelWin) << dec
  << _T(',') << c.MyEdge << _T(',') << static_cast<int>(c.Relationship) << _T(',') << static_cast<int>(c.Units)
...
struct TVariable {
  ...
  TVariable() {Value = 0; DeterminedBy = nullptr; Resolved = false;}
};
...
TLayoutWindow::~TLayoutWindow()
{
  ...
  ChildMetrics = nullptr;  //Bug fix: https://sourceforge.net/forum/message.php?msg_id=4186646
  ...
  Constraints = nullptr;  //Bug fix: https://sourceforge.net/forum/message.php?msg_id=4186646
}
...
Simplify(TConstraint* simplify, TVariable* output, TConstraint* _using)
{
  ...
  if (outputOfSimplify != -1 && target != -1) {
    ...
    simplify->Inputs[target] = nullptr;  // input has been back substituted out
                             ...
                             TWhichConstraint   whichConstraint)
{
  ...
  //
  if ((static_cast<int>(c->Relationship) != lmAbsolute) && (static_cast<int>(c->Relationship) != lmAsIs)) {
    TVariable*  variables;
...
{
  Inputs[0] = Inputs[1] = Inputs[2] = nullptr;
...
TLayoutMetrics::TLayoutMetrics()
{
  ...
  Width.RelWin = nullptr;
  Width.MyEdge = Width.OtherEdge = lmWidth;
  Width.Relationship = lmAsIs;
  Width.Units = lmLayoutUnits;
  Width.Value = 0;

  Height.RelWin = nullptr;
