...
TListBox::Transfer(void* buffer, TTransferDirection direction)
{
  ...
  long           style = GetStyle();
  TListBoxData*  listBoxData = static_cast<TListBoxData*>(buffer);
    ...
    for (uint i = 0; i < listBoxData->GetStrings().Size(); i++) {
      ...
      if (style & MULTIPLESEL) {
        for (uint j = 0; j < selCount; j++)
          if (listBoxData->GetSelIndices()[j] == static_cast<int>(i)) {
            SetSel(index, true);
            break;
          }
      }
      else {
        if (selCount && static_cast<uint>(listBoxData->GetSelIndices()[0]) == i)
          selIndex = index;
        else
...
TListBox::GetSelCount() const
{
  ...
  //
  return static_cast<int>(CONST_CAST(TListBox*,this)->SendMessage(LB_GETSELCOUNT));
}
...
{
  if (!(GetStyle() & MULTIPLESEL))
    return static_cast<int>(SendMessage(LB_SELECTSTRING, startIndex, TParam2(findStr)));
  return -1;
}
...
{
  if (!(GetStyle() & MULTIPLESEL))
    return static_cast<int>(CONST_CAST(TListBox*,this)->SendMessage(LB_GETCURSEL));
  return -1;
}
...
TListBox::GetSelIndexes(int* indexes, int maxCount) const
{
  if (!(GetStyle() & MULTIPLESEL))
    return -1;
  return static_cast<int>(CONST_CAST(TListBox*,this)->SendMessage(LB_GETSELITEMS,
                                                        maxCount,
                                                        TParam2(indexes)));
}
...
{
  if (!(GetStyle() & MULTIPLESEL))
    return static_cast<int>(SendMessage(LB_SETCURSEL, index));
  return -1;
}
...
TListBox::SetSelIndexes(int* indexes, int numSelections, bool shouldSet)
{
  ...
  if (numSelections < 0)
    return static_cast<int>(SendMessage(LB_SETSEL, shouldSet, -1));
