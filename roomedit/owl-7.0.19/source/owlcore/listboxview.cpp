...
TListBoxView::LoadData(int top, int sel)
{
  ...
  tistream* inStream;
  if ((inStream = Doc->InStream(ios::in)) == nullptr) {
...
TListBoxView::VnCommit(bool force)
{
  ...
  for (int index = 0; index < count-1; index++) {  // don't write last virtual line
    ...
    GetString(static_cast<tchar*>(buf), index);
    *outStream << static_cast<tchar*>(buf) << _T('\n');
    //delete buf;
  }
...
{
  if (!clear && Doc->GetDocPath() != nullptr)
    return LoadData(0,0);
  CmEditClear();
  DirtyFlag = false;
  return true;
...
{
  uint retVal = static_cast<uint>(DefaultProcessing());
