...
{
  PRECONDITION(owner);
  owner->MessageBox(owner->LoadString(IDS_VALNOTINLIST), owner->LoadString(IDS_VALCAPTION), MB_ICONERROR | MB_OK);
}
...
{
  if (Strings)
    return Strings->Find(tstring{str}) != Strings->NPOS;
  return false;
}
...
TStringLookupValidator::Adjust(tstring& text, int& /*begPos*/, int& /*endPos*/, int amount)
{
  ...
  const auto index = Strings->Find(text);
  if (index == Strings->NPOS)
    return 0;
