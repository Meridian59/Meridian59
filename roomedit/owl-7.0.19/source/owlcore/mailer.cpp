                       ...
                       LPCTSTR docNames, bool asynchWork)
{
  ...
  if (!docNames) {
    ...
    *tmpDocNames = 0;
    ...
    while (nameToken) {
      ...
      GetFileTitle(nameToken, tmpName, _MAX_PATH);
  ...
  //
  _USES_CONVERSION;
  const auto docPathsNarrow = const_cast<LPSTR>(_W2A(docPaths));
  const auto docNamesNarrow = const_cast<LPSTR>(_W2A(docNames));
