...
TMDIChild::EnableWindow(bool enable)
{
  ...
  {
    if (!enable && reinterpret_cast<THandle>(GetParent()->HandleMessage(WM_MDIGETACTIVE)) == *this)
      GetParent()->HandleMessage(WM_MDINEXT, TParam1(GetHandle()));
    GetParent()->HandleMessage(WM_MDIREFRESHMENU);
  }
  return retVal;
}
...
TMDIChild::Destroy(int)
{
  if (GetHandle())
  {
    ...
    if (GetParentO()) {
      ...
      SetHandle(nullptr);  // Assume success
    }
    else {
