...
//
TMDIFrame::TMDIFrame(LPCTSTR title, TResId menuResId, std::unique_ptr<TMDIClient> clientWnd, TModule* module)
{
  auto c = clientWnd ? move(clientWnd) : std::make_unique<TMDIClient>();

  // Initialize virtual bases.
  //
  TWindow::Init(nullptr, title, module);
  TFrameWindow::Init(c.get(), false);
  c.release(); // Base took ownership.
...
//
TMDIFrame::TMDIFrame(const tstring& title, TResId menuResId, std::unique_ptr<TMDIClient> clientWnd, TModule* module)
  : TMDIFrame{title.c_str(), menuResId, move(clientWnd), module}
{}
...
//
TMDIFrame::TMDIFrame(HWND handle, HWND clientHandle, TModule* module)
:
...
///
//
auto TMDIFrame::PerformCreate() -> THandle
{
  ...
  struct TMenuGuard
  {
    ...
    HMENU RelinquishOwnership() { HMENU tmp = m; m = nullptr; return tmp; }
  }
...
TMDIFrame::FindChildMenu(HMENU menu)
{
  if (menu) {
    ...
    for (int i = numItems-1; i >= 0; i--) {
      ...
      if (childMenu &&
        (TUser::GetMenuState(childMenu, CM_CASCADECHILDREN, MF_BYCOMMAND) != static_cast<uint>(-1) ||
        TUser::GetMenuState(childMenu, CM_TILECHILDREN, MF_BYCOMMAND) != static_cast<uint>(-1) ||
        TUser::GetMenuState(childMenu, CM_ARRANGEICONS, MF_BYCOMMAND) != static_cast<uint>(-1)))
      {
        return childMenu;
      }
    }
...
//
auto TMDIFrame::GetCommandTarget() -> HWND
{
...
{
  return ::DefFrameProc(GetHandle(), ClientWnd ? ClientWnd->GetHandle() : nullptr, message, param1, param2);
}
