...
{
  const auto config = unique_ptr<TConfigFile>(CreateConfigFile());
  tstring sec_name = GetSectionName() + Name;
  TConfigFileSection section(*config, sec_name.c_str());

  auto text = GetText();
  if (!text.empty())
  {
    AddToList(data(text)); // Unfortunately, AddToList is virtual and not const-correct.
    section.WriteString(valueName, text);
  }
  else
    section.EraseEntry(valueName); // If the combo box text is empty, clear the old value
  ...
  for (auto i = 0; i != count; ++i)
    if (const auto s = GetString(i); !s.empty())
      section.WriteString(valueName + to_tstring(i), s);
}
//
TConfigFile*
