...
strnewdup(const wchar_t* s, size_t allocSize)
{
  if (!s)
    s = L"";
  size_t alloc = std::max(static_cast<size_t>(::wcslen(s))+1, allocSize);
...
TTmpBufferBase::TTmpBufferBase(int size)
{
  ...
  int index;
  for(index = 0; index < static_cast<int>(COUNTOF(szArray)); index++){
    if(szArray[index] > size)
      break;
  }
  if(index < static_cast<int>(COUNTOF(szArray))){
...
void* TTmpBufferBase::operator new(size_t /*sz*/) noexcept
{
 return nullptr;
}
