...
:
  TGdiBase(::CopyMetaFile(src, fileName.empty() ? nullptr : fileName.c_str()), AutoDelete),
  Extent(src.Extent),
  Placeable(false)
{
  CheckValid();
...
TEnhMetaFilePict::TEnhMetaFilePict(const TEnhMetaFilePict& metafilepict, const tstring& fileName)
  : TGdiBase(::CopyEnhMetaFile(metafilepict, fileName.empty() ? nullptr : fileName.c_str()), AutoDelete)
{}
