...
int TSystemMessage::MessageBox(TWindow* win, const tstring& msg, const tstring& title, uint flags) const
{
  ...
  message += Message;
  TApplication* appl;
  if(!win && (appl = OWLGetAppDictionary().GetApplication(0))!=nullptr && appl->GetMainWindow())
    win = appl->GetMainWindow();
  if(win)
    return win->MessageBox(message.c_str(), title, flags);
  return ::MessageBox(nullptr, message.c_str(), title.c_str(), flags);
}
...
void TSystemMessage::Init(TLangId langId)
{
  ...
  if(::FormatMessage( FORMAT_MESSAGE_ALLOCATE_BUFFER | FORMAT_MESSAGE_FROM_SYSTEM,
                nullptr, Error, langId, (LPTSTR)&ptr, 0, nullptr) != 0)
  {
    Message = (LPTSTR)ptr;
...
TPtrArray<TModule*>* __TModuleList::ModuleArray = nullptr;
#if defined(BI_MULTI_THREAD_RTL)
TMRSWSection*  __TModuleList::Lock = nullptr;
#endif
  ...
  if(!module){
    ...
    for(uint i = 0; i < ModuleArray->Size(); i++){
      if(module == (*ModuleArray)[i])
        return (i+1 < ModuleArray->Size()) ? (*ModuleArray)[i+1] : nullptr;
    }
  }
  return nullptr;
}
...
void
TModule::InitModule(THandle hInstance, [[maybe_unused]] const tstring& cmdLine)
{
  SetHandle(hInstance);
...
TModule::Error(TXBase& x, unsigned captionResId, unsigned promptResId)
{
  ...
  return HandleGlobalException(x,
    LoadString(captionResId, cbuf, 80) ? cbuf : nullptr,
    promptResId ?
...
TModule::LoadString(uint id, LPTSTR buf, int bufSize) const
{
  ...
  if(resHdl != nullptr){
    if((glbHdl = LoadResource(resHdl)) != nullptr) {
      if ((resData = (TResText)LockResource(glbHdl)) != nullptr) {
...
#  else
          len = ::WideCharToMultiByte(CP_ACP, 0, resData, len, buf, bufSize, nullptr, nullptr);
          if (len > 0)
            buf[len] = 0;
#  endif
        }
   ...
   && (glbHdl = LoadResource(resHdl)) != 0) {
    if ((resData = (TResText)LockResource(glbHdl)) != 0) {
      ...
      if (len != 0) {
        ...
        uint i;
        for (i = 0; i < len; i++)
          retString[i] = static_cast<char>(buf[i]);
        retString[i] = 0;
  #else
...
#    else
          int n = ::WideCharToMultiByte(CP_ACP, 0, resData, len, nullptr, 0, nullptr, nullptr);
          TAPointer<tchar> __ClnObj(new tchar[n+1]);
          tchar* buf = __ClnObj;
          len = ::WideCharToMultiByte(CP_ACP, 0, resData, len, buf, n+1, nullptr, nullptr);
          // The string object does not need the terminating null of C-strings
          retString.resize(len);
          int i;
          for (i = 0; i < len; i++)
...
static char __initOWLS = static_cast<char>(__initOWL(), atexit(&__termOWL));
