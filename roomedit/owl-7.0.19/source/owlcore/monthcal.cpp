...
auto TMonthCalendar::GetWindowClassName() -> TWindowClassName
{
  return TWindowClassName{MONTHCAL_CLASS};
}
...
TMonthCalendar::GetMonthRange(TSystemTime& minm, TSystemTime& maxm, uint32 flags) const
{
  ...
  int count = static_cast<int>(const_cast<TMonthCalendar*>(this)->SendMessage(MCM_GETMONTHRANGE,
...
TMonthCalendar::GetRange(TSystemTime& minm, TSystemTime& maxm) const
{
  ...
  uint32 ranges = static_cast<uint32>(const_cast<TMonthCalendar*>(this)->SendMessage(MCM_GETRANGE, 0, TParam2(sysTimes)));
...
TMonthCalendar::GetSelRange(TSystemTime& minm, TSystemTime& maxm) const
{
  ...
  SYSTEMTIME sysTimes[2];
  bool retval = ToBool((const_cast<TMonthCalendar*>(this))->SendMessage(MCM_GETSELRANGE, 0, TParam2(sysTimes)));
  if (retval){
    minm = sysTimes[0];
    maxm = sysTimes[1];
  }
