...
DIAG_DEFINE_GROUP_INIT(OWL_INI, Objstrm, 1, 0);
...
namespace
{
  ...
  TStreamableTypes& GetTypes_()
  {
    static TStreamableTypes types; // Synchronised in C++11.
    return types;
  }

  //
  // Support for multi-threading
  // NOTE: This support is incomplete, untested and probably unreliable.
  //

#if defined(BI_MULTI_THREAD_RTL)

  TMRSWSection& GetLock_()
  {
    static TMRSWSection lock; // Synchronised in C++11.
    return lock;
  }

#define LOCK_TYPES_(s) TMRSWSection::TLock lock__(GetLock_(), s, false)
...
//
// Uses a std::set for faster insertion and lookup. The Borland sorted arrays
  ...
  ModId(id)
{
  ...
  if (id != 0) // id == 0 is used only during lookup. It flags an instance that shouldn't be registered.
    GetTypes_().RegisterType(id, *this);
}

TStreamableClass::~TStreamableClass()
{
  if (ModId != 0)
    GetTypes_().UnRegisterType(ModId, *this);
...
{
  return Data->Find(TPWObj(((tchar*)(void*)d)+1,0));
}
...
/// or -) starting at ios::seekdir. You can set ios::seekdir to one of the
/// following:
...
ipstream& ipstream::seekg( streamoff off, ios::seekdir dir )
{
  if( good() ){
    objs.RemoveAll();
#if __GNUC__ >=3
    streampos p = bp->pubseekoff(off, dir, ios::in );
#else
    streampos p = bp->pubseekoff((ios::off_type)off, dir, (ios::openmode)ios::in );
#endif
    if( p == streampos(EOF) )
      clear( ios::failbit );
  }
...
const ObjectBuilder *ipstream::readPrefix( ModuleId mid )
{
  ...
  const ObjectBuilder *res = GetTypes_().Lookup(mid, name);
...
/// or -) starting at ios::seekdir. You can set ios::seekdir to one if the
/// following:
/// - \c \b  beg (start of stream)
/// - \c \b  cur (current stream position)
/// - \c \b  end (end of stream).
opstream& opstream::seekp( streamoff off, ios::seekdir dir )
{
  if( good() ){
    objs->RemoveAll();
#if __GNUC__ >=3
    streampos p = bp->pubseekoff(off, dir, ios::out );
#else
...
void opstream::writeData( const TStreamableBase *t, ModuleId mid )
{
  ...
  if( good() ){
    ...
    const ObjectBuilder *res = GetTypes_().Lookup(mid, _OBJ_TYPENAME(t));
