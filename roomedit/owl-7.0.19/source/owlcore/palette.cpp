...
TPalette::TPalette(const TPalette& src)
{
  ...
  if (nColors) {
    LOGPALETTE* logPal = reinterpret_cast<LOGPALETTE*>( new
    ...
    Handle = nullptr;  // Force a failure
...
{
  LOGPALETTE* logPal = reinterpret_cast<LOGPALETTE*>(new uint8[
     sizeof(LOGPALETTE)+(count-1)*sizeof(PALETTEENTRY) ]);

  logPal->palVersion  = 0x300;
  logPal->palNumEntries = static_cast<uint16>(count);
...
TPalette::Create(const BITMAPINFO * info, uint flags)
{
  ...
  if (nColors) {
    LOGPALETTE* logPal = reinterpret_cast<LOGPALETTE*>(
    ...
    for (uint16 n = 0; n < nColors; n++) {
      ...
      logPal->palPalEntry[n].peFlags = static_cast<uint8>(flags);
    }
    Handle = ::CreatePalette(logPal);
    delete[] logPal;
  }
...
TPalette::Read_PAL(TRiffFile& file)
{
  ...
  if(!file.Descent(ckFile, nullptr, TRiffFile::ffFindRiff)) {
  ...
  int iSize = static_cast<int>(ckChunk.Size);
...
TPalette::Write_PAL(TRiffFile& file)
{
  ...
  TAPointer<char> pdata(new char[iSize]);
  LOGPALETTE* logPal = reinterpret_cast<LOGPALETTE*>((char*)pdata);
  logPal->palVersion = 0x300;
  logPal->palNumEntries = nColors;
...
bool TPalette::Write_ACO(TRiffFile& file)
{
  ...
  for(int j = 0; j < nColors; j++){
    if(!file.WriteStruct(reinterpret_cast<char*>(&clr[j]), ACO_ColorFields, boBig_Endian)){
...
bool TPalette::Write_ACT(TRiffFile& file)
{
  ...
  // Write the data
  if(!file.Write(reinterpret_cast<char*>(&clr), sizeof(clr))) {
...
bool TPalette::Read_BMP(TRiffFile& file)
{
  ...
  uint32 headerSize;
  if (file.Read(reinterpret_cast<char*>(&headerSize), sizeof headerSize) == TFILE_ERROR ||
  ...
  if (isCore) {
    ...
    if (file.Read(reinterpret_cast<char*>(&coreHeader.bcWidth),
        static_cast<int>(headerSize)-sizeof(uint32))==TFILE_ERROR) {
  ...
  int colorRead = isCore ?          // Color table size on disk
        static_cast<int>(ClrUsed) * sizeof(RGBTRIPLE) :
        static_cast<int>(ClrUsed) * sizeof(RGBQUAD);
