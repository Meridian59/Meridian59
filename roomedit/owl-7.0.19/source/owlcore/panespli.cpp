...
:
  TLayoutWindow(parent, nullptr, nullptr),
  PercentOf(percent),
  PaneSplitter(ps)
{
...
{
  TPoint  p = point;
  MapWindowPoints(nullptr, &p, 1);  // map to screen
...
TSplitter::RemovePane(TWindow* pane, TShouldDelete::TDelete dt)
{
  ...
  if (parentSplitter) {
    ...
    pane->SetParent(nullptr);               // remove pane.
...
TVSplitter::Move(int dist)
{
  ...
  if (pane) {
    ...
    // Calc new percent.
    //
    PercentOf = static_cast<float>(lm.Width.Value) / static_cast<float>(Attr.W);
    ...
    if (splitter) {
      while (1) {
        if (splitter->SplitDirection() == SplitDirection()) {
          ...
          lm.Width.Absolute(lm.Width.Value - dist);
          splitter->SetPercent(static_cast<float>(lm.Width.Value) /
                               static_cast<float>(splitter->GetWindowAttr().W - dist));
          splitter->SetChildLayoutMetrics(*splitter->Pane1(), lm);
        }
        if ((splitter = SPLITTER(splitter->Pane1())) == nullptr)
          break;
      }
    }
  }
...
TVSplitterIndicator::Move(int dist)
{
  ...
  // than the cushion.
  //
  if( moveArea.Width() > static_cast<int>(Cushion) )  {
...
THSplitterIndicator::Move(int dist)
{
  ...
  // than the cushion.
  //
  if( moveArea.Height() > static_cast<int>(Cushion) )  {
...
#else
  ResizeCursorH = ::LoadCursor(nullptr, IDC_SIZENS);
  ResizeCursorV = ::LoadCursor(nullptr, IDC_SIZEWE);
  ResizeCursorHV = ::LoadCursor(nullptr, IDC_SIZEALL);
#endif
}
                         ...
                         TSplitDirection splitDir, float percent)
{
  ...
  if (GetFirstChild() == nullptr) {     // if no panes.
...
TPaneSplitter::PaneCount()
{
  ...
  ForEachObject(GetFirstChild(), &TPaneSplitter::DoPaneCount, &nPanes, nullptr);
  return nPanes;
}
...
TPaneSplitter::RemoveAllPanes(TDelete dt)
{
  ...
  //
  ForEachObject(GetFirstChild(), &TPaneSplitter::GetPanes, &wList, nullptr);
...
struct TTraversalRec {
  TTraversalRec() : Cnt(0), Object(nullptr) {}
    ...
    if(!tempList.Find(si)){   // if indicator not in list then process it.
      ...
      while(listIter){
        ...
        if((i = SplitterIndicatorList->FindIndicatorWithSplitter(
              SPLITTER(pane->GetParentO()))) != nullptr)
          tempList.PushBack(i);
        if (!RemovePane(pane)) {
          tempList.PushBack(i);
          nPanesNotRemoved++;
