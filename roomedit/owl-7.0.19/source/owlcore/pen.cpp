           ...
           uint32 styleCount, const uint32* style)
{
  ...
  WARNX(OwlGDI, !Handle, 0, "Cannot create TPen from brush " << static_cast<void*>(brush.GetHandle()));
  CheckValid();
  RefAdd(Handle, Pen);
}
           ...
           uint32 styleCount, const uint32* style)
{
  ...
  WARNX(OwlGDI, !Handle, 0, "Cannot create TPen from logBrush @" << static_cast<const void*>(&logBrush));
  CheckValid();
  RefAdd(Handle, Pen);
}
