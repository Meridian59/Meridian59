...
TPreviewWin::LayoutPages()
{
  ...
  if (PrinterPageSize.cx > PrinterPageSize.cy)
    ratio = (static_cast<long>(PrinterPageSize.cy) * 100) / PrinterPageSize.cx;
  else
    ratio = (static_cast<long>(PrinterPageSize.cx) * 100) / PrinterPageSize.cy;

  bool xMajor = (((r.Width() * ratio) / 100) > r.Height());
  if (xMajor) {
    metrics1.Height.Set(lmBottom, lmAbove, lmParent, lmBottom, 15);
    metrics1.Width.PercentOf(Page1, static_cast<int>(static_cast<long>(PrinterPageSize.cx) * 95 / PrinterPageSize.cy), lmHeight);
  }
  else {
    metrics1.Height.PercentOf(Page1, static_cast<int>(static_cast<long>(PrinterPageSize.cy) * 95 / PrinterPageSize.cx), lmWidth);
...
TPreviewWin::UpdateSpeedBar()
{
  ...
  if (pgGadget) {
    ...
    if (Page2 && Page2->GetPageNumber())
      _stprintf(buffer, _T("Page %d - %d"), Page1->GetPageNumber(), Page2->GetPageNumber());
    else
