...
// needed for TPrintDC::DeviceCapabilities.
//
  extern "C" {
...
//
TPrintDC::TPrintDC(const tstring& device, const DEVMODE* initData)
  : TCreatedDC{nullptr, device.c_str(), initData}
{
  memset(&DocInfo, 0, sizeof(DocInfo));
  DocInfo.cbSize = sizeof(DocInfo);
  DocInfo.lpszOutput = _T("");
...
//
/// Creates a TPrintDC object given the print driver, device, output, and initialization data.
/// Pass `nullptr` for initData to use the default initializations specified by the user.
//
