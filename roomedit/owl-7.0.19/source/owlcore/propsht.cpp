...
TPropertySheet::Run(bool modal)
{
  ...
  //
  ForEachPage(TActionPageFunc(TPropertyPage::RegisterPageChildObjects), nullptr);
...
TPropertySheet::GetPageHandles()
{
  ...
  // Double check count
  //
  CHECK(pageCount == static_cast<int>(HeaderInfo.nPages));
#endif
  return true;
}
...
TPropertySheet::ForEachPage(TActionPageFunc action, void* paramList)
{
  if (GetLastChild()) {
    ...
    } while (curChild != GetLastChild() && GetLastChild() != nullptr);
  }
}
...
{
  CHECK(GetHandle());
  return static_cast<int>(SendMessage(PSM_QUERYSIBLINGS, p1, p2));
}
...
{
  if (notifyInfo.code <= static_cast<uint>(PSN_FIRST) && notifyInfo.code >= static_cast<uint>(PSN_LAST)){
...
TPropertyPage::DialogFunction(TMsgId msg, TParam1 p1, TParam2 p2)
{
  switch(msg) {
    case WM_NOTIFY: {
        ...
        if (nmhdr.code >= static_cast<uint>(PSN_LAST) && nmhdr.code <= static_cast<uint>(PSN_FIRST)) {
          CHECK(p1 != 0);
