...
{
  int filestosave = std::min(numSavedFiles, static_cast<int>(MaxMenuItems));
  MruName = iniName;
  if(UseRegistry){
    MruName += _T("\\");
    MruName += Section;
...
TRecentFiles::CmFile(uint id)
{
  ...
  TFrameWindow* window = app ? app->GetMainWindow() : nullptr;
...
TRecentFiles::Write()
{
  if (UseRegistry)
  {
    ...
    [[maybe_unused]] const auto r = c.DeleteKey(MruName);
