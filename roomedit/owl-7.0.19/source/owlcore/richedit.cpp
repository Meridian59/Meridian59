...
TCharFormat::SetFaceName(LPCTSTR name)
{
  PRECONDITION(name);
  szFaceName[0] = _T('\0');
  ::_tcsncat(szFaceName, name, LF_FACESIZE - 1);
}
...
TCharFormat::SetFontInfo(LOGFONT& lf) const
{
  ...
  if (dwMask & CFM_ITALIC)
    lf.lfItalic = static_cast<uint8>(dwEffects & CFE_ITALIC ? TRUE : FALSE);

  if (dwMask & CFM_UNDERLINE)
    lf.lfUnderline = static_cast<uint8>(dwEffects & CFE_UNDERLINE ? TRUE : FALSE);

  if (dwMask & CFM_FACE) {
    lf.lfPitchAndFamily = bPitchAndFamily;
    lf.lfFaceName[0] = _T('\0');
    ::_tcsncat(lf.lfFaceName, szFaceName, LF_FACESIZE - 1);
  }
...
TCharFormat::GetFontInfo(const LOGFONT& lf)
{
  ...
  szFaceName[0] = _T('\0');
  ::_tcsncat(szFaceName, lf.lfFaceName, LF_FACESIZE - 1);
}
...
//
TColor
TCharFormat::GetBkColor() const
{
  if ((dwMask & CFM_BACKCOLOR)&& !(dwEffects & CFE_AUTOBACKCOLOR))
    return TColor(crBackColor);
...
TRichEdit::SetBkgndColor(const TColor& bkColor)
{
  ...
  TParam2 p2 = bkColor == TColor::None ? 0    : static_cast<COLORREF>(bkColor);
  return TColor(static_cast<COLORREF>(SendMessage(EM_SETBKGNDCOLOR, p1, p2)));
}
...
{
  return static_cast<int>(SendMessage(EM_EXSETSEL, 0, TParam2(&cr)));
}
...
{
  return static_cast<int>(CONST_CAST(TRichEdit*,this)->SendMessage(EM_GETTEXTRANGE, 0, TParam2(&tr)));
}
...
TRichEdit::GetTextRange(const TCharRange & cr, LPTSTR buffer) const
{
  ...
  const auto r = static_cast<int>(CONST_CAST(TRichEdit*, this)->SendMessage(EM_GETTEXTRANGE, 0, TParam2(&tr)));
  return r;
}
...
struct TRichEditGetTextRange
{
  ...
  int operator()(LPTSTR buf, int)
  {
    ...
    return static_cast<int>(CONST_CAST(TRichEdit*, &edit)->SendMessage(EM_GETTEXTRANGE, 0, TParam2(&tr)));
  }
};
...
{
  return static_cast<int>(CONST_CAST(TRichEdit*,this)->SendMessage(EM_GETSELTEXT, 0, TParam2(buffer)));
}
...
struct TRichEditGetSelectedText
{
  ...
  int operator()(LPTSTR buf, int)
  {return static_cast<int>(CONST_CAST(TRichEdit*, &edit)->SendMessage(EM_GETSELTEXT, 0, TParam2(buf)));}
};
...
{
  return static_cast<int>(SendMessage(EM_FINDTEXT, TParam1(flags), TParam2(&ft)));
}
...
{
  return static_cast<int>(SendMessage(EM_FINDWORDBREAK, TParam1(code), TParam2(start)));
}
...
{
  return static_cast<int>(CONST_CAST(TRichEdit*,this)->SendMessage(EM_EXLINEFROMCHAR,
                                                         0, TParam2(charPos)));
}
...
{
  return static_cast<int>(SendMessage(EM_FORMATRANGE, TParam1(render), TParam2(&fr)));
}
...
{
  return static_cast<int>(SendMessage(EM_FORMATRANGE, TParam1(TRUE), 0));
}
...
//
auto TRichEdit::GetWindowClassName() -> TWindowClassName
{
#if defined(OWL_USERICHEDIT41)
  if(TRichEditDll::Dll()->GetVersion() >= 4)
    return TWindowClassName{MSFTEDIT_CLASS};
#endif

  if(TRichEditDll::Dll()->GetVersion() >= 2)
    return TWindowClassName{RICHEDIT_CLASS};
  else
    return TWindowClassName{_T("RICHEDIT")};
}
...
TRichEdit::CharFromPos(int16 x, int16 y)
{
  ...
  return static_cast<uint32>(SendMessage(EM_CHARFROMPOS, 0, TParam2(&pt)));
}
...
TRichEdit::PosFromChar(uint charIndex)
{
  ...
  if (TRichEditDll::Dll()->GetVersion() == 2)
    return static_cast<uint32>(SendMessage(EM_POSFROMCHAR, charIndex));
  else
  {
    POINTL pt;
    SendMessage(EM_POSFROMCHAR, reinterpret_cast<TParam1>(&pt), charIndex);
    return MkUint32(static_cast<uint16>(pt.y), static_cast<uint16>(pt.x));
  }
}
...
RichEditStrmOutWithOstream(DWORD_PTR dwCookie, LPBYTE pbBuff, LONG cb, LONG *pcb)
{
  ...
  //
  *pcb = static_cast<LONG>(os.tellp() - pCnt);
  return 0;
}
