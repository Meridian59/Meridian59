...
TRichEditPrintout::~TRichEditPrintout()
{
  ...
  if (FlushCache)
  {
    const auto ok = RichEdit.GetHandle() != nullptr;
    WARN(!ok, _T("TRichEditPrintout::~TRichEditPrintout: Terminating due to failed precondition."));
    if (!ok) std::terminate();
...
TRichEditPrintout::SetPrintParams(TPrintDC* dc, TSize pageSize)
{
  ...
  if (TextLen > 0) {
    ...
    do {
      ...
      if(PageCount >= static_cast<int>(PageIndices.Size()))
