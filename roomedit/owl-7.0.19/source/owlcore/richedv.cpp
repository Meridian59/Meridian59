...
//  EV_COMMAND(CM_FILENEW,                CmFileNew),
  ...
  EV_COMMAND(CM_FORMATBKCOLOR,          CmFormatBkColor),
  EV_COMMAND_ENABLE(CM_FORMATBKCOLOR,   CeFormatBkColor),
...
:
  TRichEdit(parent, GetNextViewId(), (LPCTSTR)nullptr,0,0,0,0),
  TView(doc)
{
...
TRichEditView::CmFormatBkColor()
{
  ...
  data.Flags      = CC_RGBINIT;
  data.Color      = cf.GetBkColor();
  data.CustColors = custColors;

  if( TChooseColorDialog(this, data).Execute() == IDOK ) {
    cf.SetBkColor( data.Color );
    SetCharFormat( cf );
  }
}
...
//
void
TRichEditView::CeFormatBkColor(TCommandEnabler& tce)
{
  tce.Enable();
}
