...
TRollDialog::Resize(bool currentlyFullSize)
{
  ...
  if (WantAnimation) {
    ...
    for (int step = 0; delta < Height; step++) {
      ...
      delta = static_cast<int>(Fibonacci(step));
    }
  }
...
TRollDialog::Shrink()
{
  if (IsFullSize) {
    ...
    if (HasSystemMenu) {
      TSystemMenu menu(*this);
      menu.ModifyMenu(SC_ROLLDIALOG, MF_BYCOMMAND | MF_STRING, SC_ROLLDIALOG, _T("&Expand"));
    }
  }
}
...
TRollDialog::Expand()
{
  if (!IsFullSize) {
    ...
    if (HasSystemMenu) {
      TSystemMenu menu(*this);
      menu.ModifyMenu(SC_ROLLDIALOG, MF_BYCOMMAND | MF_STRING, SC_ROLLDIALOG, _T("&Shrink"));
    }
  }
}
