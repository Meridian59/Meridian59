...
TScroller::SetSBarRange()
{
  ...
  if (Window && Window->GetHandle()) {
    if (HasHScrollBar) {
      ...
      int newMax = std::max(0, std::min(int(XRange - 1), static_cast<int>(INT_MAX)));
    ...
    if (HasVScrollBar) {
      ...
      int newMax = std::max(0, std::min(int(YRange - 1), static_cast<int>(INT_MAX)));
      if (newMax != curMax || curMin != 0)
  ...
  if (Window && Window->GetHandle()) {
    ...
    if (newXPos != XPos || newYPos != YPos) {
      ...
      if (AutoOrg || (abs(YPos-newYPos) < YPage && abs(XPos-newXPos) < XPage))
        Window->ScrollWindow(static_cast<int>(XPos - newXPos) * XUnit,
                static_cast<int>(YPos - newYPos) * YUnit, nullptr, nullptr);
...
TScroller::AutoScroll()
{
  ...
  if (AutoMode && Window) {
    ...
    if (cursorPos.y < 0)
      dy = std::min(static_cast<long>(-YLine), std::max(static_cast<long>(-YPage), long(cursorPos.y / 10 * YLine)));

    else if (cursorPos.y > clientRect.bottom)
      dy = std::max(static_cast<long>(YLine), std::min(static_cast<long>(YPage), long((cursorPos.y-clientRect.bottom)/10 * YLine)));

    if (cursorPos.x < 0)
      dx = std::min(static_cast<long>(-XLine), std::max(static_cast<long>(-XPage), long(cursorPos.x / 10 * XLine)));

    else if (cursorPos.x > clientRect.right)
      dx = std::max(static_cast<long>(XLine), std::min(static_cast<long>(XPage), (cursorPos.x-clientRect.right)/10 * XLine));
