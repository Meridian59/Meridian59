...
TShellItem::TShellItem(const tchar* path, bool throwOnInvalidPath, HWND windowOwner)
           :TComRef<IShellFolder>(nullptr), Pidl(nullptr), ParentFolder(nullptr)
{
...
// Expanded by Val Ovechkin 11:57 AM 6/3/98
  ...
  hr = desktop->BindToObject(cs.Pidl.StripLastItem(), nullptr, IID_IShellFolder,
...
TShellItem::TShellItem(const TCreateStruct& cs)
           :TComRef<IShellFolder>(nullptr), Pidl(cs.Pidl),
...
TShellItem::TShellItem(LPITEMIDLIST pidl, const TComRef<IShellFolder>& parentFolder)
           :TComRef<IShellFolder>(nullptr), Pidl(pidl), ParentFolder(parentFolder)
{
}
...
// Expanded by Val Ovechkin 11:57 AM 6/3/98
    ...
    HRESULT hr = desktop->BindToObject(newFullPidl.StripLastItem(), nullptr, IID_IShellFolder,
          REINTERPRET_CAST(void **, p));
...
TShellItem::GetExeType(uint* major, uint* minor) const
{
  ...
  DWORD rc = TShell::SHGetFileInfo(GetPath(), 0, nullptr, 0, SHGFI_EXETYPE);
                 ...
                 HWND windowOwner) const
{
  ...
  TShell::SHChangeNotify(isFolder? SHCNE_MKDIR: SHCNE_CREATE, SHCNF_PATH /*| SHCNF_FLUSH*/,
                   STATIC_CAST(const tchar*, GetDisplayName(ForParsing)), nullptr);
  return true;
}
                 ...
                 HWND windowOwner)
{
  ...
  *this = TShellItem(newPath);              // change myself to the new location
  ...
  TShell::SHChangeNotify(isFolder? SHCNE_RMDIR: SHCNE_DELETE, SHCNF_PATH /*| SHCNF_FLUSH*/,
                   STATIC_CAST(const tchar*, oldPath), nullptr);
...
TShellItem::Delete(const USHORT flags, const tchar* title, HWND windowOwner)
{
  ...
  //
  TComRef<IShellFolder>::operator=(nullptr);
  Pidl = nullptr;
...
// Expanded by Val Ovechkin 12:29 PM 6/3/98
  ...
  HRESULT hr = ParentFolder->GetUIObjectOf(windowOwner, 1, Pidl, IID_IExtractIcon,
                                           nullptr, REINTERPRET_CAST(void**, p));
...
TShellItem::ResolveShortcut(HWND windowOwner)
{
  ...
  hr = TOle::CoCreateInstance(CLSID_ShellLink, nullptr, CLSCTX_INPROC_SERVER, IID_IShellLink,
...
// Expanded by Val Ovechkin 12:29 PM 6/3/98
    void *p = STATIC_CAST(IShellFolder**, cs.ParentFolder);
    hr = TShellItem(Desktop)->BindToObject(resolvedPidl.StripLastItem(), nullptr, IID_IShellFolder,
                    REINTERPRET_CAST(void **, p));
...
TShellItem::RetrieveIShellFolder() const
{
  ...
  if (!I) { // Need to call BindToObject
    IShellFolder* iface;
    HRESULT hr = ParentFolder->BindToObject(Pidl, nullptr, IID_IShellFolder,
...
// Expanded by Val Ovechkin 12:29 PM 6/3/98
    void *p = STATIC_CAST(IShellFolder**, cs.ParentFolder);
    HRESULT hr = desktop->BindToObject(pidl.StripLastItem(), nullptr, IID_IShellFolder
,
         REINTERPRET_CAST(void **, p));
                             ...
                             ulong* attrib) const
{
  ...
  HRESULT hr = I->ParseDisplayName(windowOwner, nullptr, TString(displayName),
...
void TShellItemIterator::Next()
{
  ...
  HRESULT hr = I->Next(1, Pidl, nullptr);
