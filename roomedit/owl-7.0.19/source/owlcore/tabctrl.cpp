...
// Version 4.70
void
TTabItem::SetState(uint32 state, uint32 m)
{
  dwState      = state;
  dwStateMask  = m;
}
...
{
    return static_cast<int>(SendMessage(TCM_INSERTITEM, index, TParam2((const TC_ITEM *)(&item))));
}
...
{
    return static_cast<int>(CONST_CAST(TTabControl*, this)->SendMessage(TCM_GETITEMCOUNT));
}
...
{
    return static_cast<int>(CONST_CAST(TTabControl*, this)->SendMessage(TCM_GETROWCOUNT));
}
...
{
    return static_cast<int>(CONST_CAST(TTabControl*, this)->SendMessage(TCM_GETCURSEL));
}
...
{
    return static_cast<int>(SendMessage(TCM_SETCURSEL, index));
}
...
{
    return static_cast<int>(SendMessage(TCM_HITTEST, 0, TParam2((TC_HITTESTINFO *)&htInfo)));
}
