...
bool TSemaphoreSet::TLock::InitLock(int count, TWaitWhat wait, int index)
{
  ...
  {
    if (wait == WaitAny) {
      if (index >= static_cast<int>(WAIT_ABANDONED_0))
    ...
    Locked = TimedOut;
  else if (index == static_cast<int>(WAIT_IO_COMPLETION))
    Locked = IoComplete;
  return false;
}
...
# if defined(BI_MULTI_THREAD_RTL)
  Handle = (HANDLE)(UINT_PTR)::_beginthreadex(nullptr, 4096, &TThread::Execute, this, 0, (uint*)&ThreadId);
# else
