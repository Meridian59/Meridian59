...
TTimeGadget::GetSystemTime(tstring& newTime)
{
  ...
  if (::GetTimeFormat(lcid, LOCALE_NOUSEROVERRIDE, &systemTime, nullptr, timeBuffer, 100)) {
    if (::GetDateFormat(lcid, LOCALE_NOUSEROVERRIDE, &systemTime, nullptr, dateBuffer, 100)) {
      newTime += dateBuffer;
      newTime += _T(" ");
      newTime += timeBuffer;
    }
