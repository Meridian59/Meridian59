...
bool
TTooltip::AddTool(HWND toolWnd, const TRect& tipArea, uint toolId, const tstring& tip, bool enableSubclassing)
{
  TToolInfo t(toolWnd, tipArea, toolId, tip);
  t.EnableSubclassing(enableSubclassing);
  return AddTool(t);
}

bool
TTooltip::AddTool(HWND toolWnd, const TRect& tipArea, uint toolId, int tipResId, TModule* m, bool enableSubclassing)
{
  PRECONDITION(GetModule());
  TModule& module = m ? *m : *GetModule();
  TToolInfo t(toolWnd, tipArea, toolId, tipResId, module.GetHandle());
  t.EnableSubclassing(enableSubclassing);
  return AddTool(t);
}
