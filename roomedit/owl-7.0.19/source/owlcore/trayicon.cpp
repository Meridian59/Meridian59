  ...
  ShouldDelete(false)
{
  ...
  Init(parent,msg,tip,new TIcon(Module?(HINSTANCE)*Module:HINSTANCE(nullptr),icon),id);
  ShouldDelete = true;
  bHidden = false;
}
//
...
bool TTrayIcon::SetIcon(TIcon& icon, TAutoDelete shouldDelete)
{
  ...
  return TShell::Shell_NotifyIcon(NIM_MODIFY, this) != FALSE;
}
