...
TBitmap& TUIFaceData::GetGlyphMask(const TSize& minSize)
{
  ...
  if (minSize.cx > MaskSize.cx || minSize.cy > MaskSize.cy) {
    ...
    MaskBm = new TBitmap(MaskSize.cx, MaskSize.cy, 1, 1, nullptr);
  }
  return *MaskBm;
}
...
TUIFace::PaintIcon(const TIcon& icon, const TRect& glyphSrcRect,
                   ...
                   const TColor& maskColor, const TColor& fillColor)
{
  // Note that this code will not work for a B/W icon! 
  //
  const auto i = icon.GetInfo();
  PaintBm(i.Color, glyphSrcRect,dc, faceRect, dstPt, state, pressed, fillFace,
          maskColor, fillColor);
}
                   ...
                   bool fillFace, const TColor& fillColor, uint16 format)
{
  ...
  switch (state) {
    ...
    case Indeterm: {
      ...
      dstDC.GrayString(TBrush(dstDC.GetTextColor()),nullptr,text, -1, textDstRect);
      if (fillFace)
         dstDC.SetBkColor(bkColor);
      break;
    }
...
{
  if (SourceType==sText)
    PaintText(Text, dc, Rect, pt, state, pressed, fillFace, FillColor, static_cast<uint16>(Format));
              ...
              int x, int y, int cx, int cy, uint flags)
{
  ...
  //
  if (hasDrawState) {
    if (::DrawState(dc, hbr, nullptr, lp, wp, x, y, cx, cy, flags))
      return true;
