...
/// "OWL_UrlLink"
//
auto TUrlLink::GetWindowClassName() -> TWindowClassName
{
  return TWindowClassName{OWL_URLLINK};
}
...
TUrlLink::SetupWindow()
{
  ...
  if (UrlAddress.empty())
    ...
    SetText(UrlAddress);
  WARN(UrlAddress.empty(), "Both control text and UrlAddress are empty in control #" << GetId() << ".");
...
//
/// Loads the stock "hand" cursor from Windows.
//
/// \note Should be available since Windows 2000.
void
  ...
  // If it failed, get the .htm regkey and lookup the program
   if (reinterpret_cast<INT_PTR>(result) <= HINSTANCE_ERROR){
    ...
    if(!key.empty()){
      ...
      if (!key.empty())
      {
        const auto pos = key.find(tstring{_T("\"%1\"")});
        if (pos == key.npos)
        {
          const auto pos = key.find(tstring{_T("%1")});
          if (pos != key.npos)
        ...
        key += url;
        _USES_CONVERSION;
        result = reinterpret_cast<HINSTANCE>(static_cast<UINT_PTR>(WinExec(_W2A(key.c_str()), showcmd)));
      }
    }
  }
  return result;
...
TUrlLink::ReportError(int error)
{
  ...
  switch (error) {
    ...
    default:{
      FormatMessageBox(LoadString(IDS_UNCKNOWNERR), _T(""), MB_ICONEXCLAMATION | MB_OK, error);
      return;
    }
  }
